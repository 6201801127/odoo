...
class AppraisalCreateWizard(models.TransientModel):

    _name = "create.appraisal.wizard"
    _description = "Wizard model for Create Appraisal"

    employee_ids = fields.Many2many(comodel_name='hr.employee',relation="create_appraisal_wiz_hr_employee_rel", column1='create_appraisal_wiz',column2='hr_emp', string='Employees',tracking=True)
    abap_period = fields.Many2one(comodel_name='account.fiscalyear', string='APAR Period')

    @api.constrains('employee_ids','abap_period')
    @api.onchange('employee_ids','abap_period')
    def _onchange_emp_abap(self):
        model_id = self.env['employee.appraisal'].sudo().search([])
        for rec in model_id:
            ...
            for record in emp_id:
                if rec.employee_id.id == record and rec.apar_period_id.id == self.abap_period.id:
                    raise ValidationError("You can not create duplicate appraisal for %s in fiscal year %s" % (rec.employee_id.first_name,rec.apar_period_id.name))
                ...
                if emp_id:
                    for rec in emp_id:
                        ...
                        if (rec in hr_contract.employee_id.ids) and (rec not in running_hr_contract.employee_id.ids):
                            raise ValidationError('Contract not in running state for %s ' % (employee_name))
                        if (rec not in hr_contract.employee_id.ids) and (rec not in running_hr_contract.employee_id.ids):
                            raise ValidationError('User %s is not in contract. so, you can not create appraisal for %s' % (employee_name,employee_name))
    ...
    def create_appraisal_action_button(self):
        for rec in self:
            for employee in rec.employee_ids:
                ...
                self.env['employee.appraisal'].create({
                    'state': 'draft',
