...
class ContractContract(models.Model):
    ...
    contract_template_id = fields.Many2one(
        string="Contract Template", comodel_name="contract.template", tracking=True
    )
    ...
    payment_term_id = fields.Many2one(
        comodel_name="account.payment.term", string="Payment Terms", index=True, tracking=True
    )
    invoice_count = fields.Integer(compute="_compute_invoice_count", tracking=True)
    ...
    partner_id = fields.Many2one(
        comodel_name="res.partner", inverse="_inverse_partner_id", required=True, tracking=True
    )
    ...
    tag_ids = fields.Many2many(comodel_name="contract.tag", string="Tags", tracking=True)
    note = fields.Text(string="Notes", tracking=True)
    is_terminated = fields.Boolean(string="Terminated", readonly=True, copy=False, tracking=True)
    ...
    linked_contract_id = fields.Many2one('contract.contract', string='Linked Contract', copy=True, index=True, tracking=True)
