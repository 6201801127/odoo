...
class GeneralLedger(models.TransientModel):
    ...
    def gl_move_lines(self, offset=0, account=0, fetch_range=FETCH_RANGE):
        cr = self.env.cr
    ...
    def write(self, vals):
        ...
        return super(GeneralLedger, self).write(vals)

    @api.model
    def create(self, vals):
        res = super(GeneralLedger, self).create(vals)
        return res
    ...
    def get_xlsx_report(self, data, response, report_data, dfr_data):
        ...
        sheet.merge_range('A2:J3', self.env.user.company_id.name + ':' + ' General Ledger', head)
        ...
        for l_list in lst:
            ...
            if n_data[l_list]['count']:
                ...
                row += 1
                row_1 += 1
                sheet.write(row +1, col, n_data[l_list]['code'], sub_heading_sub)
                sheet.write(row +1, col + 1, n_data[l_list]['name'], sub_heading_sub)
                ...
                sheet.write(row +1, col + 7, n_data[l_list]['debit'], sub_heading_total)
                sheet.write(row +1, col + 8, n_data[l_list]['credit'], sub_heading_total)
                sheet.write(row +1, col + 9, n_data[l_list]['balance'], sub_heading_total)

                for rec_1 in n_data[l_list]['lines']:
                    row_1 += 1
                    row += 1
                    if rec_1['initial_bal']:
                        ...
                        sheet.write(row +1, col + 7, n_data[l_list]['debit'], txt)
                        sheet.write(row +1, col + 8, n_data[l_list]['credit'], txt)
                        sheet.write(row +1, col + 9, n_data[l_list]['balance'], txt)
                    ...
                    if rec_1['ending_bal']:
                        sheet.write(row +1, col, 'Ending Balance', sub_heading_main)
                        sheet.write(row + 1, col + 1, '', txt)
                        sheet.write(row + 1, col + 2, '', txt)
                        sheet.write(row + 1, col + 3, '', txt)
                        sheet.write(row + 1, col + 4, '', txt)
