...
class CustomerPortal(CustomerPortal):
    ...
    def my_support_tickets(self, page=1, date_begin=None, date_end=None, sortby=None, filterby='all', search=None, groupby='none', search_in='content', **kw):
        ...
        if filterby in ['last_message_sup', 'last_message_cust']:
            ...
            messages = request.env['mail.message'].search_read([('model', '=', 'support.ticket'), ('subtype_id', '=', discussion_subtype_id)], fields=['res_id', 'author_id'], order='date desc')
            last_author_dict = {}
            for message in messages:
                if message['res_id'] not in last_author_dict:
                    last_author_dict[message['res_id']] = message['author_id'][0]

            ticket_author_list = request.env['support.ticket'].search_read(fields=['id', 'partner_id'])
        ...
        # pager
        tickets_count = len(request.env['support.ticket'].search(domain))
        pager = portal_pager(
            url="/my/tickets",
        ...
        tickets = request.env['support.ticket'].search(domain, order=order, limit=self._items_per_page, offset=pager['offset'])
        request.session['my_tickets_history'] = tickets.ids[:100]

        if groupby == 'stage':
            grouped_tickets = [request.env['support.ticket'].concat(*g) for k, g in groupbyelem(tickets, itemgetter('stage_id'))]
        else:
        ...
        return request.render("gts_ticket_management.portal_support_ticket", values)

    @http.route([
        "/support/ticket/<int:ticket_id>",
        "/support/ticket/<int:ticket_id>/<access_token>",
    ...
    def tickets_followup(self, ticket_id=None, access_token=None, **kw):
        try:
            ticket_sudo = self._document_check_access('support.ticket', ticket_id, access_token)
        except (AccessError, MissingError):
            return request.redirect('/my')

        values = self._ticket_get_page_view_values(ticket_sudo, access_token, **kw)
        return request.render("gts_ticket_management.tickets_followup", values)
