...
class WebsiteSupport(http.Controller):
    ...
    @http.route(['/support/rating', '/support/rating/<model("gts_ticket_management.team"):team>'], type='http', auth="public", website=True, sitemap=True)
    def page(self, team=False, **kw):
        # to avoid giving any access rights on Support Team to the public user, let's use sudo
        ...
        if user.has_group('gts_ticket_management.group_heldpesk_manager'):
            domain = AND([[('use_rating', '=', True)], team_domain])
        else:
            domain = AND([[('use_rating', '=', True), ('portal_show_rating', '=', True)], team_domain])
        teams = request.env['support.team'].search(domain)
        team_values = []
        for team in teams:
            tickets = request.env['support.ticket'].sudo().search([('team_id', '=', team.id)])
            domain = [
                ('res_model', '=', 'support.ticket'), ('res_id', 'in', tickets.ids),
        ...
        return request.render('gts_ticket_management.team_rating_page', {'page_name': 'rating', 'teams': team_values})
