...
class SupportTeam(models.Model):
    ...
    def create(self, vals):
        team = super(SupportTeam, self).create(vals)
        if 'use_website_support_form' in vals and vals['use_website_support_form']:
    ...
    def _ensure_submit_form_view(self):
        for team in self:
            if not team.website_form_view_id:
                default_form = etree.fromstring(self.env.ref('website_support_form.ticket_submit_form').arch)
                xmlid = 'website_support_form.team_form_' + str(team.id)
    ...
    @api.depends('name', 'use_website_support_form')
    def _compute_form_url(self):
        base_url = self.env['ir.config_parameter'].sudo().get_param('web.base.url')
        for team in self:
            team.feature_form_url = (team.use_website_support_form and team.name and team.id) and (base_url + '/support/' + slug(team)) or False
