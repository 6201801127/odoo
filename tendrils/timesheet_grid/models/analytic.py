...
class AnalyticLine(models.Model):
    ...
    @api.model
    def _read_group_task_ids(self, tasks, order,domain):
    ...
    def action_validate_timesheet(self):
        ...
        return {
            ...
            'views': [(False, 'form')],
            'context': {'get_validation_date':validation.validation_date}
        }
    ...
    def unlink(self):
        if not self.user_has_groups('hr_timesheet.group_timesheet_manager') and self.filtered(lambda r: r.is_timesheet and r.validated):
            raise AccessError(_('Only a Timesheet Manager is allowed to delete a validated entry.'))
    ...
    def adjust_grid(self, row_domain, column_field, column_value, cell_field, change):
        ...
        analytic_id = self.search(row_domain, limit=1)
