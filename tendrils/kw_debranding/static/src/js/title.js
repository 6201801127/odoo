odoo.define('kw_debranding.title', function (require) {
    "use strict";
    ...
    WebClient.include({
        init: function (parent) {
            ...
            this._rpc({
                ...
                self.set('title_part', {"zopenerp": "Tendrils v6.0"});
            });
        },
    ...
    CrashManager.include({
        rpc_error: function (error) {
            ...
            // console.log("error rpc calleld=================",error)
            if (error.code === -32098) {
        ...
        show_warning: function (error) {
            ...
            // changed Server Error to Alert 14 April 2021 (Gouranga)
            error.message = error.message && error.message.replace("Server Error", "Alert")
            // changed - validation error to none 14 April 2021 (Gouranga)
            error.data.title = error.data.title && error.data.title.replace("- Validation Error","")
        ...
        show_error: function (error) {
            // console.log("debug=====",typeof(error),error.data.debug)
            if (!this.active) {
                return;
            }
            error.message = error.message && error.message.replace("Odoo", "")
            if (!odoo.debug){
                error.data.debug = "Something went wrong.please Try again later."
                }
