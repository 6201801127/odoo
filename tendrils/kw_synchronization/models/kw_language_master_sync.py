# *******************************************************************************************************************
#  File Name             :   kw_language_master_sync.py
...
class kwemp_language_master(models.Model):
    ...
    def _check_sync_status(self):
        for record in self:
            log_record = self.env['kw_emp_sync_log'].sudo().search(
                ['&', ('model_id', '=', 'kwemp_language_master'), ('rec_id', '=', record.id)])
            if log_record:
    ...
    # --------Method For language Synchronization---------
    @api.model
    def action_sync_language(self):
        ...
        log_rec = self.env['kw_emp_sync_log'].sudo().search(['&', ('model_id', '=', 'kwemp_language_master'), ('status', '=', 0)])
        language_sync = {}
        parameterurl = self.env['ir.config_parameter'].sudo().get_param('kw_employee_sync')
        languageurl = parameterurl + 'AddLanguageDetails'
        for rec in log_rec:
            language_record = self.env['kwemp_language_master'].sudo().search([('id', '=', rec.rec_id)])
            ...
            language_sync['CreatedBy'] = str(
                self.env['hr.employee'].sudo().search([('user_id', '=', language_record.create_uid.id)]).kw_id)
            language_sync['UpdatedBy'] = str(
                self.env['hr.employee'].sudo().search([('user_id', '=', language_record.create_uid.id)]).kw_id)
    ...
    def create(self, vals):
        language_record = super(kwemp_language_master, self).create(vals)
        log_rec = self.env['kw_emp_sync_log'].sudo().search(['&', '&', ('model_id', '=', 'kwemp_language_master'),
                                                             ('rec_id', '=', language_record.id), ('status', '=', 0)])
        if len(log_rec) == 0:
            sync_data = self.env['kw_emp_sync_log'].create(
                {'model_id': 'kwemp_language_master', 'rec_id': language_record.id, 'code': False, 'status': 0})
        return language_record

    # write method for language ......
    @api.multi
    def write(self, vals):
        language_record = super(kwemp_language_master, self).write(vals)
        w_id = self.env['kwemp_language_master'].sudo().search([('id', '=', self.id)])
        if w_id.write_uid.id != 1:
            log_rec = self.env['kw_emp_sync_log'].sudo().search(['&', '&', ('model_id', '=', 'kwemp_language_master'),
                                                                 ('rec_id', '=', w_id.id), ('status', '=', 0)])
            if len(log_rec) == 0:
                sync_data = self.env['kw_emp_sync_log'].create(
                    {'model_id': 'kwemp_language_master', 'rec_id': w_id.id, 'code': False, 'status': 0})
        return language_record
