...
class AccountReport(models.AbstractModel):
    ...
    def _get_templates(self):
        return {
                'main_template': 'kw_accounting.main_template',
    ...
    def _create_hierarchy(self, lines):
        ...
        def get_hierarchy_lines(values, depth=1):
            ...
            sorted_groups = sorted((values.get('children', {}).keys()), key=lambda r: r[1])
            for key in sorted_groups:
                sum_columns, sub_lines = get_hierarchy_lines(values['children'][key], depth=values['depth'])
                header_line = {
                    'id': 'hierarchy',
                    'account_code': 'NA',
                    'name': key[1].name,  # second member of the tuple
                    'unfoldable': False,
                    'unfolded': True,
                    'level': depth,
                    'parent_id': values['parent_id'],
                    'columns': [{'name': self.with_context(accounting_format=True).format_value(c) if not isinstance(c, str) else c} for c in sum_columns],
                }
    ...
    def format_value(self, value, currency=False):
        ...
        if self.env.context.get('column_type') == 'debit':
            ...
            formatted_amount = lang.format(fmt, abs(round(value,2)), grouping=True, monetary=True)\
        ...
        if self.env.context.get('accounting_format'):
            formatted_amount = lang.format(fmt, currency_id.round(value), grouping=True, monetary=True)\
                .replace(r' ', u'\N{NO-BREAK SPACE}').replace(r'-', u'-\N{ZERO WIDTH NO-BREAK SPACE}')
    ...
    def _get_company_columns(self, options):
        return {}
    ...
    def get_xlsx(self, options, response):
        ...
        title_style = workbook.add_format({'font_name': 'Arial', 'bold': True, 'bottom': 2,'top':2})
        super_col_style = workbook.add_format({'font_name': 'Arial', 'bold': True, 'align': 'center','bottom': 2,'top':2})
        company_col_style = workbook.add_format({'font_name': 'Arial', 'bold': True, 'align': 'center','bottom': 2})
        level_0_style = workbook.add_format({'font_name': 'Arial', 'bold': True, 'font_size': 13, 'bottom': 6, 'font_color': '#666666','num_format': '#,###0.00'})
        level_1_style = workbook.add_format({'font_name': 'Arial', 'bold': True, 'font_size': 13, 'bottom': 1, 'font_color': '#666666','num_format': '#,###0.00'})
        level_2_col1_style = workbook.add_format({'font_name': 'Arial', 'bold': True, 'font_size': 12, 'font_color': '#666666', 'indent': 1,'num_format': '#,###0.00'})
        level_2_col1_total_style = workbook.add_format({'font_name': 'Arial', 'bold': True, 'font_size': 12, 'font_color': '#666666','num_format': '#,###0.00'})
        level_2_style = workbook.add_format({'font_name': 'Arial', 'bold': True, 'font_size': 12, 'font_color': '#666666','num_format': '#,###0.00'})
        level_3_col1_style = workbook.add_format({'font_name': 'Arial', 'font_size': 12, 'font_color': '#666666', 'indent': 2,'num_format': '#,###0.00'})
        level_3_col1_total_style = workbook.add_format({'font_name': 'Arial', 'bold': True, 'font_size': 12, 'font_color': '#666666', 'indent': 1,'num_format': '#,###0.00'})
        level_3_style = workbook.add_format({'font_name': 'Arial', 'font_size': 12, 'font_color': '#666666','num_format': '#,###0.00'})
        ...
        if self._get_report_name() in ['Trial Balance', 'General Ledger']:
            ...
            company_columns = self._get_company_columns(options)
            y_offset = bool(company_columns.get('columns')) and 1 or 0
            sheet.write(y_offset, 0, '', company_col_style)

        super_columns = self._get_super_columns(options)
        y_offset = bool(super_columns.get('columns'))+1 and 2 or 0
        sheet.write(y_offset, 0, '', super_col_style)

        # Todo in master: Try to put this logic elsewhere
        branch_list,company_list = [],[]
        branch_select = options.get('company_type') and [options['company_type']] or []
        for a in branch_select:
            for b in a:
                branch_list.append(b['name']) if b['selected'] == True else ''

        company_select = options.get('multi_company') and [options['multi_company']] or []
        for a in company_select:
            for b in a:
                company_list.append(b['name']) if b['selected'] == True else ''
        
        branch_l = 'Branch: ' + ','.join(branch_list)
        company_l = 'Company: ' + ','.join(company_list)
        
        company_column_data = [{'x_merge':4,'string': company_l},{'x_merge':4,'string': branch_l}]
        # company_column_data.append()

        x = 0
        for super_col in company_column_data:
            cell_content = super_col['string']
            x_merge = super_col['x_merge']
            if x_merge and x_merge > 1:
                sheet.merge_range(0, x, 0, x + (x_merge - 1), cell_content, company_col_style)
                x += x_merge
            else:
                sheet.write(0, x, cell_content, company_col_style)
                x += 1

        # x = company_columns.get('x_offset', 0)
        # for super_col in company_columns.get('columns', []):
        #     cell_content = super_col.get('string', '').replace('<br/>', ' ').replace('&nbsp;', ' ')
        #     x_merge = company_columns.get('merge')
        #     if x_merge and x_merge > 1:
        ...
        #         x += 1
        y = super_columns.get('x_offset', 0)
        for super_col in super_columns.get('columns', []):
            cell_content = super_col.get('string', '').replace('<br/>', ' ').replace('&nbsp;', ' ')
            x_merge = super_columns.get('merge')
            if x_merge and x_merge > 1:
                sheet.merge_range(1, y, 1, y + (x_merge - 1), cell_content, super_col_style)
                y += x_merge
            else:
                sheet.write(1, y, cell_content, super_col_style)
                y += 1
