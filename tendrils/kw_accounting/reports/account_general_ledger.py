...
class report_account_general_ledger(models.AbstractModel):
    ...
    def _get_templates(self):
        ...
        templates['line_template'] = 'kw_accounting.line_template_general_ledger_report'
        templates['main_table_header_template'] = 'kw_accounting.general_ledger_table_header'
        return templates
    ...
    def _get_super_columns(self, options):
        ...
        return {'columns': columns, 'x_offset': 0, 'merge': 10}
    ...
    def _do_query_unaffected_earnings(self, options, line_id, company=None):
        ...
        if company:
            aml_domain += [('branch_id.code', '=', company.code)]
        ...
        currency_convert = lambda x: self.env.user.company_id.currency_id and self.env.user.company_id.currency_id._convert(x, self.env.user.company_id.currency_id, self.env.user.company_id, date) or x
    ...
    def _group_by_account_id(self, options, line_id):
        ...
        for cid in context.get('company_type', []):
            company = self.env['accounting.branch.unit'].search([('code','=',cid['id'])])
        ...
        for account_id, result in results.items():
            ...
            if account.user_type_id == unaffected_earnings_type and account.branch_id not in unaff_earnings_treated_companies:
                #add the benefit/loss of previous fiscal year to unaffected earnings accounts
                unaffected_earnings_results = unaffected_earnings_per_company[account.branch_id]
                ...
                unaff_earnings_treated_companies.add(account.branch_id)
        ...
        for cid in context.get('company_type', []):
            ...
            if company not in unaff_earnings_treated_companies and not float_is_zero(unaffected_earnings_per_company[company]['balance'], precision_digits=user_currency.decimal_places):
                unaffected_earnings_account = self.env['account.account'].search([
                    ('user_type_id', '=', unaffected_earnings_type.id), ('branch_id.code', '=', cid['id'])
                ], limit=1)
    ...
    def _get_lines(self, options, line_id=None):
        ...
        for account in sorted_accounts:
            ...
            if offset == 0:
                lines.append({
                    ...
                    'columns': [{'name': v} for v in ['',amount_currency, self.with_context(accounting_format=True).format_value(debit), self.with_context(accounting_format=True).format_value(credit), (self.with_context(accounting_format=True).format_value(balance) if context.get('print_mode') else ((self.with_context(accounting_format=True,column_type='credit').format_value(balance)) if balance < 0 else (self.with_context(accounting_format=True,column_type='debit').format_value(balance))))]],
                    'level': 2,
                    'unfoldable': True,
            ...
            if 'account_%s' % (account.id,) in options.get('unfolded_lines') or unfold_all:
                ...
                if offset == 0:
                    domain_lines.append({
                        ...
                        'columns': [{'name': v} for v in ['', '','', '', initial_currency, '-', '-', (self.with_context(accounting_format=True).format_value(initial_balance) if context.get('print_mode') else ((self.with_context(accounting_format=True,column_type='credit').format_value(initial_balance)) if initial_balance < 0 else (self.with_context(accounting_format=True,column_type='debit').format_value(initial_balance))))]],
                    })
                ...
                for line in amls:
                    ...
                    name = line.particulars if line.particulars else ''
                    # if line.ref:
                    #     name = name
                    # if line.move_id.narration and len(line.move_id.narration) > 35 and not self.env.context.get('no_format') and not self.env.context.get('print_mode'):
                    #     narration = (line.move_id.narration[:32] + "...")  if line.move_id.narration else ''
                    # else:
                    narration = line.move_id.narration
                    name_title = name
                    ...
                    if line.invoice_id:
                        ...
                        action_context = {
                            'default_type': line.invoice_id.type,
                            'type': line.invoice_id.type,
                            'journal_type': line.invoice_id.journal_id.type,
                        }
                    elif line.payment_id:
                        caret_type = 'account.payment'
                    columns = [{'name': v} for v in [format_date(self.env, line.date), name, narration, partner_name, currency,
                                    line_debit != 0 and self.with_context(accounting_format=True).format_value(line_debit) or '',
                                    line_credit != 0 and self.with_context(accounting_format=True).format_value(line_credit) or '',
                                    (self.with_context(accounting_format=True).format_value(progress) if context.get('print_mode') else ((self.with_context(accounting_format=True,column_type='credit').format_value(progress)) if progress < 0 else (self.with_context(accounting_format=True,column_type='debit').format_value(progress))))]]
                    columns[1]['class'] = 'whitespace_print'
                    columns[2]['class'] = 'whitespace_narration'
        ...
        if not line_id:
            ...
            lines.append({
                ...
                'class': 'total',
                'level': 1,
                'columns': [{'name': v} for v in ['', '','', '', '', self.with_context(accounting_format=True).format_value(sum_debit), self.with_context(accounting_format=True).format_value(sum_credit), (self.with_context(accounting_format=True).format_value(abs(sum_balance)) if context.get('print_mode') else ((self.with_context(accounting_format=True,column_type='credit').format_value(abs(sum_balance))) if sum_balance < 0 else (self.with_context(accounting_format=True,column_type='debit').format_value(abs(sum_balance)))))]],
            })
