...
_FOLLOWUP_STATUS = [('in_need_of_action', 'In need of action'), ('with_overdue_invoices', 'With overdue invoices'),
...
class ResPartner(models.Model):
    ...
    def _search_status(self, operator, value):
        ...
        if value in ('in_need_of_action', 'with_overdue_invoices'):
            return [('id', 'in', results.filtered(lambda r: r.followup_status == value).ids)]
        return [('id', 'not in', results.ids)]
    ...
    def _get_needofaction_fup_lines_domain(self, date):
        ...
        overdue_domain = ['|', '&', ('date_maturity', '!=', False), ('date_maturity', '<', date), '&',
    ...
    def get_followup_lines_domain(self, date, overdue_only=False, only_unblocked=False):
        ...
        domain = super(ResPartner, self).get_followup_lines_domain(date, overdue_only=overdue_only,
    ...
    def get_next_action(self):
        ...
        date_auto = format_date(self.env, fields.datetime.now() + timedelta(
            days=self.env.user.company_id.days_between_two_followups), lang_code=lang_code)
    ...
    def change_expected_date(self, options=False):
        ...
        for record in self:
            ...
            msg = _('Expected pay date has been changed from %s to %s for invoice %s') % (
            old_date or _('any'), aml.expected_pay_date, aml.invoice_id.number)
    ...
    def open_action_followup(self):
        ...
        return {
            ...
            'view_mode': 'form',
            'views': [[self.env.ref('kw_accounting.customer_statements_form_view').id, 'form']],
    ...
    def get_followup_html(self):
        ...
        return self.env['account.followup.report'].with_context(print_mode=True,
                                                                lang=self.lang or self.env.user.lang).get_html(options)
