...
class Base(models.AbstractModel):
    ...
    def read_grid(self, row_fields, col_field, cell_field, domain=None, range=None):
        ...
        for r in rows:
            ...
            for c in cols:
                ...
                row[-1]['is_holiday'] = c.get('is_holiday', False)
                row[-1]['is_week_off'] = c.get('is_week_off', False)
                row[-1]['is_leave'] = c.get('is_leave', False)
    ...
    def _grid_column_info(self, name, range):
        ...
        if field.type == 'selection':
            ...
            holidays = self.env['resource.calendar.leaves'].sudo().get_calendar_global_leaves(employee_id=self.env.user.employee_ids.id, cal_start=r.start, cal_end=r.end)
            holidays_list = {r.get('date_from'):r.get('name') for r in holidays.get('holiday_calendar') if r.get('week_off') == '2'} if holidays else {}
            week_off_list = [r.get('date_from') for r in holidays.get('holiday_calendar') if r.get('week_off') == '1'] if holidays else []
            leave_days_list = self.env['hr.leave'].get_employee_leave_detail(employee_id=self.env.user.employee_ids.id, date_from=r.start, date_to=r.end)
            ...
            return ColumnMetadata(
                ...
                values=[{
                        ...
                        'is_holiday': self._grid_date_is_holiday(field, span, step, d, holidays_list.keys()),
                        'is_holiday_name': self._grid_date_is_holiday_name(field, span, step, d, holidays_list),
                        'is_week_off': self._grid_date_is_week_off(field, span, step, d, week_off_list),
                        'is_leave': self._grid_date_is_leave(field, span, step, d, leave_days_list),
    ...
    def _grid_date_is_holiday(self, field, span, step, date, holidays):
        if step == 'day':
            return date in holidays
        return False

    def _grid_date_is_holiday_name(self, field, span, step, date, holidays_name):
        if step == 'day':
            if date in holidays_name.keys():
                return holidays_name.get(date)
        return False

    def _grid_date_is_week_off(self, field, span, step, date, week_off):
        if step == 'day':
            return date in week_off
        return False
    
    def _grid_date_is_leave(self, field, span, step, date, leave_days):
        if step == 'day':
            return date in leave_days
        return False
