...
class BudgetMapping(models.Model):
    ...
    def init(self):
        ...
        CASE WHEN active is true THEN 'Active' ELSE 'Inactive' END AS active_display
        ...
        for record in emp_rec:
            if self.emp_role != record.emp_role or self.emp_category != record.emp_category:
                if self.emp_role and self.emp_category and self.budget_effective_from:
                    ...
                    sync_record = self.env['kw_emp_sync_log'].sudo().search(
                        [('model_id', '=', 'hr.employee'), ('rec_id', '=', record.id), ('code', '=', 1),
                        ('status', '=', 0)])
                    if not sync_record.exists():
                        sync_record.create({'model_id': 'hr.employee', 'rec_id': record.id, 'code': 1, 'status': 0})
                    else:
                        pass
