...
class File(models.Model):
    ...
    _inherit = [
        'kw_dms_security.mixins.access_rights',
        'kw_dms_security.mixins.locking',
        'kw_dms.mixins.thumbnail',
    ...
    active = fields.Boolean(
        string="Archived",
        default=True,
        help="If a file is set to archived, it is not displayed, but still exists.", track_visibility=True)
    ...
    def _get_binary_max_size(self):
        ...
        return int(get_param('kw_dms_utils.binary_max_size', default=25))

    @api.model
    def _get_forbidden_extensions(self):
        get_param = self.env['ir.config_parameter'].sudo().get_param
        extensions = get_param('kw_dms.forbidden_extensions', default="")
    ...
    def _search_panel_directory(self, **kwargs):
        ...
        if category_domain and len(category_domain):
            ...
            ## Start : Changed by T Ketaki 
            if len(category_domain) > 1:
                return category_domain[1][1], category_domain[1][2]
            else:
                return category_domain[0][1], category_domain[0][2]

            # return '=',category_domain[0][2]
            ## End : Changed by T Ketaki 
    ...
    def search_panel_select_range(self, field_name, **kwargs):
        ...
        if directory_id and field_name == 'directory':
            ...
            values = self.env['kw_dms.directory'].search_read(
    ...
    def search_panel_select_multi_range(self, field_name, **kwargs):
        ...
        if field_name == 'tags':
            sql_query = '''
                ...
                FROM kw_dms_tag t
                JOIN kw_dms_category c ON t.category = c.id
                LEFT JOIN kw_dms_file_tag_rel r ON t.id = r.tid 
    ...
    def _compute_path(self):
        ...
        if records_with_directory:
            ...
            paths = [list(map(int, rec.directory.parent_path.split('/')[:-1])) for rec in records_with_directory if
                     rec.directory.parent_path]
            ##condition applied by T Ketaki 
            if paths:
                model = self.env['kw_dms.directory'].with_context(dms_directory_show_path=False)
    ...
    def _compute_migration(self):
        storage_model = self.env['kw_dms.storage']
    ...
    def _get_directories_from_database(self, file_ids):
        if not file_ids:
            return self.env['kw_dms.directory']
        sql_query = '''
            SELECT directory 
            FROM kw_dms_file
        ...
        return self.env['kw_dms.directory'].browse(result)
    ...
    def _read_group_process_groupby(self, gb, query):
        ...
        directories = self.env['kw_dms.directory'].with_context(prefetch_fields=False).search([])
        if directories:
    ...
    def check_directory_access(self, operation, vals={}, raise_exception=False):
        ...
        if 'directory' in vals and vals['directory']:
            records = self.env['kw_dms.directory'].browse(vals['directory'])
        else:
    ...
    def copy(self, default=None):
        ...
        if 'directory' in default:
            model = self.env['kw_dms.directory']
