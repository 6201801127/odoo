...
var DocumentTreeMainView = ActionDocumentTreeView.extend({
	cssLibs: [
        '/kw_dms/static/lib/jquery-splitter/css/jquery.splitter.css',
    ],
    jsLibs: [
        '/kw_dms/static/lib/jquery-splitter/js/jquery.splitter.js',
    ],
    ...
    _preview_node: function(node) {
    	var self = this;
    	if(node.data && node.data.odoo_model === "kw_dms.file") {
    		if (this.manager) {
    ...
    _create_selected_node: function() {
    	var node = this.controller.getSelectedItem();
    	if(node) {
    		var parent = node.data.odoo_model === "kw_dms.file" ? this.controller.getParentNode(node) : node;
...
core.action_registry.add('kw_dms_view.documents', DocumentTreeMainView);
