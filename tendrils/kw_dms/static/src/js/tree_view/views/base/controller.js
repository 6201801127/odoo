...
odoo.define('kw_dms_view.DocumentsController', function(require) {
"use strict";
...
var files 		= require('kw_dms_utils.files');
var async 		= require('kw_dms_utils.async');
var mimetype 	= require('kw_dms_utils.mimetype');
...
var DocumentsController = Widget.extend(FileUpload, {
    ...
    refresh: function(message) {
    	...
    	if(message && message.ids && message.model && !message.create) {
    		...
    		if(id && message.model) {
    			var field = message.model === 'kw_dms.directory' ?
    ...
    getSelectedDirectory: function() {
    	...
    	var directories = _.filter(selected, function(node) {
    		return node.data.odoo_model === "kw_dms.directory";
    	});
    ...
    _checkCallback: function (operation, node, parent, position, more) {
    	if(operation === "copy_node" || operation === "move_node") {
    		...
    		if(parent.data && parent.data.odoo_model === "kw_dms.settings") {
    			return false;
            }
    		// prevent moving a child to a file
    		if(parent.data && parent.data.odoo_model === "kw_dms.file") {
    			return false;
            }
    	}
    ...
    _checkSelect: function(node, event) {
    	if(this.params.filesOnly && node.data.odoo_model !== "kw_dms.file") {
    		return false;
    	}
    	return !(node.parent === '#' && node.data.odoo_model === "kw_dms.settings");
    },
...
//    	$("#menuContinenti").prop('disabled', function (_, val) { return ! val; });
    ...
    _openInfo: function(node) {
		if(node.data.odoo_model == "kw_dms.file") {
    ...
    _moveNode: function(ev) {
		...
		} else if(node.data.odoo_model === "kw_dms.directory") {
    ...
    _copyNode: function(ev) {
		...
		if(original.data.odoo_model === "kw_dms.file") {
			vals.directory = parent.data.odoo_id;
		} else if(original.data.odoo_model === "kw_dms.directory") {
			vals.parent_directory = parent.data.odoo_id;
		}
		this._rpc({
   ...
   /* _replaceFile: function(node) {
    ...
    _loadContextMenu: function(node, callback) {
    	...
    	var jstree = this.renderer.$tree.jstree(true);
    	if(node.data) {
    		if(node.data.odoo_model === "kw_dms.directory") {
				menu = this._loadContextMenuBasic(jstree, node, menu);
				
				//commented by T Ketaki
				// menu = this._loadContextMenuDirectory(jstree, node, menu);
				
    		} else if(node.data.odoo_model === "kw_dms.file") {
    			menu = this._loadContextMenuBasic(jstree, node, menu);
