...
odoo.define('kw_dms_view.DocumentsModel', function(require) {
"use strict";
...
var DocumentsModel = Class.extend(EventDispatcherMixin, ServicesMixin, {
    	...
    	return this._rpc({
            ...
            fields: _.union(args.file.fields || [], [
            	...
            	'is_locked', 'is_lock_editor', 'extension', 
            	 '__last_update', // 'actions', 'actions_multi',
			]),
			//extra domain added for latest version file, by T Ketaki
            domain: this._buildDomain(
            	[['directory', operator, value],['next_version_id', '=',false]],
            	args.file.domain
            ),
            context: args.file.context || session.user_context,
    	...
    	this._rpc({
        	...
        	this._rpc({
                ...
                domain: this._buildDomain([
					...
					//extra domain added for latest versioned files , By T Ketaki
    ...
    _loadNode: function(node, args) {
    	var result = $.Deferred();
    	if(node.data && node.data.odoo_model === "kw_dms.storage") {
    	...
    	} else if(node.data && node.data.odoo_model === "kw_dms.directory") {
