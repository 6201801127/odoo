...
class APIController(http.Controller):
    ...
    def get(self, model=None, id=None, **payload):
        ioc_name = model
        model = request.env[self._model].sudo().search([("model", "=", model), ("rest_api", "=", True)], limit=1)
        if model:
    ...
    def put(self, model=None, id=None, **payload):
        ...
        if model:
            ...
            _model = (
                request.env[self._model].sudo().search([("model", "=", model), ("rest_api", "=", True)], limit=1)
            )
            if not _model:
                return invalid_response(
                    "invalid object model",
                    "The model %s is not available in the registry." % model.model,
                    404,
                )
            try:
    ...
    def delete(self, model=None, id=None, **payload):
        ...
        if model:
            ...
            try:
                record = request.env[model.model].sudo().search([("id", "=", _id)])
                if record:
                    record.unlink()
                else:
                    return invalid_response(
