odoo.define('recruitment.survey', function (require) {
'use strict';
...
// translation.database.parameters.date_format = '%d-%b-%Y%Y'
/*
...
if(!the_form.length) {
    ...
    $('.js_radio input[type="radio"][data-oe-survey-otherr!="1"]').click(function(){
        var _el = $(this);
        if(_el.next('span').text() == 'Absent'){
            $('input[type="number"]').val(0);
        }
        //console.log('>>>>>>>>>>>>>>>>>>>>>>', $(this).val(),$(this).next('span').text());
        ...
        if (! _.isUndefined(prefill_controller)) {
            var prefill_def = $.ajax(prefill_controller, {dataType: "json"})
                .done(function(json_data){
                    $("input.tmpdate").each(function(el){
                        //console.log('prefill controller', $(this).attr('value'));
                        $(this).val($(this).attr('value')).attr({'type':'hidden'});
                    });
