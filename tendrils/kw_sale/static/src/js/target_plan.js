odoo.define('kw_sale.individual_target_plan', function (require) {
    "use strict";
    ...
    var TargetPlanView = AbstractAction.extend(ControlPanelMixin, {
//        el:$('#recr_budget_table').find('tr.row-0').clone(),
        ...
        events: {
            ...
            'change #target_team_name': _.debounce(function(){
                var self = this;
                self.selected_team_name = $('#target_team_name').val();
                console.log('In Here', self.selected_team_name);
                self.fetch_team_member(self.selected_team_name);
                self.fetch_data('clear');
            }),
            'change #target_team_member': _.debounce(function(){
                var self = this;
                self.selected_team_member = $('#target_team_member').val();
                self.fetch_data('clear');
            }),
...
//                console.log($('tr').find('.designation').attr('data-id'));
        ...
        render: function() {
            var self = this;
            var target_plan = QWeb.render('kw_sale.individual_target_plan', {widget: self});
        ...
        fetch_team_member: function(mode){
            var self = this;
            var el = mode || false;
            return rpc.query({
                    model: 'kw_sale_target_master',
                    method: 'fetch_team_member',
                    kwargs: {'team_id': mode},
                }).then(function(result){
                    $.each(result, function(key, val){
                        $("#target_team_member").append($("<option>").html(val.name).attr({"value":val.id}));
                    });
                    //self.fetch_data('clear');
                });
        },
        fetch_team: function(mode){
            var self = this;
            var el = mode || false;
            return rpc.query({
                    model: 'kw_sale_target_master',
                    ...
                    $.each(result, function(key, val){
                        $("#target_team_name").append($("<option>").html(val.name).attr({"value":val.id}));
                    });
                    //self.fetch_data('clear');
                });
        },
        ...
        fetch_data: function(mode){
            ...
            $("#form_target_box").addClass('o_hidden');
            self.el = $('#target_plan_table').find('tr.row-0').clone();
            return rpc.query({
...
//            console.log('compute_budget');
        ...
        save_budget: function(){
            ...
            var frm = $('#form_target_box').serializeArray();
            console.log(frm);
            var params = {
                'rec':frm,
    ...
    core.action_registry.add('kw_sale.individual_target_plan', TargetPlanView);
