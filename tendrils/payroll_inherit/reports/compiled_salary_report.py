...
class CompiledSalryReport(models.Model):
    _name = 'payroll_salary_compiled_report'
    _description = 'Compiled Salary Report'
    ...
    total = fields.Integer( string='Total Amount',compute='_calculate_total_budget')
    ...
    with a as (select row_number() over() as id,sum(l.amount) FILTER (WHERE e.budget_type ='treasury' 
							and e.acc_branch_unit_id = (select id from accounting_branch_unit where code='c1' ) ) 
							as treasury_budget,
...
class CompiledSalryVeifiedReport(models.Model):
    _name = 'c1_salary_verified_report'
    _description = 'Verified C1 Salary Report'
    _order = 'rule_sequence'
    _auto = False
    ...
    with a as (select row_number() over() as id,sum(l.amount) FILTER (WHERE e.budget_type ='treasury' 
			...
			l.year as year_data,l.month as month_data,l.name as salary_rule,sequence as rule_sequence from hr_payslip_line l join hr_payslip h  on l.slip_id = h.id
	join hr_employee e on h.employee_id = e.id
	where l.year='{year_data}' and l.month={month_data} and l.code not in ('CTC') and h.company_id = {company} and h.state='done' group by l.year,l.month,l.name,sequence),
