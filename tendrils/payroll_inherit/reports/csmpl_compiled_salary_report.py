...
class CSMPLCompiledSalryReport(models.Model):
    _name = 'csmpl_salary_compiled_report'
    _description = 'CSMPL Compiled Salary Report'
    ...
    def init(self):
     ...
     with a as (select row_number() over() as id,sum(l.amount) FILTER (WHERE e.budget_type ='treasury' 
							and e.acc_branch_unit_id = (select id from accounting_branch_unit where code='csmpl' ) ) 
							as treasury_budget,
	...
	where l.year='{year_data}' and l.month={month_data} and l.code not in ('CTC') and h.company_id = {company} group by l.year,l.month,l.name,sequence),
 ...
 b as (select (select 1000) as id,count(e.id) FILTER (WHERE e.budget_type ='treasury' 
		...
		join hr_employee e on h.employee_id = e.id
		where l.year='{year_data}' and l.month={month_data} and l.code = 'CTC' and h.company_id = {company} group by l.year,l.month,l.name,sequence),
	...
	join hr_employee e on h.employee_id = e.id
    where l.year='{year_data}' and l.month={month_data} and h.company_id = {company}
    ...
    join hr_payslip h on h.id=l.slip_id  where l.year='{year_data}' and l.month={month_data} and h.company_id = {company} group by l.year,l.month,l.employee_id)
...
SELECT * FROM e
    ...
    # union
	# select 
	# (select 601) as id,sum(h.gratuity_value) FILTER (WHERE e.budget_type ='treasury' 
	# 						and e.acc_branch_unit_id = (select id from accounting_branch_unit where code='csmpl' ) and  l.code ='BASIC') 
	# 						as treasury_budget,			 
	# sum(h.gratuity_value) FILTER (WHERE e.budget_type ='project' 
	# 						and e.acc_branch_unit_id = (select id from accounting_branch_unit where code='csmpl' ) and  l.code ='BASIC') 
	# 						as project_amount,
	# l.year as year_data,l.month as month_data,(select 'Gratuity') as salary_rule,(select 499) as rule_sequence from hr_payslip_line l join hr_payslip h  on l.slip_id = h.id
	# join hr_employee e on h.employee_id = e.id
	# join hr_contract con on h.contract_id = con.id
    # where l.year='{year_data}' and l.month={month_data}
	# group by l.year,l.month


class CSMPLCompiledSalryReportVerified(models.Model):
    _name = 'csmpl_salary_verified_report'
    _description = 'CSMPL Compiled Salary Verified Report'
    _order = 'rule_sequence'
    _auto = False
    ...
    def init(self):
     ...
     with a as (select row_number() over() as id,sum(l.amount) FILTER (WHERE e.budget_type ='treasury' 
			...
			l.year as year_data,l.month as month_data,l.name as salary_rule,sequence as rule_sequence from hr_payslip_line l join hr_payslip h  on l.slip_id = h.id
	join hr_employee e on h.employee_id = e.id
	where l.year='{year_data}' and l.month={month_data} and l.code not in ('CTC') and h.state='done' and h.company_id = {company} group by l.year,l.month,l.name,sequence),
 ...
 b as (select (select 1000) as id,count(e.id) FILTER (WHERE e.budget_type ='treasury' 
		...
		from hr_payslip_line l join hr_payslip h  on l.slip_id = h.id
		join hr_employee e on h.employee_id = e.id
		where l.year='{year_data}' and l.month={month_data} and l.code = 'CTC' and h.state='done' and h.company_id = {company} group by l.year,l.month,l.name,sequence),
	...
	l.year as year_data,l.month as month_data,(select 'Total Deduction') as salary_rule,(select 499) as rule_sequence from hr_payslip_line l join hr_payslip h  on l.slip_id = h.id
	join hr_employee e on h.employee_id = e.id
    where l.year='{year_data}' and l.month={month_data} and h.state='done' and h.company_id = {company}
	group by l.year,l.month),
	...
	from hr_payslip_line l
    join hr_payslip h on h.id=l.slip_id  where l.year='{year_data}' and l.month={month_data} and h.state='done' and h.company_id = {company} group by l.year,l.month,l.employee_id)
