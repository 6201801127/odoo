odoo.define('kw_remove_export_option.remove_export_option', function (require) {
"use strict";
...
var _lt = core._lt;
    Sidebar.include({
        _addItems: function (sectionCode, items) {
            var self = this;
            if (items) {
                this.getSession().user_has_group('kw_remove_export_option.group_kw_export_manager').done(function(has_group) {
                            ...
                            for (var i = 0; i < items.length; i++) {
                                ...
                                if (items[i]['label'] == export_label) {                                 
                                    ...
                                    if ($('a[data-section="other"][data-index="0"]').length > 0)
                                        $('a[data-section="other"][data-index="0"]').remove();
                                }
                            }
                        }                        
                    }
