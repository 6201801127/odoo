...
class kw_training_report(models.Model):
    ...
    def init(self):
        ...
        self.env.cr.execute(""" CREATE or REPLACE VIEW %s as (		
			...
			(
				...
				(
					...
					(
						...
						union
						select s.training_id, employee_id, attendance_status
						from kw_training_schedule s 
						join kw_meeting_events e on e.id=s.meeting_id
						join kw_meeting_attendee a on a.event_id=e.id
					) training_attendance
			...
			select t.id as id,fiscalyear as fiscal_year, course as course, name as name, instructor_type as instructor_type, totalattendee as totalattendee
