...
class LoginCase(HttpCase):
    def test_web_login_authenticate(self, redirect_mock, *args):
        ...
        admin_password = self.env.ref('password_security.kw_test_web_login_authenticate_password').sudo().value
    ...
    def test_web_login_authenticate_fail(self, redirect_mock, *args):
        """It should fail auth"""
        no_admin_password = self.env.ref('password_security.kw_test_web_login_authenticate_fail_password').sudo().value

        response = self.url_open(
            "/web/login",
            {"login": "admin", "password": no_admin_password},
        )
