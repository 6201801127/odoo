...
if(!the_form.length) {
    ...
    $('.js_surveyform').ajaxForm({
        ...
        beforeSubmit: function(formData, $form, options){           // hide previous errmsg before resubmitting
            ...
            var button_finish = _.findWhere(formData, {'value': 'finish'});
            var button_approve = _.findWhere(formData, {'value': 'approve'});
            var button_final_approve = _.findWhere(formData, {'value': 'final_approve'});
            var button_reassessment = _.findWhere(formData, {'value': 'reassessment'});
            if (button_finish || button_approve || button_final_approve || button_reassessment){
                // alert('Called button data')
