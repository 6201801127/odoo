...
state_selection = [('draft', 'Draft'), ('applied', 'Applied'),
                   ...
                   ('grant', 'Grant'), ('cancel', 'Cancelled'), ('reject', 'Rejected')]
...
class kw_apply_salary_advance(models.Model):
    ...
    def _check_ra(self):
        for rec in self:
            if rec.current_user.has_group('kw_employee.group_hr_ra'):
    ...
    closer_type = fields.Selection(string='Closure Type', selection=[('partial', 'Partial'), ('full', 'Full')],
                                   track_visibility='onchange')
    total_month_count = fields.Integer(string='No. of Installment(s)', track_visibility='onchange')
    ...
    def advance_amt_val(self):
        if self.total_install <= 0 or self.total_install > 48 :
            raise ValidationError("No of installment can't be 0 and should be equal or less than 48.")
    ...
    def default_get(self, fields):
        ...
        if self.env.user.employee_ids.id not in users_list and self.env.user.employee_ids.date_of_joining:
            eligibility_criteria = self.env['ir.config_parameter'].sudo().get_param('kw_advance_claim.advance_eligibility_criteria')
            dote_of_join = self.env.user.employee_ids.date_of_joining + relativedelta.relativedelta(months=+int(eligibility_criteria))
            max_limit = dote_of_join - self.env.user.employee_ids.date_of_joining
            days_total = date.today() - self.env.user.employee_ids.date_of_joining
    ...
    def apply_salary_advance(self):
        ...
        if self.applicant == 'self':
            ...
            if self.employee_id.department_id.manager_id:
                email_cc_list+=self.employee_id.department_id.manager_id.mapped('work_email')
            if self.employee_id.coach_id:
                email_cc_list+=self.employee_id.coach_id.mapped('work_email')
            ...
            template_id.with_context(status=status, work_email=','.join(set(email_cc_list))).send_mail(self.id,
    ...
    @api.multi      
    def advance_settelment_move_to_release_btn(self):
            if self.state == 'paid':
                self.state = 'release'   
    ...
    def calculate_emi(self):
        ...
        if self.req_date and not self.buffer_period and not self.payment_date:
            ...
            if self.req_date < datetime.now().date() and not (self.env.user.has_group('kw_advance_claim.group_kw_advance_claim_account') or self.env.user.has_group('kw_employee.group_hr_ra')) :
                self.req_date = False
                return {'warning': {
        ...
        if self.total_install > 0 and self.adv_amnt > 0 and self.emi_start_date \
            ...
            if self.req_date and self.payment_date and self.payment_date.day > 25:
                new_dt = self.payment_date + relativedelta.relativedelta(months=1, day=1)
            elif self.req_date and self.payment_date and self.payment_date.day < 25:
                new_dt = self.payment_date
            elif self.req_date and not self.payment_date and self.req_date.day > 25:
                new_dt = self.req_date + relativedelta.relativedelta(months=1, day=1)
            elif self.req_date and not self.payment_date and self.req_date.day < 25:
                new_dt = self.req_date
            else:
                new_dt = self.req_date
                  # fetch EMI start date from db
            dt = new_dt
            r = self.interest / 100
            required_date = new_dt = self.req_date
            if self.req_date and self.payment_date and self.payment_date.day > 25:
                required_date=new_dt = self.payment_date + relativedelta.relativedelta(months=1, day=1)
            elif self.req_date and self.payment_date and self.payment_date.day <= 25:
                required_date=new_dt = self.payment_date
            elif self.req_date and not self.payment_date and self.req_date.day > 25:
                required_date=new_dt = self.req_date + relativedelta.relativedelta(months=1, day=1)
            elif self.req_date and not self.payment_date and self.req_date.day <= 25:
                required_date=new_dt = self.req_date
            else:
                required_date=new_dt = self.req_date
                ...
                for rec in range(total_instal):
                    ...
                    if new_dt:
                        ...
                        if next_month == False:
                            ...
                            dat =self.req_date
                            if self.req_date and self.payment_date and self.payment_date.day > 25:
                                dat = self.payment_date + relativedelta.relativedelta(months=1, day=1)
                            elif self.req_date and self.payment_date and self.payment_date.day < 25:
                                dat = self.payment_date
                            elif self.req_date and not self.payment_date and self.req_date.day > 25:
                                dat = self.req_date + relativedelta.relativedelta(months=1, day=1)
                            elif self.req_date and not self.payment_date and self.req_date.day < 25:
                                dat = self.req_date
                            else:
                                dat = self.req_date
                    ...
                    if new_dt:
                        ...
                        for buffer in delete_list[:self.buffer_period.buffer_period]:
                            if not buffer_count:
                                ...
                                if self.req_date and self.payment_date and self.payment_date.day > 25:
                                    next_month = self.payment_date + relativedelta.relativedelta(months=1, day=1)
                                elif self.req_date and self.payment_date and self.payment_date.day < 25:
                                    next_month = self.payment_date
                                elif self.req_date and not self.payment_date and self.req_date.day > 25:
                                    next_month = self.req_date + relativedelta.relativedelta(months=1, day=1)
                                elif self.req_date and not self.payment_date and self.req_date.day < 25:
                                    next_month = self.req_date
                                else:
                                    next_month = self.req_date
