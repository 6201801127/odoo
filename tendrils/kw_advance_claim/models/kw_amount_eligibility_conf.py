    ...
    def onchange_select_branch(self):
        for rec in self:
            if rec.select_branch == 'all':
                branch_ids = self.env['kw_res_branch'].search([])
    ...
    def validate_grade_band(self):
        if self.grade_ids and self.band_ids:
            rec_id = self.env['kw_advance_amount_eligibility_conf'].sudo().search(
                [('grade_ids', 'in', self.grade_ids.ids),('location_ids','=',self.location_ids.ids),('band_ids', 'in', self.band_ids.ids)]) - self
            if rec_id:
                for rec in rec_id:
                    raise ValidationError("Grade and Band cannot be same.")
        if self.grade_ids and not self.band_ids:
            rec_id = self.env['kw_advance_amount_eligibility_conf'].sudo().search(
                [('grade_ids', 'in', self.grade_ids.ids),('location_ids','=',self.location_ids.ids)]) - self
            if rec_id:
                for rec in rec_id:
