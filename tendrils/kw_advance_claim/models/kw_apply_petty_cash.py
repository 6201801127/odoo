...
class kw_adv_apply_petty_cash(models.Model):
    ...
    division = fields.Many2one('hr.department', string="Division", related='user_emp_id.division', store=True,
                               track_visibility='onchange')
    section = fields.Many2one('hr.department', string="Practice", related='user_emp_id.section', store=True,
                              track_visibility='onchange')
    practise = fields.Many2one('hr.department', string="Section", related='user_emp_id.practise', store=True,
                               track_visibility='onchange')
    ...
    def create(self, vals):
        ...
        if result.mail_sent == True:
            status = "Applied"
            email_cc_list=[]
            if result.user_emp_id.department_id.manager_id:
                email_cc_list+=result.user_emp_id.department_id.manager_id.mapped('work_email')
            if result.user_emp_id.coach_id:
                email_cc_list+=result.user_emp_id.coach_id.mapped('work_email')            
            ...
            template_id.with_context(work_email=','.join(set(email_cc_list)), status=status).send_mail(result.id,
    ...
    def users_grant_cc(self):
        ...
        unique_mail_list += self.user_emp_id.coach_id.mapped('work_email')
        param = self.env['ir.config_parameter'].sudo()
    ...
    def get_records(self):
        self.ensure_one()
        res = self.env['ir.actions.act_window'].for_xml_id('kw_advance_claim', 'kw_adv_claim_settlement_report_action')
