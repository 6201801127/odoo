odoo.define('web.EmpChart', function (require) {
"use strict";
...
var FieldOrgChart = AbstractField.extend({
    ...
    _render: function () {
        if (!this.recordData.id) {
            return this.$el.html(QWeb.render("hr_employee_chart", {
                managers: [],
                children: [],
            }));
        }
        ...
        return this._getOrgData(this.recordData.id).then(function () {
            self.$el.html(QWeb.render("hr_employee_chart", self.orgData));
            self.$('[data-toggle="popover"]').each(function () {
                $(this).popover({
                    html: true,
                    title: function () {
                        var $title = $(QWeb.render('hr_info_emp_popover_title', {
                            employee: {
                    ...
                    trigger: 'focus',
                    content: function () {
                        var $content = $(QWeb.render('hr_info_emp_popover_content', {
                            employee: {
                                id: $(this).data('emp-id'),
                                name: $(this).data('emp-name'),
                    ...
                    template: QWeb.render('hr_info_emp_popover', {}),
                });
            });
        });
    },
...
field_registry.add('hr_employee_chart', FieldOrgChart);
