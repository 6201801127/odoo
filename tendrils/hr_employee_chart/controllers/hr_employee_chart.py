...
class HrEmpChartController(http.Controller):
    ...
    def _prepare_employee_data(self, employee):
        ...
        return dict(
            id=employee.id,
            name=employee.name,
            link='/mail/view?model=hr_employee_info&res_id=%s' % employee.id,
            job_id=job.id,
    ...
    @http.route('/info/get_info_chart', type='json', auth='user')
    def get_info_chart(self, employee_id):
        if not employee_id:  # to check
            return {}
        employee_id = int(employee_id)

        Employee = request.env['hr_employee_info']
        # check and raise
        ...
        # compute employee data for info chart
        ancestors, current = request.env['hr_employee_info'], employee
...
class HrOrgChartController(http.Controller):
    ...
    def _prepare_employee_data(self, employee):
        ...
        return dict(
            ...
            job_id=job.id,
            job_name=job.name or '',
            direct_sub_count=len(employee.child_ids.filtered(lambda x:x.employement_type.code != 'O')),
            indirect_sub_count=employee.child_emp_all_count,
        )
    ...
    def get_org_chart(self, employee_id):
        ...
        try:
            ...
            employee = Employee.browse(employee_id).filtered(lambda x:x.employement_type.code != 'O')
        ...
        values = dict(
            self=self._prepare_employee_data(employee),
            managers=[self._prepare_employee_data(ancestor) for idx, ancestor in enumerate(ancestors) if idx < self._managers_level and ancestor.employement_type.code != 'O'],
            managers_more=len(ancestors) > self._managers_level,
            children=[self._prepare_employee_data(child) for child in employee.child_ids.filtered(lambda x:x.employement_type.code != 'O')],
        )
        values['managers'].reverse()
        return values
