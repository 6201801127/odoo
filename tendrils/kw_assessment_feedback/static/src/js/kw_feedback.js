odoo.define('kw_assessment_feedback.feedback_form', function (require) {
    ...
    if(!the_form.length) {
        ...
        console.debug("[survey] Custom JS for assesssment feedback is loading...");
        ...
        $('#kw_assessment_feedback_form').ajaxForm({
            ...
            success: function(response, status, xhr, wfe){ // submission attempt
                if(_.has(response, 'errors')){  // some questions have errors
                    _.each(_.keys(response.errors), function(key){
                        ...
                        $("#" + key + '_prefix_remark'+'>.js_errzone').append('<p>' +'This field is mandatory.' +'</p>').show();
                    });
                    return false;
                }
