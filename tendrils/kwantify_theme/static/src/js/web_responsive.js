odoo.define('kwantify_theme', function (require) {
    'use strict';
    ...
    AppsMenu.include({
        events: _.extend({
            "click .apps": "_onUsedAppsMenuItemClicked",
            // "click .other-apps": "_onAppsMenuItemClicked",
        ...
        init: function (parent, menuData) {
            ...
            this._get_most_used_apps(menuData);
        },
        ...
        _onAppsMenuItemClicked: function (ev) {
            ...
            var current_menu = $(ev.currentTarget).data('menu-id');
            var parameters = {'menuID': current_menu}
            // console.log(parameters)
            ev.preventDefault();
            this._rpc({
                model: 'res.users',
                method: 'store_recent_used_apps',
                kwargs: parameters
            }).done(function (result) {
                // console.log(result)
            });
            // this.trigger_up('Store Recent Clicked Apps', {
            //     callback: function () {
        ...
        _get_most_used_apps: function(menudata){
            var menuitems = ''
            this._rpc({
                model: 'res.users',
                method: 'get_current_users_most_used_apps',
            }).done(function (result) {
                if (result.length != 0) {
                    $.each( result, function(key,value ) {
                        // console.log(key, value)
                        menuitems += "<a class='text-center apps' role='menuitem' href='#menu_id="+value['menuId']+"' data-menu-id='"+value['menuId']+"'>\
                                <div class='apps-icon'><img \
                                     src='data:image/png;base64,"+value['web_icon_data']+"'/></div> \
                                    <span class='o-app-name'> \
                                        "+value['name']+"\
                                    </span> \
                            </a>"
                    });  
                    $('.most-used-app-items').append(menuitems);
                }
                else{
                    menuitems = "<div style='margin-top:2rem;'><img src='/kwantify_theme/static/src/img/Kwantify-Dashboard.png' style='width:85%' /></div>"
                    $('.most-used-app-items').append(menuitems);
                }
            })
        },
        _openUsedApp: function(app){
        ...
        _onUsedAppsMenuItemClicked: function (ev) {
        ...
        _searchMenus: function () {
            ...
            this.$search_results.html(
                core.qweb.render(
                    "kwantify_theme.MenuSearchResults",
                    {
                        results: results,
                        widget: this,
                    }
