...
class ResourceCalendar(models.Model):
    ...
    excluded_public_holidays_ids = fields.Many2many(
        string='Exclude Branch Holiday',
        comodel_name='hr.holidays.public.line',
        relation='public_holiday_shift_excluded_rel',
        column1='holiday_id',
        column2='shift_id', 
    ...
    def _public_holidays_leave_intervals(self, start_dt, end_dt, employee_id,tz):
        ...
        for day in rrule.rrule(rrule.YEARLY, dtstart=start_dt, until=end_dt):
            lines = HrHolidaysPublic.get_holidays_list(
                day.year, employee_id=employee_id,branch_id = self.branch_id.id if self.branch_id else False, shift_id = self.id,
            )
            # print(lines)
            lines = lines - self.excluded_public_holidays_ids
            # print(lines)
            # print('------')
            for line in lines:
                leaves.append(
                    (
                        ...
                        leave_obj.new({
                            'date_from': datetime.combine(
                                line.date, time(0, 0, 0)),
                            'date_to': datetime.combine(
                                line.date, time(23, 59, 59)),
                        })
                        # line
                    ),
                )
