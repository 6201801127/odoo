        ...
        _create_calender: function(branch_id, shift_id,personal_calendar,employee_id) {
            ...
            var calender = new Calendar('#holiday_calender', {
                ...
                customDataSourceRenderer: function(element, date, event) {
                    ...
                    if(event[0].color == '#108bbb') { $(element).css('color', '#FFF');}

                     //'color', '#FFF',

                    $(element).css('background-color', event[0].color);
                    if(event[0].overlap_public_holiday == 1) { $(element).css('background-image','repeating-linear-gradient(-45deg, rgba(0,0,0, 0.30), rgba(0,0,0, 0.30) 2px, transparent 5px, transparent 4px)');}
                ...
                mouseOnDay: function(e) {
                    if (e.events.length > 0) {
                        ...
                        for (var i in e.events) {
                            ...
                            if(e.events[i].prime_color == '#ffffff'){color  = '#108bbb'}
                ...
                if (result) {
                    ...
                    $.each(result, function(key, value) {
                        ...
                        if (value.week_off == '2') {
                            ...
                            holiday_div += '<li class="row ' + value.year + ' holiday_year mb-3"><span class="col-4 pr-0 font-weight-bold" style="color: #d83d2b">' + value.holidayDate + '</span><span class="col-8 pl-0">' + value.name + '</span></li>'
                        }
