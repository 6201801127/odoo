odoo.define('kw_surveys.survey_validator', function (require) {
    'use strict';
...
var the_form = $('.js_kw_survey_form');

if(!the_form.length) {
    return $.Deferred().reject("DOM doesn't contain '.js_kw_survey_form'");
}
    ...
    if (_.isUndefined(submit_controller)) {
        $(".js_kw_survey_form .input-group-text span.fa-calendar").css("pointer-events", "none");
        $('.js_kw_survey_form :input').prop('disabled', true);
        print_mode = true;
    }
    ...
    console.debug("[survey] Custom JS for kw_survey loaded!");
