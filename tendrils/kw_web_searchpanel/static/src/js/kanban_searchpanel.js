...
odoo.define('kw_web_searchpanel.SearchPanel', function (require) {
"use strict";
...
var SearchPanel = Widget.extend({
    ...
    _createCategoryTree: function (categoryId, values) {
        ...
        // Sort values array alphabetically by name
        // console.log('check values coming =================',values)
        var keyToCheck = 'sort_no';
        var containsKey = values.some(obj => obj.hasOwnProperty(keyToCheck));

        if (containsKey) {
            values.sort(function(a, b) {
                var aValue = (typeof a.sort_no !== 'undefined') ? a.sort_no : '';
                var bValue = (typeof b.sort_no !== 'undefined') ? b.sort_no : '';
        ...
        // values.sort(function(a, b) {
        //     console.log('find the values of a,b=====',a,b)
        //     var aValue = (typeof a.name !== 'undefined') ? a.name : '';
        //     var bValue = (typeof b.name !== 'undefined') ? b.name : '';
        //     return aValue.localeCompare(bValue);
        // });
     
        // Iterate through sorted values
        values.forEach(function (value) {
            var key = (typeof(value.sequence) == 'undefined') ? value.id : ++x;
            category.values[key] = _.extend({}, value, {
                childrenIds: [],
                folded: true,
        ...
        // Sort parent values by name
        var parentValues = Object.keys(category.values)
            .map(function(valueId) {
                return category.values[valueId];
            })
            .filter(function(value) {
                return value.parentId === false;
            });
     
        // parentValues.sort(function(a, b) {
        //     var nameA = (typeof a.name !== 'undefined') ? a.name : '';
        //     var nameB = (typeof b.name !== 'undefined') ? b.name : '';
        ...
        if (containsKey) {
            ...
            parentValues.sort(function(a, b) {
                    var nameA = (typeof a.sort_no !== 'undefined') ? a.sort_no : '';
                    var nameB = (typeof b.sort_no !== 'undefined') ? b.sort_no : '';
                    // return nameA.localeCompare(nameB);
                    return nameA - nameB
        ...
        // Sort children under each parent value by name
        Object.keys(category.values).forEach(function (valueId) {
            var value = category.values[valueId];
            if (value.childrenIds.length > 0 && containsKey) {
                value.childrenIds.sort(function(a, b) {
                    var childA = category.values[a];
                    var childB = category.values[b];
                    var nameA = (typeof childA.sort_no !== 'undefined') ? childA.sort_no : '';
                    var nameB = (typeof childB.sort_no !== 'undefined') ? childB.sort_no : '';
                    // return nameA.localeCompare(nameB);
                    return nameA - nameB
                });
            }
        ...
        // Set rootIds
        category.rootIds = parentValues.map(function(parent) {
            return parent.id;
        });
    ...
    _getCategoryDomain: function () {
        var self = this;
        //function to travers all child ids
        function getChildValueIds(category,ch_ids) {
            var all_child_ids = [];
           
            ch_ids.forEach(function(ch_id){
                var gch_ids = [];
                if(category.values[ch_id].childrenIds){

                    all_child_ids = all_child_ids.concat(category.values[ch_id].childrenIds)  
                    gch_ids       = getChildValueIds(category,category.values[ch_id].childrenIds)

                    if(gch_ids){
                        all_child_ids = all_child_ids.concat(gch_ids)  
                    }
                } 
        ...
        function categoryToDomain(domain, categoryId) {
            ...
            if (category.activeValueId) {
                var activeId = category.activeValueId

                var kw_child_ids    = category.values[activeId].childrenIds
                var all_child_ids   = getChildValueIds(category,kw_child_ids);
               

                all_child_ids       = all_child_ids.concat(kw_child_ids) 

                if(all_child_ids.length >0 ){                     
                    domain.push('|',[category.fieldName, '=', category.activeValueId],[category.fieldName, 'in', all_child_ids]);
