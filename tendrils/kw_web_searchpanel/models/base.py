...
class Base(models.AbstractModel): 
    ...
    def search_panel_select_range(self, field_name, **kwargs):
        ...
        order = 'sequence asc' if 'sequence' in Comodel._fields else ''

        fields.append('display_name')
        parent_name = Comodel._parent_name if Comodel._parent_name in Comodel._fields else False
        if parent_name:
            fields.append(parent_name)
        if 'sequence' in Comodel._fields:
            fields.append('sequence')
        if 'sort_no' in Comodel._fields:
            fields.append('sort_no')
        # print(Comodel)
        # print(fields)

        return {
            'parent_field': parent_name,
            'values': Comodel.with_context(hierarchical_naming=False).search_read([], fields, order=order),
        }
