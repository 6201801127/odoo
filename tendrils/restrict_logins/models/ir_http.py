        ...
        try:
            if request.session.uid:
                ...
                def _update_user(u_sid, u_now, u_exp_date, u_uid):
                    ...
                    """
                    # print(u_uid)
                    # print("inside update user  ---"+u_sid)
                    if u_uid and u_exp_date and u_sid and u_now:
                        ...
                        try:
                            ...
                            # print("""Exception during updating user session...%s""" %(str(e)))
                            pass
                ...
                if 'options' in request_params and 'bus_inactivity' in request_params['options']:
                    # update session if there is sid mismatch
                    if uid and user_pool.sid and sid != user_pool.sid:
                        # print("if sid mismatch--")
                        _update_user(sid, now, exp_date, uid)
                        
                else:
                    # update if there is no session data and user is active
                    if not user_pool.last_update and not user_pool.sid and not user_pool.logged_in:
                        # print("if no session data --")                                                
                        _update_user(sid, now, exp_date, uid)
