    ...
    var KsDashboardNinja = AbstractAction.extend(ControlPanelMixin, {
        ...
        events: {
            ...
            'click .subordinates-timesheet': _.debounce(function () {
                ...
                this.renderSubordinatesTimesheet()
            }, 200, true),
            ...
            'click .refresh_health_insurance': _.debounce(function () { this.renderHealthInsurance('refresh_health_insurance') }, 200, true),
            'click .refresh_debtor_list': _.debounce(function () { this.renderDebtorListData('refresh_debtor_list'); this.renderCountDebtorListData('refresh_debtor_list'); }, 200, true),
            'click .total_debtor_list': _.debounce(function () { this.renderDebtorListData('total_debtor_list'); this.renderCountDebtorListData('total_debtor_list'); }, 200, true),
            'click .expired_debtor_list': _.debounce(function () { this.renderDebtorListData('expired_debtor_list'); this.renderCountDebtorListData('expired_debtor_list'); }, 200, true),
            'click .within_90_days_debtor_list': _.debounce(function () { this.renderDebtorListData('within_90_days_debtor_list'); this.renderCountDebtorListData('within_90_days_debtor_list'); }, 200, true),
            'click .within_91_180_days_debtor_list': _.debounce(function () { this.renderDebtorListData('within_91_180_days_debtor_list'); this.renderCountDebtorListData('within_91_180_days_debtor_list'); }, 200, true),
            'click .above_180_days_debtor_list': _.debounce(function () { this.renderDebtorListData('above_180_days_debtor_list'); this.renderCountDebtorListData('above_180_days_debtor_list'); }, 200, true),
            'click .refresh_guest_house': _.debounce(function () { this.renderGuestHouseData('refresh_guest') }, 200, true),
            'click .guest_house_booking': _.debounce(function () { this.render_book_guest_house() }, 200, true),
            'click .refresh_opp_port': _.debounce(function () { this.renderOpportunityData('refresh_opp'); this.renderCountOppListData('refresh_opp'); }, 200, true),
            'click .total_opp_list': _.debounce(function () { this.renderOpportunityData('total_opp_list'); this.renderCountOppListData('total_opp_list'); }, 200, true),
            'click .expired_opp_list': _.debounce(function () { this.renderOpportunityData('expired_opp_list'); this.renderCountOppListData('expired_opp_list'); }, 200, true),
            'click .pac_approved_opp_list': _.debounce(function () { this.renderOpportunityData('pac_approved_opp_list'); this.renderCountOppListData('pac_approved_opp_list'); }, 200, true),
            'click .pending_at_pac_opp_list': _.debounce(function () { this.renderOpportunityData('pending_at_pac_opp_list'); this.renderCountOppListData('pending_at_pac_opp_list'); }, 200, true),
            'click .not_applied_to_pac_opp_list': _.debounce(function () { this.renderOpportunityData('not_applied_to_pac_opp_list'); this.renderCountOppListData('not_applied_to_pac_opp_list'); }, 200, true),
            'click .refresh_billing_port': _.debounce(function () { this.renderBillingData('refresh_billing_port') }, 200, true),
            ...
            // guesthouse images preview.
            'click .preview_images': _.debounce(function (e) {
                var self = this;
                var guestHouseId = $(e.currentTarget).data('guest-house-id');
                console.log(guestHouseId)
                self.guest_house_images_preview(guestHouseId)
            }, 200, true),

            'click .toggleAmount': _.debounce(function (e) {
                var self = this;
                self.toggleAmountVisibility()
            }, 200, true),

            //Click Event For Opportunity Portlet

            'click .open_modal': _.debounce(function (e) {
                self.oppid = e.currentTarget.dataset.oppid;
                $('#OppPortModal').attr('data-oppid', self.oppid);
            }, 200, true),

            'click #btnRespondDateSubmit': _.debounce(function (e) {
                var self = this;
                var given_date = $('#respond_opp_lf').val();
                var opp_id = $('#OppPortModal').attr('data-oppid');
                self.openPopup(opp_id, given_date);
            }, 200, true),

            'click .open_modal_debt': _.debounce(function (e) {
                self.debtid = e.currentTarget.dataset.debtid;
                $('#DebtPortModal').attr('data-debtid', self.debtid);
            }, 200, true),

            'click #btnRespondDebtorDateSubmit': _.debounce(function (e) {
                var self = this;
                var given_date = $('#respond_dlf').val();
                var debt_id = $('#DebtPortModal').attr('data-debtid');
                self.openDebtPopup(debt_id, given_date);
            }, 200, true),

            'click #btnRespondViewAll': _.debounce(function (e) {
                var self = this;
                var projectId = $(e.currentTarget).data('project-id');
                var sbuName = $(e.currentTarget).data('sbu-name');
                var woCode = $(e.currentTarget).data('wo-code');
                var woName = $(e.currentTarget).data('wo-name');

                self.do_action({
                    name: _t("Milestone Details"),
                    type: 'ir.actions.act_window',
                    res_model: 'kw_billing_dashboard_port',
                    view_id: 'kw_crm.kw_billing_port_expand_tree_view',
                    views: [[false, 'list']],
                    target: 'new',
                    domain: [['project_id', '=', projectId], ['sbu_name', '=', sbuName], ['wo_code', '=', woCode], ['wo_name', '=', woName]],
                },
        ...
        ksRenderDashboardItems: function (items) {
            ...
            for (var i = 0; i < items.length; i++) {
                if (self.grid) {
                    ...
                    } else if (items[i].ks_item_code === 'health_insurance') {
                        self._renderHealthInsurancePortlet(items[i], self.grid)
                    } else if (items[i].ks_item_code === 'guest_house') {
                        self._renderGuestHousePortlet(items[i], self.grid)
                    } else if (items[i].ks_item_code === 'opp_port') {
                        self._renderOpportunityPortlet(items[i], self.grid)
                    } else if (items[i].ks_item_code === 'debtor_list') {
                        self._renderDebtorListPortlet(items[i], self.grid)
                    } else if (items[i].ks_item_code === 'billing_port') {
                        self._renderBillingPortlet(items[i], self.grid)
                    }
                    else {
        ...
        ksUpdateDashboardItem: function (ids) {
            ...
            for (var i = 0; i < ids.length; i++) {
                ...
                if (item_data['ks_dashboard_item_type'] == 'ks_list_view') {
                    ...
                    self.$el.find(".grid-stack-item[data-gs-id=" + item_data.id + "]").find(".ks_dashboarditem_id").replaceWith($(item_view).find('.ks_dashboarditem_id'));
                } else if (item_data['ks_item_code'] == 'health_insurance') {
                    var item_view = self._renderHealthInsurancePortlet(item_data);
                    self.$el.find(".grid-stack-item[data-gs-id=" + item_data.id + "]").find(".ks_dashboarditem_id").replaceWith($(item_view).find('.ks_dashboarditem_id'));
                } else if (item_data['ks_item_code'] == 'guest_house') {
                    var item_view = self._renderGuestHousePortlet(item_data);
                    self.$el.find(".grid-stack-item[data-gs-id=" + item_data.id + "]").find(".ks_dashboarditem_id").replaceWith($(item_view).find('.ks_dashboarditem_id'));
                } else if (item_data['ks_item_code'] == 'debtor_list') {
                    var item_view = self._renderDebtorListPortlet(item_data);
                    self.$el.find(".grid-stack-item[data-gs-id=" + item_data.id + "]").find(".ks_dashboarditem_id").replaceWith($(item_view).find('.ks_dashboarditem_id'));
                }
                else if (item_data['ks_item_code'] == 'opp_port') {
                    var item_view = self._renderOpportunityPortlet(item_data);
                    self.$el.find(".grid-stack-item[data-gs-id=" + item_data.id + "]").find(".ks_dashboarditem_id").replaceWith($(item_view).find('.ks_dashboarditem_id'));
                }
                else if (item_data['ks_item_code'] == 'billing_port') {
                    var item_view = self._renderBillingPortlet(item_data);
                    self.$el.find(".grid-stack-item[data-gs-id=" + item_data.id + "]").find(".ks_dashboarditem_id").replaceWith($(item_view).find('.ks_dashboarditem_id'));
                }
                else {
        ...
        renderEmployeeDirectory: async function (status) {
            ...
            return this._rpc({
                ...
                var kwantify_employee_directory = $(QWeb.render('KwantifyEmployeeDirectory', {
                    'has_group_employee_manager': self.employee_data.kwantify_employee_list[7],
        ...
        renderTeamAttendance: async function (status) {
            ...
            return this._rpc({
                ...
                var team_attendance_portlet_data = $(QWeb.render('TeamAttendanceData', {
                    ...
                    team_wfh_data: self.team_attendance_status.team_attendance_data[4],
                    team_wfh_data_length: self.team_attendance_status.team_attendance_data[4].length,
                    time_list: self.team_attendance_status.team_attendance_data[5],
                }));
        ...
        applyTour: function () {
            window.location.href = 'web#&action=kw_tour.action_kw_tour_act_window&model=kw_tour&view_type=form';
        },
        ...
        wish_employee: async function (status) {
            ...
            if (status == 'birthday') {
                ...
                greeting_context = {
                    'search_default_today_greetings': 1,
        ...
        _renderLostAndFoundPortlet: async function (item, grid) {
            ...
            var $ks_gridstack_container = $(QWeb.render('GridstackLostAndFoundPortletContainer', {
                ksIsDashboardManager: self.ks_dashboard_data.ks_dashboard_manager,
                ks_dashboard_list: self.ks_dashboard_data.ks_dashboard_list,
                lost_found_spoc: self.ks_dashboard_data.lost_found_spoc,
                item_id: item.id,
                item_name: item.name,
            })).addClass('ks_dashboarditem_id');
        ...
        _renderHealthInsurancePortlet: async function (item, grid) {
            var self = this;
            var $ksItemContainer = self.renderHealthInsurance();
            var $ks_gridstack_container = $(QWeb.render('GridstackHealthInsuranceContainer', {
                ksIsDashboardManager: self.ks_dashboard_data.ks_dashboard_manager,
                ks_dashboard_list: self.ks_dashboard_data.ks_dashboard_list,
                item_id: item.id,
                item_name: item.name,
            ...
            $ks_gridstack_container.find('#dashboard_health_insurance_block').before("<div class='preload preload-container d-flex align-items-center justify-content-center'><span></span><span></span><span></span></div>");
            var data = $ks_gridstack_container.find('#dashboard_health_insurance_block').append($ksItemContainer);
            data.hide();
            $.when($ks_gridstack_container.find(".preload").delay(100).fadeOut("slow")).then(function () {
                $ks_gridstack_container.find('.preload').remove();
                $.when($ks_gridstack_container.find('.preload').remove()).then(function () {
        ...
        renderHealthInsurance: async function (status) {
            framework.unblockUI();
            var self = this;
            return this._rpc({
                model: 'ks_dashboard_ninja.board',
                method: 'get_health_insurance_data',
                context: self.getContext(),
            }).done(function (result) {
                console.log(result)
                $('#dashboard_health_insurance_block').empty();
                self.health_insurance_data = result;
                var health_insurance = $(QWeb.render('HealthInsurance', {
                    'first_health_insurance': self.health_insurance_data.health_insurance_data[0],
                    'first_health_insurance_length': self.health_insurance_data.health_insurance_data[0].length,
                    'rest_health_insurance': self.health_insurance_data.health_insurance_data[1],
                    'rest_health_insurance_length': self.health_insurance_data.health_insurance_data[1].length,
                    'first_accidental_policy': self.health_insurance_data.health_insurance_data[2],
                    'first_accidental_policy_length': self.health_insurance_data.health_insurance_data[2].length,
                    'rest_accidental_policy': self.health_insurance_data.health_insurance_data[3],
                    'rest_accidental_policy_length': self.health_insurance_data.health_insurance_data[3].length
                }));
                if (status == 'refresh_health_insurance') {
                    $('#dashboard_health_insurance_block').before("<div class='preload preload-container d-flex align-items-center justify-content-center'><span></span><span></span><span></span></div>");
                    var data = $('#dashboard_health_insurance_block').append(health_insurance);
                    data.hide();
                    $.when($(".preload").delay(100).fadeOut("slow")).then(function () {
                        $('.preload').remove();
                        $.when($('.preload').remove()).then(function () {
                    ...
                    $('#dashboard_health_insurance_block').append(health_insurance);
                }
            });
        },
        ...
        _renderDebtorListPortlet: async function (item, grid) {
            var self = this;
            var $ksItemContainer = self.renderDebtorListData();
            self.renderCountDebtorListData();
            var $ks_gridstack_container = $(QWeb.render('GridstackDebtorListContainer')).addClass('ks_dashboarditem_id');

            $ks_gridstack_container.find('#DebtorListDataLoad').before("<div class='preload preload-container d-flex align-items-center justify-content-center'><span></span><span></span><span></span></div>");
            var data = $ks_gridstack_container.find('#DebtorListDataLoad').append($ksItemContainer);
            data.hide();
            $.when($ks_gridstack_container.find(".preload").delay(100).fadeOut("slow")).then(function () {
                $ks_gridstack_container.find('.preload').remove();
                $.when($ks_gridstack_container.find('.preload').remove()).then(function () {
            ...
            item.$el = $ks_gridstack_container;
            if (item.id in self.gridstackConfig) {
                grid.addWidget($ks_gridstack_container, self.gridstackConfig[item.id].x, self.gridstackConfig[item.id].y, 36, self.gridstackConfig[item.id].height, true, 10, null, 6, 6, item.id);
            } else {
                grid.addWidget($ks_gridstack_container, 0, 6, 36, 6, true, 10, null, 6, 6, item.id);
            }
        },
        renderCountDebtorListData: async function (status) {
            framework.unblockUI();
            var self = this;
            return this._rpc({
                model: 'ks_dashboard_ninja.board',
                method: 'get_count_debtor_list_data',
                context: self.getContext(),
            }).done(function (result) {
                self.count_debtor_list_data = result['count_debtor_opp_list_data'];
                $('.span_total_debtor_list').html('Total Debtors: ');
                $('.span_total_debtor_list_val').html(self.count_debtor_list_data['len_total']);
                $('.span_expired_debtor_list').text('(Collection Plan Expired) Nos Of Invoices / Values : ');
                $('.span_expired_debtor_list_val').text(self.count_debtor_list_data['len_exp'] + ' / ' + self.count_debtor_list_data['total_expired_debtor_value'] + 'Cr');
                $('.span_within_90_days_debtor_list').text('Debtors (Within 90 Days): ');
                $('.span_within_90_days_debtor_list_val').text(self.count_debtor_list_data['len_90']);
                $('.span_within_91_180_days_debtor_list').text('Debtors (91 - 180 Days): ');
                $('.span_within_91_180_days_debtor_list_val').text(self.count_debtor_list_data['len_91_180']);
                $('.span_above_180_days_debtor_list').text('Debtors (Above 180 Days): ');
                $('.span_above_180_days_debtor_list_val').text(self.count_debtor_list_data['len_180']);

                if (status == 'expired_debtor_list') {
                    $('.sales_current_records_debt').text(' || Expired Collection Plan : ' + self.count_debtor_list_data['len_exp']);
                } else if (status == 'within_90_days_debtor_list') {
                    $('.sales_current_records_debt').text(' || Within 90 Days : ' + self.count_debtor_list_data['len_90']);
                } else if (status == 'within_91_180_days_debtor_list') {
                    $('.sales_current_records_debt').text(' || 91 - 180 Days : ' + self.count_debtor_list_data['len_91_180']);
                } else if (status == 'above_180_days_debtor_list') {
                    $('.sales_current_records_debt').text(' || Above 180 Days : ' + self.count_debtor_list_data['len_180']);
                } else {
                    $('.sales_current_records_debt').text(' || Total : ' + self.count_debtor_list_data['len_total']);
                }
            });
        },

        renderDebtorListData: async function (status) {
            framework.unblockUI();
            var self = this;
            return this._rpc({
                model: 'ks_dashboard_ninja.board',
                method: 'get_debtor_list_data',
                context: self.getContext(),
                args: [status],
            }).done(function (result) {
                $('#DebtorListDataLoad').empty();
                self.debtor_list_data = result['debtor_list_data'][0];
                var debtor_lists = $(QWeb.render('DebtorInsideTemplate', {
                    'guest': self.debtor_list_data
                }));
                if (status == 'refresh_debtor_list') {
                    $('#DebtorListDataLoad').before("<div class='preload preload-container d-flex align-items-center justify-content-center'><span></span><span></span><span></span></div>");
                    var data = $('#DebtorListDataLoad').append(debtor_lists);
                    data.hide();
                    $.when($(".preload").delay(100).fadeOut("slow")).then(function () {
                        $('.preload').remove();
                        $.when($('.preload').remove()).then(function () {
                    ...
                    $('#DebtorListDataLoad').append(debtor_lists);
                }
            });
        },

        _renderGuestHousePortlet: async function (item, grid) {
            var self = this;
            var $ksItemContainer = self.renderGuestHouseData();
            var $ks_gridstack_container = $(QWeb.render('GridstackGuestHouseContainer')).addClass('ks_dashboarditem_id');

            $ks_gridstack_container.find('#GuestHouseDataLoad').before("<div class='preload preload-container d-flex align-items-center justify-content-center'><span></span><span></span><span></span></div>");
            var data = $ks_gridstack_container.find('#GuestHouseDataLoad').append($ksItemContainer);
            data.hide();
            $.when($ks_gridstack_container.find(".preload").delay(100).fadeOut("slow")).then(function () {
                $ks_gridstack_container.find('.preload').remove();
                $.when($ks_gridstack_container.find('.preload').remove()).then(function () {
        ...
        renderGuestHouseData: async function (status) {
            framework.unblockUI();
            var self = this;
            return this._rpc({
                model: 'ks_dashboard_ninja.board',
                method: 'get_guest_house_data',
                context: self.getContext(),
            }).done(function (result) {
                $('#GuestHouseDataLoad').empty();
                self.guest_house_data = result['guest_house_data'][0];
                var guest_house = $(QWeb.render('GuestHouseInsideTemplate', {
                    'guest': self.guest_house_data
                }));
                if (status == 'refresh_guest') {
                    $('#GuestHouseDataLoad').before("<div class='preload preload-container d-flex align-items-center justify-content-center'><span></span><span></span><span></span></div>");
                    var data = $('#GuestHouseDataLoad').append(guest_house);
                    data.hide();
                    $.when($(".preload").delay(100).fadeOut("slow")).then(function () {
                        $('.preload').remove();
                        $.when($('.preload').remove()).then(function () {
                    ...
                    $('#GuestHouseDataLoad').append(guest_house);
                }
            });
        },

        render_book_guest_house: function (guestHouseId) {
            var self = this;
            return this._rpc({
                model: 'ks_dashboard_ninja.board',
                method: 'get_guest_house_data',
                ...
                this.do_action({
                    type: 'ir.actions.act_window',
                    res_model: 'kw_guest_house_booking',
                    view_id: 'kw_guest_house.kw_guest_house_booking_form',
                    views: [
                        [false, 'form']
                    ],
                    target: 'current',
                    context: { 'addition_requirments': 'reserve' }
                });
            });
        },


        guest_house_images_preview: function (guestHouseId) {
            return this._rpc({
                model: 'ks_dashboard_ninja.board',
                method: 'get_guest_house_data',
                context: this.getContext(),
                ...
                this.do_action({
                    type: 'ir.actions.act_window',
                    res_model: 'kw_guest_house_photo_master',
                    view_id: 'kw_guest_house.kw_guesthouse_image_preview',
                    views: [
                        [false, 'kanban']
                    ],
                    target: 'new',
                    domain: [['photo_guest_house_id', '=', guestHouseId]],
                    context: { 'create': 0 }
                });
            });
        },

        //        Start Opportunity portlet

        _renderOpportunityPortlet: async function (item, grid) {
            var self = this;
            self.renderCountOppListData();
            var $ksItemContainer = self.renderOpportunityData();
            var $ks_gridstack_container = $(QWeb.render('GridstackOpportunityPortletContainer')).addClass('ks_dashboarditem_id');

            $ks_gridstack_container.find('#OpportunityPortletDataLoad').before("<div class='preload preload-container d-flex align-items-center justify-content-center'><span></span><span></span><span></span></div>");
            var data = $ks_gridstack_container.find('#OpportunityPortletDataLoad').append($ksItemContainer);
            data.hide();
            $.when($ks_gridstack_container.find(".preload").delay(100).fadeOut("slow")).then(function () {
                $ks_gridstack_container.find('.preload').remove();
                $.when($ks_gridstack_container.find('.preload').remove()).then(function () {
        ...
        renderCountOppListData: async function (status) {
            framework.unblockUI();
            var self = this;
            return this._rpc({
                model: 'ks_dashboard_ninja.board',
                ...
                self.count_opp_list_data = result['count_debtor_opp_list_data'];
                $('.span_total_opp_list').text('Nos of Opp / Values : ');
                $('.span_total_opp_list_val').text(self.count_opp_list_data['len_opp_total'] + ' / ' + self.count_opp_list_data['total_opp_val'] + 'Cr');
                $('.span_expired_opp_list').text('(Closing Date Expired) Nos of Opp / Values : ');
                $('.span_expired_opp_list_val').text(self.count_opp_list_data['len_opp_exp'] + ' / ' + self.count_opp_list_data['total_expired_opp_val'] + 'Cr');
                $('.span_pac_approved_opp_list').text('(PAC Approved) Nos of Opp / Values : ');
                $('.span_pac_approved_opp_list_val').text(self.count_opp_list_data['len_pac_3'] + ' / ' + self.count_opp_list_data['total_pac_3_opp_val'] + 'Cr');
                $('.span_pending_at_pac_opp_list').text('(Pending at PAC) Nos of Opp / Values : ');
                $('.span_pending_at_pac_opp_list_val').text(self.count_opp_list_data['len_pac_2'] + ' / ' + self.count_opp_list_data['total_pac_2_opp_val'] + 'Cr');
                $('.span_not_applied_to_pac_opp_list').text('(Not Applied to PAC) Nos of Opp / Values : ');
                $('.span_not_applied_to_pac_opp_list_val').text(self.count_opp_list_data['len_pac_1'] + ' / ' + self.count_opp_list_data['total_pac_1_opp_val'] + 'Cr');

                if (status == 'total_opp_list') {
                    $('.sales_current_records_opp').text(' || Total : ' + self.count_opp_list_data['len_opp_total']);
                } else if (status == 'expired_opp_list') {
                    $('.sales_current_records_opp').text(' || Expired Closing Date : ' + self.count_opp_list_data['len_opp_exp']);
                } else if (status == 'pending_at_pac_opp_list') {
                    $('.sales_current_records_opp').text(' || Pending At PAC : ' + self.count_opp_list_data['len_pac_2']);
                } else if (status == 'not_applied_to_pac_opp_list') {
                    $('.sales_current_records_opp').text(' || Not Applied to PAC : ' + self.count_opp_list_data['len_pac_1']);
                } else {
                    $('.sales_current_records_opp').text(' || PAC Approved : ' + self.count_opp_list_data['len_pac_3']);
                }
            });
        },


        renderOpportunityData: async function (status) {
            framework.unblockUI();
            var self = this;
            return this._rpc({
                model: 'ks_dashboard_ninja.board',
                method: 'get_opportunity_data',
                context: self.getContext(),
                args: [status],
            }).done(function (result) {
                $('#OpportunityPortletDataLoad').empty();
                self.opp_data = result['opportunity_data'][0];
                var oppor_data = $(QWeb.render('OpportunityPortletInsideTemplate', {
                    'opp_data': self.opp_data
                }));

                if (status == 'refresh_opp') {
                    $('#OpportunityPortletDataLoad').before("<div class='preload preload-container d-flex align-items-center justify-content-center'><span></span><span></span><span></span></div>");
                    var data = $('#OpportunityPortletDataLoad').append(oppor_data);
                    data.hide();
                    $.when($(".preload").delay(100).fadeOut("slow")).then(function () {
                        $('.preload').remove();
                        $.when($('.preload').remove()).then(function () {
                    ...
                    $('#OpportunityPortletDataLoad').append(oppor_data);
                }

            });
        },

        openPopup: function (opp_id, given_date) {
            var self = this;
            return this._rpc({
                model: 'kw_opp_port',
                method: 'update_model_field',
                args: [opp_id, given_date],
                context: this.getContext(),
            }).done(function (result) {
                $('.modal-backdrop').remove();
                self.renderOpportunityData('refresh_opp');
            })
        },


        toggleAmountVisibility: function () {
            var isChecked = $('.toggleAmount').is(":checked");
            if (!isChecked) {
                $('.amount-column').hide();
            } else {
                $('.amount-column').show();
        ...
        openDebtPopup: function (debt_id, given_date) {
            var self = this;
            return this._rpc({
                model: 'kw_debtor_list_master',
                method: 'update_debt_model_field',
                args: [debt_id, given_date],
                context: this.getContext(),
            }).done(function (result) {
                $('.modal-backdrop').remove();

                self.renderDebtorListData('refresh_debtor_list');
            })
        },

        // SALES BILLING PORTLET 

        _renderBillingPortlet: async function (item, grid) {
            var self = this;
            var $ksItemContainer = self.renderBillingData();
            var $ks_gridstack_container = $(QWeb.render('GridstackBillingPortletContainer')).addClass('ks_dashboarditem_id');

            $ks_gridstack_container.find('#BillingPortletDataLoad').before("<div class='preload preload-container d-flex align-items-center justify-content-center'><span></span><span></span><span></span></div>");
            var data = $ks_gridstack_container.find('#BillingPortletDataLoad').append($ksItemContainer);
            data.hide();
            $.when($ks_gridstack_container.find(".preload").delay(100).fadeOut("slow")).then(function () {
                $ks_gridstack_container.find('.preload').remove();
                $.when($ks_gridstack_container.find('.preload').remove()).then(function () {
        ...
        renderBillingData: async function (status) {
            framework.unblockUI();
            var self = this;
            return this._rpc({
                model: 'ks_dashboard_ninja.board',
                ...
                $('#BillingPortletDataLoad').empty();
                self.billing_data = result['billing_data'][0];
                var bill_data = $(QWeb.render('BillingPortletInsideTemplate', {
                    'billing_data': self.billing_data
                }));

                if (status == 'refresh_billing_port') {
                    $('#BillingPortletDataLoad').before("<div class='preload preload-container d-flex align-items-center justify-content-center'><span></span><span></span><span></span></div>");
                    var data = $('#BillingPortletDataLoad').append(bill_data);
                    data.hide();
                    $.when($(".preload").delay(100).fadeOut("slow")).then(function () {
                        $('.preload').remove();
                        $.when($('.preload').remove()).then(function () {
                    ...
                    $('#BillingPortletDataLoad').append(bill_data);
                }
            });
        },

        openBillingPopup: function (billing_id, given_date) {
            var self = this;
            return this._rpc({
                model: 'kw_billing_dashboard_port',
                method: 'update_billing_model_field',
                args: [billing_id, given_date],
                context: this.getContext(),
            }).done(function (result) {
                $('.modal-backdrop').remove();

                self.renderBillingData('refresh_billing_port');
            });
        }
