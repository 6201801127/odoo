...
class CustomerPortal(CustomerPortal):
    ...
    def _prepare_portal_layout_values(self):
        ...
        ticket_count = (
            request.env["grievance.ticket"].search_count([])
            if request.env["grievance.ticket"].check_access_rights(
    ...
    def _grievance_ticket_check_access(self, ticket_id):
        ticket = request.env['grievance.ticket'].browse([ticket_id])
    ...
    @http.route(
        ['/my/grievances', '/my/grievances/page/<int:page>'],
        type='http',
        auth="user",
        website=True,
    )
    def portal_my_grievances(
            self,
            page=1,
            date_begin=None,
            date_end=None,
        ...
        HelpdesTicket = request.env['grievance.ticket']
        domain = []
        ...
        for stage in request.env['grievance.ticket.stage'].search([]):
        ...
        return request.render("kw_grievance_new.portal_my_grievances", values)

    @http.route(['/my/grievance/<int:ticket_id>'], type='http', website=True)
    def portal_my_grievance(self, ticket_id=None, **kw):
        try:
            ticket_sudo = self._grievance_ticket_check_access(ticket_id)
        except AccessError:
            return request.redirect('/my')
        values = self._ticket_get_page_view_values(ticket_sudo, **kw)
        return request.render("kw_grievance_new.portal_grievance_ticket_page",
                              values)

    def _ticket_get_page_view_values(self, ticket, **kwargs):
        closed_stages = request.env['grievance.ticket.stage'].search(
            [('closed', '=', True)])
        files = request.env['ir.attachment'].search(
            [('res_model', '=', 'grievance.ticket'), ('res_id', '=', ticket.id)])
        values = {
            'page_name': 'ticket',
            'ticket': ticket,
