...
class TestPartner(SavepointCase):
    def setUp(self):
        ...
        self.ticket_obj = self.env["grievance.ticket"]
        ...
        self.parent_id.grievance_ticket_ids = [(4, self.tickets[0].id)]
        self.child_id_1.grievance_ticket_ids = [(4, self.tickets[1].id)]
        self.child_id_2.grievance_ticket_ids = [(4, self.tickets[2].id)]
        self.child_id_3.grievance_ticket_ids = [(4, self.tickets[3].id)]
        self.child_id_3.grievance_ticket_ids[-1].stage_id = self.stage_id_closed

    def test_ticket_count(self):
        self.assertEqual(self.parent_id.grievance_ticket_count, 4)

    def test_ticket_active_count(self):
        self.assertEqual(self.parent_id.grievance_ticket_active_count, 3)

    def test_ticket_string(self):

        self.assertEqual(self.parent_id.grievance_ticket_count_string, "3 / 4")
