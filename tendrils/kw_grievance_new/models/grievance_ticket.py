...
class GrievanceTicket(models.Model):
    ...
    # call_type_id = fields.Many2one(comodel_name="grievance.calltype.master", string="Call Type", required=True)
    ...
    attachment_type     = fields.Selection([('none','None'),('attachment','Attachment'),('url','URL'),('video','Video'),('audio','Audio')],default='none',tracking=True)
    ...
    approver_user_ids = fields.Many2many(
        ...
        column1 ='tickets_id',
        column2 ='us_id',
        relation='ticket_user_approvers_rel',
        string='New Users')
    ...
    # users_id = fields.Many2one(comodel_name='res.users', string='Employee', required=True, default=lambda self: self.env.user.id)
    users_id = fields.Many2one(comodel_name='hr.employee', string='Employee', required=True,readonly=True, default=get_employee)
    ...
    def is_assign_ticket_check_compute(self):
        for record in self:
            if record.user_id.id:
                ...
                # elif self.env.user.has_group('kw_grievance_new.group_grievance_officer'):
                #     record.is_assign_ticket = True
    ...
    def _compute_access_url(self):
        super(GrievanceTicket, self)._compute_access_url()
        for ticket in self:
    ...
    def _notify_get_groups(self, message, groups):
        groups = super(GrievanceTicket, self)._notify_get_groups(message, groups)
        self.ensure_one()
    ...
    def _onchange_users_id(self):
        if self.users_id:
            self.users_email = self.users_id.work_email
            # # self.emp_code = self.users_id.email
            self.emp_dept = self.users_id.department_id.name
            self.emp_desig = self.users_id.job_id.name
    @api.multi
    ...
    #     cat_master = self.env['grievance.ticket.category'].sudo().search([('id','=',cat_id)],limit=1)
    ...
    #     # email = ",".join(mail_to) or ''
        ...
        # template = self.env.ref('kw_grievance_new.create_email_template_for_grievance_ticket')
        # email_cc = ','.join(manager.mapped('email'))
    
        # template.with_context(email_from=res.users_email, email_to=email_cc, names=res.users_id.name).send_mail(
        #     res.id, notif_layout="kwantify_theme.csm_mail_notification_light")
    #     if random_team_member:
    #         template2 = self.env.ref('kw_grievance_new.email_template_assigned_grievance_ticket')
    #         template2.with_context(to_name=res.user_id.name, email_to=res.user_id.email, email_from=email_cc).send_mail(
    #             res.id, notif_layout="kwantify_theme.csm_mail_notification_light")
        return res
    ...
    def copy(self, default=None):
        ...
        res = super(GrievanceTicket, self).copy(default)
        return res
    ...
    def write(self, vals):
        ...
        return super(GrievanceTicket, self).write(vals)

    def _prepare_ticket_number(self, values):
        seq = self.env["ir.sequence"]
        if "company_id" in values:
            seq = seq.with_context(force_company=values["company_id"])
        return seq.next_by_code("grievance.ticket.sequence") or "/"
    ...
    def _track_template(self, tracking):
        res = super(GrievanceTicket, self)._track_template(tracking)
        test_task = self[0]
    ...
    #     self_rec = self.env['grievance.ticket'].sudo().search([('stage_id.name', '=', 'New')])
    #     user = self.env['res.users'].sudo().search([])
    #     manager = user.filtered(lambda user: user.has_group('kw_grievance_new.group_grievance_manager') == True)
    ...
    #         for rec in self_rec:
    #             if not rec.user_id:
    #                 template = self.env.ref('kw_grievance_new.email_template_for_timer')
    #                 template.with_context(email_to=to_email, names=user_name).send_mail(rec.id,
    #                                                                                 notif_layout="kwantify_theme.csm_mail_notification_light")

    # def archive_grievance_ticket_from_cron(self):
    #     self_rec = self.env['grievance.ticket'].sudo().search([('stage_id.name', '=', 'Done')])
    ...
    def assign_spoc_action(self):
        # cat_id = self.category_id.team
        return {
            'name': self.name,
            'view_type': 'form',
            'view_mode': 'form',
            'res_model': "grievance.incident.wizard",
            'view_id':self.env.ref('kw_grievance_new.grievance_incident_wizard_form_view').id,
    ...
    def raise_grievance_action(self):
        team_id = []
        random_team = 0
        team_member = []
        random_team_member = 0
        ...
        if self.request == 'self':
            ...
            if random_team_member:
                template2 = self.env.ref('kw_grievance_new.email_template_assigned_grievance_ticket')
                template2.with_context(to_name=self.user_id.name, email_to=self.user_id.email, email_cc=email_cc).send_mail(
                    self.id, notif_layout="kwantify_theme.csm_mail_notification_light")
    ...
    def grievance_ticket_auto_assign(self):
        data = self.env['grievance.ticket'].sudo().search([('stage_id', 'in', [1, 2])])
        for rec in data:
            if rec.assigned_date and rec.create_date:
                ...
                if cron_date:
                    if date.today() >= cron_date.date() and rec.stage_id.id in [2, 1]:
                        random_emp_data = self.env['grievance.ticket.team'].sudo().search([('id', '=', rec.team_id.id)], limit=1)
                ...
                if rec.create_date and not rec.assigned_date and rec.stage_id.id == 1:
                    ...
                    if cron_date:
                        if date.today() >= cron_date.date():
                            random_emp_data = self.env['grievance.ticket.team'].sudo().search([('name', '=', rec.category_id.name.id)], limit=1)
    ...
    def _search(self, args, offset=0, limit=None, order=None, count=False, access_rights_uid=None):
        if self._context.get('take_action'):
            if self.env.user.has_group('kw_grievance_new.group_grievance_manager'):
                ...
                # print(args)
        return super(GrievanceTicket, self)._search(args, offset=offset, limit=limit, order=order, count=count, access_rights_uid=access_rights_uid)