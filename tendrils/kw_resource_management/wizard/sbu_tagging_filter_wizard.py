...
class SBUTaggingFilter(models.TransientModel):
    _name           = 'sbu_tagging_filter_wizard'
    _description    = "SBU Tagging Filter Wizard"
    ...
    @api.multi
    def button_tagging_employees_filter(self):
        tree_view_id = self.env.ref('kw_resource_management.sbu_resource_tagging_tree').id
        domain = []
        if self.emp_role:
        ...
        action = {
            ...
            'view_mode': 'tree',
            'view_type': 'form',
            'res_model': 'sbu_resource_tagging',
            'target': 'main',
            'domain': domain
        }
        return action