...
class KsDashboardNinjaItems(models.Model):
    ...
    name = fields.Char(string="Name", size=50, default=lambda self: self._context['item_name'] if 'item_name' in self._context else False)
    ...
    portlet_master_id = fields.Many2one('portlet_master',string="Portlet", default=lambda self: self._context[
                                                    'portlet_id'] if 'portlet_id' in self._context else False)
    ...
    item_code = fields.Char("Item Code",default=lambda self: self._context['item_code'] if 'item_code' in self._context else False)
    is_published = fields.Boolean(string="Published", default=True)
    common_portlet = fields.Boolean(string="Visible To All", default=True)
    employee_visible = fields.Many2many('hr.employee',string="Employees To Visible", required=True)
    ...
    def name_validation(self):
        ...
        for record in self:
            for i in range(len(record.name)):
                ...
                if record.name[i] in s:
                    c += 1   # if special character found then add 1 to the c
        if c:
            raise ValidationError("Special Character are not allowed in Name.")
    ...
    def create(self, values):
        ...
        res =  super(KsDashboardNinjaItems, self).create(values)
        user_portlet_ids = self.env['kw_user_portlets'].search([('ks_dashboard_ninja_board_id','=',res.ks_dashboard_ninja_board_id.id)])
        for rec in user_portlet_ids:
            rec.write({
                'ks_dashboard_items_id':[(4, res.id)]
            })
    ...
    @api.onchange('common_portlet')
    def onchange_common_portlet(self):
        if self.common_portlet == True:
            self.employee_visible = [(6,0,[])]

    @api.constrains('name','item_code')
    def check_duplicate_items(self):
        if re.match("^[a-zA-Z0-9 ,./()_+-]+", self.name) == None:
            raise ValidationError("Invalid item name! Please provide a valid name.")

        record = self.env['ks_dashboard_ninja.item'].search([]) - self
        for info in record:
            if not self.item_code:
                if info.name.lower() == self.name.lower():
                    raise ValidationError(f"{self.name} already exists.")
            else:
                # print(info.name.lower(), self.name.lower(), info.item_code.lower(), self.item_code.lower())
                if info.ks_dashboard_ninja_board_id.id == self.ks_dashboard_ninja_board_id.id and (info.name == self.name or info.item_code == self.item_code):
                    raise ValidationError(f"{self.name} already exists.")

    @api.onchange('portlet_master_id')
    def get_item_code_onchange(self):
        self.name = False
        self.item_code = False
        portlet_master = self.env['portlet_master'].search([('id','=',self.portlet_master_id.id)])
        self.name = portlet_master.name or ''
        self.item_code = portlet_master.portlet_code
