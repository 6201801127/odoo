...
class KsDashboardNinjaBoard(models.Model):
    ...
    name = fields.Char(string="Dashboard Name", required=True, size=50, help="Only 50 character allowed")
    ks_dashboard_items_ids = fields.One2many('ks_dashboard_ninja.item', 'ks_dashboard_ninja_board_id',
                                             string='Dashboard Items')
    ks_dashboard_menu_name = fields.Char(string="Menu Name",limit=50)
    ...
    @api.constrains('name','ks_dashboard_menu_name')
    def name_validation(self):
        for record in self:
            if not (re.match('^[ a-zA-Z\s]*$', record.name)) :
                raise ValidationError("Dashboard Name accepts only alphabet values.")
            if not (re.match('^[ a-zA-Z\s]*$', record.ks_dashboard_menu_name)):
                raise ValidationError(" Menu Name accepts only alphabet values.")
    ...
    def kwantify_employee_directory(self,args, limit=15):
        ...
        if 'offset' in args:
            if 'operation' in args:
                if args.get('operation') == 'empSearch':
                    ...
                    if args.get('value') == 'present':
                        attendance = 'Present'
                        present_attendance_records = self.env['hr.attendance'].sudo().search([('atten_date','=',datetime.date.today()),('check_in','!=',False)]).mapped('employee_id').ids
                        ...
                        attendance = 'Absent'
                        absent_attendance_records = self.env['hr.attendance'].sudo().search([('atten_date','=',datetime.date.today()),('check_in','=',False),('day_status', 'in',['0','3'])]).mapped('employee_id').ids
                        ...
                        attendance = 'Tour'
                        tour_attendance_records = self.env['hr.attendance'].sudo().search([('atten_date','=',datetime.date.today())]).mapped('employee_id').ids
                        ...
                        attendance = 'Leave'
                        leave_attendance_records = self.env['hr.attendance'].sudo().search([('atten_date','=',datetime.date.today())]).mapped('employee_id').ids
            ...
            query = f"{emp_table} {join_table} where e.name ilike '%{args.get('empSearch')}%' or e.work_email ilike '%{args.get('empSearch')}%' or e.mobile_phone ilike '%{args.get('empSearch')}%' "
        ...
        if len(employees) == limit or len(offset_employees) == limit:
            load_button = True
        print(employees_list, offset_employees_list, '***********')
        for emp in employees:
            ...
            if emp.active == True:
                ...
                state = ''
                city = ''
                for address in emp.address_ids:
                    if address.address_type == 'present_add':
                ...
                employees_list.append({
                    ...
                    'dob': emp.birthday,
                    # 'date_of_joining' : datetime.datetime.strftime(emp.transfer_date, "%d-%b-%Y") if emp.transfer_date else 'NA',
                    ...
                    # 'today_attendance_status' : emp.today_attendance_status,
                    'present_address1': street,
                    'present_address2': street2,
        ...
        for off_emp in offset_employees:
            if off_emp.active == True and off_emp.employement_type.code != 'O':
                offset_employees_list.append({
                    ...
                    'today_attendance_status' : off_emp.today_attendance_status,
        ...
        return [department, employees_list, offset_employees_list, location, attendance, departments_filter, load_button]
    ...
    def current_month_attendance(self):
        ...
        attendance_records = self.env['hr.attendance'].search([('employee_id.user_id','=',self.env.user.id),('create_date','=',current_date)])
        ...
        month_attendance_records = self.env['hr.attendance'].search([('employee_id.user_id','=',self.env.user.id)])
        for data in month_attendance_records:
            if data.create_date.month == current_month and data.create_date.year == current_year:
                # if data.check_in != False and data.state in ['1','7','8']:
                if data.check_in != False:
                    present_data_list = [data.create_date.year, data.create_date.month, data.create_date.day, 'present']
                    present_days.append(present_data_list)

                if data.check_in == False and data.day_status in ['0','3']:
                    absent_data_list = [data.create_date.year, data.create_date.month, data.create_date.day, 'absent']
                    absent_days.append(absent_data_list)

                # if data.leave_status != False:
                #     leave_data_list = [data.create_date.year, data.create_date.month, data.create_date.day, 'leave']
                #     leave_days.append(leave_data_list)

                # if data.check_in_status in ['2','3'] and data.day_status in ['0','3']:
                #     late_entry_data_list = [data.create_date.year, data.create_date.month, data.create_date.day, 'late_entry']
                #     late_entry_days.append(late_entry_data_list)
        
        # attendance_data = self.env['hr.attendance'].search([('employee_id.user_id','=',self.env.user.id)], order="attendance_recorded_date asc")
        attendance_data = self.env['hr.attendance'].search([('employee_id.user_id','=',self.env.user.id)])
        for rec in attendance_data:
            emp_tz = rec.employee_id.tz or 'UTC' 
            emp_timezone = pytz.timezone(emp_tz)
            worked = 0
            if rec.create_date.month == current_month and rec.create_date.year == current_year:
                ...
                # if rec.shift_id.hours_per_day > 0:
                #     worked = (rec.worked_hours/rec.shift_id.hours_per_day)*100
                worked = float("%.2f" % worked)
                color = '#28a745' if rec.check_in != False else '#ff9800' if rec.check_in == False and rec.day_status in ['0','3']  else '#108fbb' if rec.check_in_status in ['2','3'] and rec.day_status in ['0','3'] else '#7f7f7f'
        ...
        print(start_date,'deepak')
        attendance_start_date = ''
        if start_date:
            attendance_start_date = start_date['date'].day
    ...
    def current_employee_subordinates(self):
        ...
        if has_group_attendance_manager:
            ...
            for rec in emp_records:
                currentUserAndSubordinates.append({'id': rec.id,'name': rec.name})
        else:
            currentUserAndSubordinates.append({'id': employee_id.id,'name': employee_id.name})
            if subordinates_ids:
                for record in subordinates_ids:
                    currentUserAndSubordinates.append({'id': record.id,'name': record.name})
    ...
    def get_filter_attendance_data(self, args):
        ...
        attendance_records = self.env['hr.attendance'].search([('employee_id','=',int(employeeSelect))])
        for record in attendance_records:
            if record.check_in.strftime("%m") == monthSelect and record.check_in.year == int(yearSelect):
                if record.check_in != False:
                    present_data_list = [record.create_date.year, record.create_date.month, record.create_date.day, 'present']
                    present_days.append(present_data_list)

                if record.check_in == False and record.day_status in ['0','3']:
                    absent_data_list = [record.create_date.year, record.create_date.month, record.create_date.day, 'absent']
                    absent_days.append(absent_data_list)
        ...
        attendance_data = self.env['hr.attendance'].search([('employee_id','=',int(employeeSelect))])
        for rec in attendance_data:
            emp_tz = rec.employee_id.tz or 'UTC' 
            emp_timezone = pytz.timezone(emp_tz)
            worked = 0
            color = '#28a745' if rec.check_in != False else '#ff9800' if rec.check_in == False and rec.day_status in ['0','3']  else '#108fbb' if rec.check_in_status in ['2','3'] and rec.day_status in ['0','3'] else '#7f7f7f'
            if rec.check_in.strftime("%m") == monthSelect and rec.check_in.year == int(yearSelect):
                check_in = datetime.datetime.strftime(rec.check_in.replace(tzinfo=pytz.utc).astimezone(emp_timezone), "%I:%M %p") if rec.check_in else  ''
                check_out = datetime.datetime.strftime(rec.check_out.replace(tzinfo=pytz.utc).astimezone(emp_timezone), "%I:%M %p") if rec.check_out else  ''
                # if rec.shift_id.hours_per_day > 0:
                #     worked = (rec.worked_hours/rec.shift_id.hours_per_day)*100
    ...
    def team_attendance_data(self):
        team_absent, team_leave, team_late_entry, team_late_exit = [], [], [], []
        current_date = datetime.datetime.now().date()
        absent_attendance = self.env['hr.attendance'].search([('employee_id.parent_id.user_id','=',self.env.user.id),('atten_date','=',current_date),('check_in','=',False),('day_status','in',['0','3'])])
        for record in absent_attendance:
            team_absent.append({
                'id': record.id,
                'attendance_date': record.atten_date,
        ...
        leave_attendance = self.env['hr.attendance'].search([('employee_id.parent_id.user_id','=',self.env.user.id),('atten_date','=',current_date)])
        ...
        late_entry_attendance = self.env['hr.attendance'].search([('employee_id.parent_id.user_id','=',self.env.user.id),('atten_date','=',current_date),('check_in_status','in',['2','3']),('day_status','in',['0','3'])])
        ...
        late_exit_attendance = self.env['hr.attendance'].search([('employee_id.parent_id.user_id','=',self.env.user.id),('atten_date','=',(current_date - datetime.timedelta(days=1))),('check_out_status','in',['2','3']),('day_status','in',['0','3'])])
        ...
        # wfh_attendance = self.env['hr.attendance'].search([('employee_id.parent_id.user_id','=',self.env.user.id),('atten_date','=',current_date),('day_status','in',['0','3'])])
        ...
        time_list = self.env['cms.announcement']._get_time_list()
        return [team_absent, team_leave, team_late_entry, team_late_exit, time_list]
    ...
    def get_meeting_room_availability(self, meeting_date):
        ...
        calendar_events = event_calendar.sudo().search(
            [('state', '!=', 'cancelled'), ('recurrency', '=', False), ('start', '>=', start_datetime),('start', '<', end_datetime)])
    ...
    def get_holiday_calendar_data(self,args):
        ...
        if args:
            ...
            calendar_data = self.env['resource.calendar.leaves'].get_calendar_global_leaves(self, branch_id, shift_id, personal_calendar, employee_id)
        else:
            branch_id = self.env.user.employee_ids.resource_calendar_id.branch_id.id 
            personal_calendar = 1
            shift_id = self.env.user.employee_ids.resource_calendar_id.id
        ...
        for rec in calendar_data:
            for data in rec.get('holiday_calendar'):
    ...
    def get_grievance_data(self):
        grievance_datas = []
        grievances = self.env['bsap.grievance'].sudo().search([('state','=','closed')], order="id desc")
        for grievance in grievances:
            emp_attendance = self.env['hr.attendance'].search([('employee_id','=',grievance.employee_id.id),('atten_date','=',datetime.date.today()),('check_in','!=',False)],limit=1)
            # if grievance.start_date == datetime.date.today() and grievance.category_id.alias == 'late_entry' and emp_attendance:
            #     grievance.update({'active': False})
            grievance_datas.append({
                'id': grievance.id,
                'state' : grievance.state,
                'employee_id' : grievance.employee_id.name,
                'grievance_code' : grievance.grievance_code,
                'grievance_type' : grievance.grievance_type_id.name, 
                'description': grievance.description,
                'create_uid': grievance.create_uid.name,
                'create_date': grievance.create_date
            })
        return grievance_datas

    def get_tour_data(self):
        current_date = datetime.date.today()
        tour_list = []
        # tour_ids = self.env['bsscl.tour'].sudo().search([('state','in',['approved_by_manager','approved_by_deputy']),('date_of_travel','<=',datetime.date.today()),('return_date','>=',datetime.date.today())])
        tour_ids = self.env['bsscl.tour'].sudo().search([('state','in',['approved_by_manager','approved_by_deputy'])])
        for tour in tour_ids:
            cities = []
            city_list = []
            for index,data in enumerate(tour.tour_details_ids):
                if index == 0:
                    cities.append({
                    ...
                    city_list.append(data.from_city)
                    cities.append({
    ...
    def get_leave_and_absent_data(self):
        ...
        leave_data = self.env['hr.leave'].sudo().search([('state','=','validate'),('employee_id.user_id','=',self.env.user.id),('date_from','<=',datetime.datetime.today().date()),('date_to','>=',datetime.datetime.today().date())])
    ...
    def get_attendance_count(self):
        ...
        for time in time_list:
            ...
            attendance_ids = self.env['hr.attendance'].sudo().search([('atten_date','=',datetime.date.today())])
    ...
    def get_self_timesheet(self, args):
        ...
        present_attendance_ids = self.env['kw_daily_employee_attendance'].search([('employee_id','=',self.env.user.employee_ids.id),('attendance_recorded_date','>=',start_date),('attendance_recorded_date','<=',end_date),('check_in','!=',False),('day_status','in',['0','3'])])
        absent_attendance_ids = self.env['kw_daily_employee_attendance'].search([('employee_id','=',self.env.user.employee_ids.id),('attendance_recorded_date','>=',start_date),('attendance_recorded_date','<=',end_date),('check_in','=',False),('day_status','in',['0','3'])])
        ...
        for d in self.get_week(last_week_date):
            ...
            last_week_present_attendance_ids = self.env['kw_daily_employee_attendance'].search([('employee_id','=',self.env.user.employee_ids.id),('attendance_recorded_date','=',d.date()),('check_in','!=',False),('day_status','in',['0','3'])])
            last_week_absent_attendance_ids = self.env['kw_daily_employee_attendance'].search([('employee_id','=',self.env.user.employee_ids.id),('attendance_recorded_date','=',d.date()),('check_in','=',False),('day_status','in',['0','3'])])
        ...
        for d in self.get_week(current_date):
            ...
            current_week_present_attendance_ids = self.env['kw_daily_employee_attendance'].search([('employee_id','=',self.env.user.employee_ids.id),('attendance_recorded_date','=',d.date()),('check_in','!=',False),('day_status','in',['0','3'])])
            current_week_absent_attendance_ids = self.env['kw_daily_employee_attendance'].search([('employee_id','=',self.env.user.employee_ids.id),('attendance_recorded_date','=',d.date()),('check_in','=',False),('day_status','in',['0','3'])])
    ...
    def get_sub_ordinates_timesheet(self):
        ...
        for sub in sub_ordinate_ids:
            ...
            for d in self.get_week(last_week_date):
                ...
                last_week_present_attendance_ids = self.env['kw_daily_employee_attendance'].search([('employee_id','=',sub.id),('attendance_recorded_date','=',d.date()),('check_in','!=',False),('day_status','in',['0','3'])])
                last_week_absent_attendance_ids = self.env['kw_daily_employee_attendance'].search([('employee_id','=',sub.id),('attendance_recorded_date','=',d.date()),('check_in','=',False),('day_status','in',['0','3'])])
            ...
            for d in self.get_week(current_date):
                ...
                current_week_present_attendance_ids = self.env['kw_daily_employee_attendance'].search([('employee_id','=',sub.id),('attendance_recorded_date','=',d.date()),('check_in','!=',False),('day_status','in',['0','3'])])
                current_week_absent_attendance_ids = self.env['kw_daily_employee_attendance'].search([('employee_id','=',sub.id),('attendance_recorded_date','=',d.date()),('check_in','=',False),('day_status','in',['0','3'])])
    ...
    def grievance_data(self):
        grievance_datas = {
            'grievance': self.get_grievance_data()
        }
        return grievance_datas
    ...
    def fetch_leave_and_absent_data(self):
        ...
        print(leave_and_absent_data, 'DEPAK')
