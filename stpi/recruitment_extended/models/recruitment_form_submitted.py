...
class HrApplicantSubmittedTemp(models.Model):
    _name = "hr_applicant_submitted_temp"
    _description = 'Hr Recruitment Form Submitted'
    _rec_name = 'advertisement_job_id'

    # Personal Details Start
    advertisement_job_id = fields.Many2one('hr.job', string="Published Advertisement Job")
    advertisement_branch_id = fields.Many2one('res.branch', string="Center")
    ...
    address_ids = fields.One2many('communication_address_submit_temp','hra_id',string='Communication Address')
    #Communication Address End

    #Educational Details Start
    education_ids = fields.One2many('educational_history_submit_temp','hra_id',string='Educational Qualification')
    #Educational Details End

    #Employment Details Start
    experience_ids = fields.One2many('experience_submit_temp','hra_id',string='Employment Details')
    ...
    def _compute_certificate_upload_attachment_id(self):
        for applicant in self:
            attachment_id = self.env['ir.attachment'].search([('res_id', '=', applicant.id), ('res_model', '=', 'hr_applicant_submitted_temp'), ('res_field', '=', 'certificate_upload')])
            applicant.certificate_upload_attachment_id = attachment_id

    def _compute_nationality_upload_attachment_id(self):
        for applicant in self:
            attachment_id = self.env['ir.attachment'].search([('res_id', '=', applicant.id), ('res_model', '=', 'hr_applicant_submitted_temp'), ('res_field', '=', 'nationality_upload')])
            applicant.nationality_upload_attachment_id = attachment_id

    def _compute_pan_upload_attachment_id(self):
        for applicant in self:
            attachment_id = self.env['ir.attachment'].search([('res_id', '=', applicant.id), ('res_model', '=', 'hr_applicant_submitted_temp'), ('res_field', '=', 'pan_upload')])
            applicant.pan_upload_attachment_id = attachment_id

    def _compute_aadhar_upload_attachment_id(self):
        for applicant in self:
            attachment_id = self.env['ir.attachment'].search([('res_id', '=', applicant.id), ('res_model', '=', 'hr_applicant_submitted_temp'), ('res_field', '=', 'aadhar_upload')])
            applicant.aadhar_upload_attachment_id = attachment_id

    def _compute_payment_document_attachment_id(self):
        for payment in self:
            attachment_id = self.env['ir.attachment'].search([('res_id', '=', payment.id), ('res_model', '=', 'hr_applicant_submitted_temp'), ('res_field', '=', 'payment_document')])
            payment.payment_document_attachment_id = attachment_id

    def _compute_signature_document_attachment_id(self):
        for signature in self:
            attachment_id = self.env['ir.attachment'].search([('res_id', '=', signature.id), ('res_model', '=', 'hr_applicant_submitted_temp'), ('res_field', '=', 'signature_document')])
            signature.signature_document_attachment_id = attachment_id

    def _compute_other_document_attachment_id(self):
        for applicant in self:
            attachment_id = self.env['ir.attachment'].search([('res_id', '=', applicant.id), ('res_model', '=', 'hr_applicant_submitted_temp'), ('res_field', '=', 'other_doc')])
            applicant.other_doc_attachment_id = attachment_id

    def _compute_dob_document_attachment_id(self):
        for applicant in self:
            attachment_id = self.env['ir.attachment'].search([('res_id', '=', applicant.id), ('res_model', '=', 'hr_applicant_submitted_temp'), ('res_field', '=', 'dob_doc')])
            applicant.dob_doc_attachment_id = attachment_id

class CommunicationAddressSubmitTemp(models.Model):
    _name = "communication_address_submit_temp"
    _description = "Communication Address"

    hra_id = fields.Many2one('hr_applicant_submitted_temp',string='Applicant Id',ondelete='cascade')
...
class EducationalHistorySubmitTemp(models.Model):
    _name = "educational_history_submit_temp"
    ...
    def _compute_attachment_id(self):
        for education in self:
            attachment_id = self.env['ir.attachment'].search([('res_id', '=', education.id), ('res_model', '=', 'educational_history_submit_temp'), ('res_field', '=', 'certificate')])
            education.attachment_id = attachment_id

class ExperienceSubmitTemp(models.Model):
    _name = "experience_submit_temp"
    ...
    def _compute_attachment_id(self):
        for experience in self:
            attachment_id = self.env['ir.attachment'].search([('res_id', '=', experience.id), ('res_model', '=', 'experience_submit_temp'), ('res_field', '=', 'document')])
