    ...
    var KsDashboardNinja = AbstractAction.extend(ControlPanelMixin, {
        ...
        events: {
            ...
            'keyup #emp_search': _.debounce(function(e) {
                ...
                if(self.empSearchInput.length >=3) {self.renderNewJoinee("search_employee");}
                if(self.empSearchInput.length ==0){ self.renderNewJoinee("refresh_employee"); }
            }, 0, true),
            'click #filter-employee-button': _.debounce(function(){
                var self = this;
                emp_offset = 0;
                self.branchselect = $('#branch-select').val();
                self.departmentselect = $('#department-select').val();
                setTimeout(function(){self.renderNewJoinee("filter");},200);
            },200,true),
            'click .refresh_new_joinee': _.debounce(function() {
                emp_offset = 0;
                var self = this;
                $('#emp_search').val('');
                self.refresh_employee_data = true;
                self.renderNewJoinee("refresh_employee");
            }, 200, true),
            'click .new-joinee': _.debounce(function() {
                emp_offset = 0;
                var self = this;
                self.renderNewJoinee("new_joinee");
            },0, true),
            'click .leave-apply': _.debounce(function() {
                window.location.href = 'web#&action=hr_holidays.hr_leave_action_my&model=hr.leave&view_type=form';
            },0, true),
            'click .loan-apply': _.debounce(function() {
                window.location.href = 'web#&action=ohrms_loan.hr_loan_menu_action&model=hr.loan&view_type=form';
            },0, true),
            'click .reimbursement-apply': _.debounce(function(e){ 
                var self = this;
                self.reimbursements_type = e.currentTarget.dataset.reimbursement;
                self.reimbursements_period = e.currentTarget.dataset.period;
                self.reimbursement_apply();
            },0, true),
            'click .refresh_managerial_inbox': _.debounce(function() {
                var self = this;
                self.renderManagerialInbox();
            },0, true),
            'click .refresh_my_leaves': _.debounce(function() {
                var self = this;
                self.renderMyLeaves();
            },0, true),
            'click .refresh_employees_on_leave': _.debounce(function() {
                var self = this;
                self.renderEmployeesOnLeave();
            },0, true),
            'click .refresh_holiday_calendar': _.debounce(function() {
                var self = this;
                self.renderHolidayCalendar();
            },0, true),
            'click .refresh_best_wishes': _.debounce(function() {
                var self = this;
                self.renderBestWishes();
            },0, true),
            'click .refresh_ltc': _.debounce(function() {
                var self = this;
                self.renderLtcDetails();
            },0, true),
            'click .refresh_loan': _.debounce(function() {
                var self = this;
                self.renderLoanDetails();
            },0, true),
            'click .refresh_my_reimbursement': _.debounce(function() {
                var self = this;
                self.renderMyReimbursement();
            },0, true),
            'click .refresh_employee_inbox': _.debounce(function() {
                var self = this;
                self.renderEmployeeInbox();
            },0, true),
            'click .refresh_employees_on_tour': _.debounce(function() {
                var self = this;
                self.renderEmployeesOnTourDetails();
            },0, true),
            'click .refresh_my_tour': _.debounce(function() {
                var self = this;
                self.renderMyTour();
            },0, true),
            'click .refresh_my_department': _.debounce(function() {
                var self = this;
                self.renderMyDepartment();
            },0, true),
            'change #filter-appraisal': _.debounce(function(){
                var self = this;
                self.appraisal_year_id = $('#filter-appraisal').val();
                console.log(self.appraisal_year_id);
                self.renderMyAppraisal("filter_appraisal");
            },0,true),
        ...
        ksRenderDashboardItems: function(items) {
            ...
            for (var i = 0; i < items.length; i++) {
                if (self.grid) {
                    ...
                    } else if (items[i].ks_item_code === 'managerial_inbox') {
                        self._renderManagerialInboxPortlet(items[i], self.grid)
                    } else if (items[i].ks_item_code === 'my_leaves') {
                        self._renderMyLeavesPortlet(items[i], self.grid)
                    } else if (items[i].ks_item_code === 'new_joinee') {
                        self._renderNewJoineePortlet(items[i], self.grid)
                    } else if (items[i].ks_item_code === 'employees_on_leave') {
                        self._renderEmployeesOnLeavePortlet(items[i], self.grid)
                    } else if (items[i].ks_item_code === 'employees_on_tour') {
                        self._renderEmployeesOnTourPortlet(items[i], self.grid)
                    } else if (items[i].ks_item_code === 'my_tour') {
                        self._renderMyTourPortlet(items[i], self.grid)
                    } else if (items[i].ks_item_code === 'my_department') {
                        self._renderMyDepartmentPortlet(items[i], self.grid)
                    } else if (items[i].ks_item_code === 'best_wishes') {
                        self._renderBestWishesPortlet(items[i], self.grid)
                    } else if (items[i].ks_item_code === 'loan_details') {
                        self._renderLoanPortlet(items[i], self.grid)
                    } else if (items[i].ks_item_code === 'ltc') {
                        self._renderLtcPortlet(items[i], self.grid)
                    } else if (items[i].ks_item_code === 'holiday_calendar') {
                        self._renderHolidayCalendarPortlet(items[i], self.grid)
                    } else if (items[i].ks_item_code === 'bi_dashboard') {
                        self._renderBIDashboardPortlet(items[i], self.grid)
                    } else if (items[i].ks_item_code === 'my_reimbursement') {
                        self._renderMyReimbursementPortlet(items[i], self.grid)
                    } else if (items[i].ks_item_code === 'employee_inbox') {
                        self._renderEmployeeInboxPortlet(items[i], self.grid)
                    } else if (items[i].ks_item_code === 'my_appraisal') {
                        self._renderMyAppraisalPortlet(items[i], self.grid)
                    } else if (items[i].ks_item_code === 'efile') {
                        self._rendereFilePortlet(items[i], self.grid)
                    } else {
        ...
        ksUpdateDashboardItem: function(ids) {
            ...
            for (var i = 0; i < ids.length; i++) {
                ...
                } else if (item_data['ks_item_code'] == 'managerial_inbox') {
                    var item_view = self._renderManagerialInboxPortlet(item_data);
                    self.$el.find(".grid-stack-item[data-gs-id=" + item_data.id + "]").find(".ks_dashboarditem_id").replaceWith($(item_view).find('.ks_dashboarditem_id'));
                } else if (item_data['ks_item_code'] == 'my_leaves') {
                    var item_view = self._renderMyLeavesPortlet(item_data);
                    self.$el.find(".grid-stack-item[data-gs-id=" + item_data.id + "]").find(".ks_dashboarditem_id").replaceWith($(item_view).find('.ks_dashboarditem_id'));
                } else if (item_data['ks_item_code'] == 'new_joinee') {
                    var item_view = self._renderNewJoineePortlet(item_data);
                    self.$el.find(".grid-stack-item[data-gs-id=" + item_data.id + "]").find(".ks_dashboarditem_id").replaceWith($(item_view).find('.ks_dashboarditem_id'));
                } else if (item_data['ks_item_code'] == 'employees_on_leave') {
                    var item_view = self._renderEmployeesOnLeavePortlet(item_data);
                    self.$el.find(".grid-stack-item[data-gs-id=" + item_data.id + "]").find(".ks_dashboarditem_id").replaceWith($(item_view).find('.ks_dashboarditem_id'));
                } else if (item_data['ks_item_code'] == 'employees_on_tour') {
                    var item_view = self._renderEmployeesOnTourPortlet(item_data);
                    self.$el.find(".grid-stack-item[data-gs-id=" + item_data.id + "]").find(".ks_dashboarditem_id").replaceWith($(item_view).find('.ks_dashboarditem_id'));
                } else if (item_data['ks_item_code'] == 'holiday_calendar') {
                    var item_view = self._renderBestWishesPortlet(item_data);
                    self.$el.find(".grid-stack-item[data-gs-id=" + item_data.id + "]").find(".ks_dashboarditem_id").replaceWith($(item_view).find('.ks_dashboarditem_id'));
                } else if (item_data['ks_item_code'] == 'loan_details') {
                    var item_view = self._renderLoanPortlet(item_data);
                    self.$el.find(".grid-stack-item[data-gs-id=" + item_data.id + "]").find(".ks_dashboarditem_id").replaceWith($(item_view).find('.ks_dashboarditem_id'));
                } else if (item_data['ks_item_code'] == 'ltc') {
                    var item_view = self._renderLtcPortlet(item_data);
                    self.$el.find(".grid-stack-item[data-gs-id=" + item_data.id + "]").find(".ks_dashboarditem_id").replaceWith($(item_view).find('.ks_dashboarditem_id'));
                } else if (item_data['ks_item_code'] == 'holiday_calendar') {
                    var item_view = self._renderHolidayCalendarPortlet(item_data);
                    self.$el.find(".grid-stack-item[data-gs-id=" + item_data.id + "]").find(".ks_dashboarditem_id").replaceWith($(item_view).find('.ks_dashboarditem_id'));
                } else if (item_data['ks_item_code'] == 'bi_dashboard') {
                    var item_view = self._renderBIDashboardPortlet(item_data);
                    self.$el.find(".grid-stack-item[data-gs-id=" + item_data.id + "]").find(".ks_dashboarditem_id").replaceWith($(item_view).find('.ks_dashboarditem_id'));
                } else if (item_data['ks_item_code'] == 'my_reimbursement') {
                    var item_view = self._renderMyReimbursementPortlet(item_data);
                    self.$el.find(".grid-stack-item[data-gs-id=" + item_data.id + "]").find(".ks_dashboarditem_id").replaceWith($(item_view).find('.ks_dashboarditem_id'));
                } else if (item_data['ks_item_code'] == 'employee_inbox') {
                    var item_view = self._renderEmployeeInboxPortlet(item_data);
                    self.$el.find(".grid-stack-item[data-gs-id=" + item_data.id + "]").find(".ks_dashboarditem_id").replaceWith($(item_view).find('.ks_dashboarditem_id'));
                } else if (item_data['ks_item_code'] == 'my_tour') {
                    var item_view = self._renderMyTourPortlet(item_data);
                    self.$el.find(".grid-stack-item[data-gs-id=" + item_data.id + "]").find(".ks_dashboarditem_id").replaceWith($(item_view).find('.ks_dashboarditem_id'));
                }  else if (item_data['ks_item_code'] == 'my_department') {
                    var item_view = self._renderMyDepartmentPortlet(item_data);
                    self.$el.find(".grid-stack-item[data-gs-id=" + item_data.id + "]").find(".ks_dashboarditem_id").replaceWith($(item_view).find('.ks_dashboarditem_id'));
                } else if (item_data['ks_item_code'] == 'my_appraisal') {
                    var item_view = self._renderMyAppraisalPortlet(item_data);
                    self.$el.find(".grid-stack-item[data-gs-id=" + item_data.id + "]").find(".ks_dashboarditem_id").replaceWith($(item_view).find('.ks_dashboarditem_id'));
                } else if (item_data['ks_item_code'] == 'efile') {
                    var item_view = self._rendereFilePortlet(item_data);
                    self.$el.find(".grid-stack-item[data-gs-id=" + item_data.id + "]").find(".ks_dashboarditem_id").replaceWith($(item_view).find('.ks_dashboarditem_id'));
                } else {
        ...
        _renderNewJoineePortlet: async function(item, grid){
            var self = this;
            var $ksItemContainer = self.renderNewJoinee();
            var $ks_gridstack_container = $(QWeb.render('GridstackNewJoineePortletContainer', {
                ksIsDashboardManager: self.ks_dashboard_data.ks_dashboard_manager,
                ks_dashboard_list: self.ks_dashboard_data.ks_dashboard_list,
                item_id: item.id,
                item_name: item.name,
                branch: self.ks_dashboard_data.branch_department[0],
                department: self.ks_dashboard_data.branch_department[1],
                stpi_new_joinee : self.ks_dashboard_data.stpi_new_joinee
            })).addClass('ks_dashboarditem_id');
            ...
            $ks_gridstack_container.find('#dashboard_new_joinee_block').html($ksItemContainer);
            item.$el = $ks_gridstack_container;
            if (item.id in self.gridstackConfig) {
                grid.addWidget($ks_gridstack_container, self.gridstackConfig[item.id].x, self.gridstackConfig[item.id].y, self.gridstackConfig[item.id].width, self.gridstackConfig[item.id].height,  true, 12, null, 6, 6, item.id);
            } else {
                grid.addWidget($ks_gridstack_container, 0, 0, 12, 6,  true, 12, null, 6, 6, item.id);
            }
        },
        
        renderNewJoinee : async function(status){
            var self = this;
            var params = {}
            if(status == 'search_employee') {operation = 'empSearch'; operation_value = self.empSearchInput; params = {'empSearch':self.empSearchInput}}
            ...
            else if(status == 'filter') { params = {'branch': $('#branch-select').val(), 'department': self.departmentselect}}
            return this._rpc({
                model:'ks_dashboard_ninja.board',
                method : 'getEmployeeDirectory',
                kwargs: params,
            }).done(function(result){
                self.new_joinee_data = result;
                $('#dashboard_new_joinee_block').empty();
                var $newJoinee = $(QWeb.render('NewJoinee', {
                    new_joinee_data:self.new_joinee_data.new_joinee_data,
                    new_joinee_data_length: self.new_joinee_data.new_joinee_data.length
                }));
                $('#dashboard_new_joinee_block').append($newJoinee);
            })
        },
        
        _renderManagerialInboxPortlet: async function(item, grid){
            var self = this;
            var $ksItemContainer = self.renderManagerialInbox();
            var $ks_gridstack_container = $(QWeb.render('GridstackManagerialInboxPortletContainer', {
                ksIsDashboardManager: self.ks_dashboard_data.ks_dashboard_manager,
                ks_dashboard_list: self.ks_dashboard_data.ks_dashboard_list,
                item_id: item.id,
                item_name: item.name,
            })).addClass('ks_dashboarditem_id');
            $ks_gridstack_container.find('#dashboard_managerial_inbox_block').html($ksItemContainer);
            item.$el = $ks_gridstack_container;
            if (item.id in self.gridstackConfig) {
                grid.addWidget($ks_gridstack_container, self.gridstackConfig[item.id].x, self.gridstackConfig[item.id].y, self.gridstackConfig[item.id].width, self.gridstackConfig[item.id].height,  true, 12, null, 6, 6, item.id);
            } else {
        ...
        renderManagerialInbox: async function(){
            var self = this;
            return this._rpc({
                model:'ks_dashboard_ninja.board',
                method : 'getManagerialInbox',
                context : self.getContext()
            }).done(function(result){
                console.log(result)
                self.managerial_counts = result;
                $('#dashboard_managerial_inbox_block').empty();
                var $managerialInbox = $(QWeb.render('MangerialInbox', {
                    leaves : self.managerial_counts.managerial_inbox[0],
                    tours : self.managerial_counts.managerial_inbox[1],
                    loans : self.managerial_counts.managerial_inbox[2],
                    reimbursements: self.managerial_counts.managerial_inbox[3],
                    ltc: self.managerial_counts.managerial_inbox[4]
                }));
                $('#dashboard_managerial_inbox_block').append($managerialInbox);
            })
        },

        _renderMyLeavesPortlet: async function(item, grid){
            var self = this;
            var $ksItemContainer = self.renderMyLeaves();
            var $ks_gridstack_container = $(QWeb.render('GridstackMyLeavesPortletContainer', {
                ksIsDashboardManager: self.ks_dashboard_data.ks_dashboard_manager,
                ks_dashboard_list: self.ks_dashboard_data.ks_dashboard_list,
                item_id: item.id,
                item_name: item.name,
            })).addClass('ks_dashboarditem_id');
            $ks_gridstack_container.find('#dashboard_my_leaves_block').html($ksItemContainer);
            item.$el = $ks_gridstack_container;
            if (item.id in self.gridstackConfig) {
                grid.addWidget($ks_gridstack_container, self.gridstackConfig[item.id].x, self.gridstackConfig[item.id].y, self.gridstackConfig[item.id].width, self.gridstackConfig[item.id].height,  true, 12, null, 6, 6, item.id);
            } else {
        ...
        renderMyLeaves: async function(){
            ...
            return this._rpc({
                ...
                console.log(result);
                self.my_leaves_data = result;
                $('#dashboard_my_leaves_block').empty();
                var $myLeavesData = $(QWeb.render('MyLeaves', {
                    leave_data: self.my_leaves_data.myLeaveData[0],
                    leave_data_length : self.my_leaves_data.myLeaveData[0].length,
                }));
                $('#dashboard_my_leaves_block').append($myLeavesData);
                $('.leave-progress-bar').tooltip({'delay': { show: 100, hide: 100 }, 'placement': "top", 'customClass': 'dashboard-tooltip'});
            });
        },

        _renderEmployeesOnLeavePortlet: async function(item, grid){
            var self = this;
            var $ksItemContainer = self.renderEmployeesOnLeave();
            var $ks_gridstack_container = $(QWeb.render('GridstackEmployeesOnLeavePortletContainer', {
                ksIsDashboardManager: self.ks_dashboard_data.ks_dashboard_manager,
                ks_dashboard_list: self.ks_dashboard_data.ks_dashboard_list,
                item_id: item.id,
                item_name: item.name,
            })).addClass('ks_dashboarditem_id');
            $ks_gridstack_container.find('#dashboard_employees_on_leave_block').html($ksItemContainer);
            item.$el = $ks_gridstack_container;
            if (item.id in self.gridstackConfig) {
                grid.addWidget($ks_gridstack_container, self.gridstackConfig[item.id].x, self.gridstackConfig[item.id].y, self.gridstackConfig[item.id].width, self.gridstackConfig[item.id].height,  true, 12, null, 6, 6, item.id);
            } else {
        ...
        renderEmployeesOnLeave: async function(){
            var self = this;
            return this._rpc({
                model: 'ks_dashboard_ninja.board',
                method: 'getEmployeesOnLeave',
                context: self.getContext(),
            }).done(function(result) {
                console.log(result)
                self.employee_on_leave_data = result;
                $('#dashboard_employees_on_leave_block').empty();
                var $employeesOnLeaveData = $(QWeb.render('EmployeesOnLeave', {
                    todayLeave: self.employee_on_leave_data.employeesOnLeaveData[0],
                    todayLeaveLength: self.employee_on_leave_data.employeesOnLeaveData[0].length,
                    thisweekLeave: self.employee_on_leave_data.employeesOnLeaveData[1],
                    thisweekLeaveLength: self.employee_on_leave_data.employeesOnLeaveData[1].length
                }));
                $('#dashboard_employees_on_leave_block').append($employeesOnLeaveData);
                $('.leave-count').tooltip({'delay': { show: 100, hide: 100 }, 'placement': "top", 'customClass': 'dashboard-tooltip'});
            });
        },
        ...
        _renderHolidayCalendarPortlet: async function(item, grid){
            ...
            $ks_gridstack_container.find('#dashboard_holiday_calendar_block').html($ksItemContainer);
            item.$el = $ks_gridstack_container;
            if (item.id in self.gridstackConfig) {
                grid.addWidget($ks_gridstack_container, self.gridstackConfig[item.id].x, self.gridstackConfig[item.id].y, self.gridstackConfig[item.id].width, self.gridstackConfig[item.id].height,  true, 12, null, 6, 6, item.id);
            } else {
        ...
        renderHolidayCalendar: async function(status){
            ...
            return this._rpc({
                ...
                console.log(result);
                self.holiday_calendar_data = result;
                $('#dashboard_holiday_calendar_block').empty();
                var $holidayCalendarData = $(QWeb.render('HolidayCalendar', {}));
                var filterDate = null;
                setTimeout(function() {
                    var dates = self.holiday_calendar_data.holidayCalendarData;
                        ...
                        beforeShowDay: function(date) {
                            ...
                            for (var i = 0; i < dates.length; ++i)
                            {
                                if (year == dates[i][0] && month == dates[i][1] - 1 && day == dates[i][2]) {
                                    if (dates[i][4] === "#d83d2b")
                                    {console.log("called");
                                    return [false, 'stpi_gh_holiday', dates[i][3]];}
                                    else if (dates[i][4] === "#00A09D")
                                    return [false, 'stpi_rh_holiday', dates[i][3]];
                                    else if (dates[i][4] === "#F5BB00")
                                    return [false, 'stpi_weekoff_holiday', dates[i][3]];
                                    else
                                    return [false, 'stpi_working_day', dates[i][3]]
                                }
                            }
                            return [false];
                        }
                    });
                    $('.stpi_gh_holiday, .stpi_rh_holiday, .stpi_working_day','stpi_weekoff_holiday').tooltip({'delay': { show: 100, hide: 100 }, 'placement': "top", 'customClass': 'dashboard-tooltip'});
                },500); 
                $('#dashboard_holiday_calendar_block').append($holidayCalendarData);
            });
        },
        
        _renderBIDashboardPortlet: async function(item, grid){
            var self = this;
            var $ks_gridstack_container = $(QWeb.render('GridstackBIDashboardPortletContainer', {
                ksIsDashboardManager: self.ks_dashboard_data.ks_dashboard_manager,
                ks_dashboard_list: self.ks_dashboard_data.ks_dashboard_list,
                item_id: item.id,
                item_name: item.name,
            ...
            item.$el = $ks_gridstack_container;
            if (item.id in self.gridstackConfig) {
                grid.addWidget($ks_gridstack_container, self.gridstackConfig[item.id].x, self.gridstackConfig[item.id].y, self.gridstackConfig[item.id].width, self.gridstackConfig[item.id].height,  true, 36, null, 6, 6, item.id);
            } else {
                grid.addWidget($ks_gridstack_container, 0, 0, 36, 6,  true, 12, null, 6, 6, item.id);
            }
        } ,
        ...
        _renderBestWishesPortlet: async function(item, grid){
            ...
            $ks_gridstack_container.find('#dashboard_best_wishes_block').html($ksItemContainer);
            item.$el = $ks_gridstack_container;
            if (item.id in self.gridstackConfig) {
                grid.addWidget($ks_gridstack_container, self.gridstackConfig[item.id].x, self.gridstackConfig[item.id].y, self.gridstackConfig[item.id].width, self.gridstackConfig[item.id].height,  true, 12, null, 6, 6, item.id);
            } else {
        ...
        renderBestWishes: async function(){
            ...
            return this._rpc({
                model: 'ks_dashboard_ninja.board',
                method: 'getBestWishes',
                context: self.getContext(),
            }).done(function(result) {
                console.log(result)
                self.bestWishesData = result;
                $('#dashboard_best_wishes_block').empty();
                var $bestWishesData = $(QWeb.render('BestWishes', {
                    birthday : self.bestWishesData.bestWishesData[0],
                    birthday_length: self.bestWishesData.bestWishesData[0].length,
                    year_of_service: self.bestWishesData.bestWishesData[1],
                    year_of_service_length: self.bestWishesData.bestWishesData[1].length
                }));
                $('#dashboard_best_wishes_block').append($bestWishesData);
                $('.birthday-img-employee').tooltip({'delay': { show: 100, hide: 100 }, 'placement': "top", 'customClass': 'dashboard-tooltip'});
            });
        },

        _renderLoanPortlet: async function(item, grid){
            var self = this;
            var $ksItemContainer = self.renderLoanDetails();
            var $ks_gridstack_container = $(QWeb.render('GridstackLoanPortletContainer', {
                ksIsDashboardManager: self.ks_dashboard_data.ks_dashboard_manager,
                ks_dashboard_list: self.ks_dashboard_data.ks_dashboard_list,
                item_id: item.id,
                item_name: item.name,
            })).addClass('ks_dashboarditem_id');
            $ks_gridstack_container.find('#dashboard_loan_block').html($ksItemContainer);
            item.$el = $ks_gridstack_container;
            if (item.id in self.gridstackConfig) {
                grid.addWidget($ks_gridstack_container, self.gridstackConfig[item.id].x, self.gridstackConfig[item.id].y, self.gridstackConfig[item.id].width, self.gridstackConfig[item.id].height,  true, 12, null, 6, 6, item.id);
            } else {
        ...
        renderLoanDetails: async function(){
            var self = this;
            return this._rpc({
                model: 'ks_dashboard_ninja.board',
                ...
                self.loan_details = result;
                $('#dashboard_loan_block').empty();
                var $loanDetails = $(QWeb.render('LoanDetails', {
                    loanDetails : self.loan_details.loanDetails[0],
                    loanDetails_length: self.loan_details.loanDetails[0].length,
                    paidloanDetails : self.loan_details.loanDetails[1],
                    paidloanDetails_length : self.loan_details.loanDetails[1].length,
                }));
                $('#dashboard_loan_block').append($loanDetails);
                $('.emi-block').tooltip({'delay': { show: 100, hide: 100 }, 'placement': "top", 'customClass': 'dashboard-tooltip'});
            });
        },
        
        _renderLtcPortlet: async function(item, grid){
            var self = this;
            var $ksItemContainer = self.renderLtcDetails();
            var $ks_gridstack_container = $(QWeb.render('GridstackLtcPortletContainer', {
                ksIsDashboardManager: self.ks_dashboard_data.ks_dashboard_manager,
                ks_dashboard_list: self.ks_dashboard_data.ks_dashboard_list,
                item_id: item.id,
                item_name: item.name,
            })).addClass('ks_dashboarditem_id');
            $ks_gridstack_container.find('#dashboard_ltc_block').html($ksItemContainer);
            item.$el = $ks_gridstack_container;
            if (item.id in self.gridstackConfig) {
                grid.addWidget($ks_gridstack_container, self.gridstackConfig[item.id].x, self.gridstackConfig[item.id].y, self.gridstackConfig[item.id].width, self.gridstackConfig[item.id].height,  true, 12, null, 6, 6, item.id);
            } else {
        ...
        renderLtcDetails: async function(){
            var self = this;
            return this._rpc({
                model: 'ks_dashboard_ninja.board',
                method: 'getLtcDetails',
                context: self.getContext(),
            }).done(function(result) {
                console.log(result)
                self.ltc_details = result;
                $('#dashboard_ltc_block').empty();
                var $ltcDetails = $(QWeb.render('LTC', {
                    block_year : self.ltc_details.ltcDetails[0],
                    ltcDetails : self.ltc_details.ltcDetails[1],
                    ltcDetails_length : self.ltc_details.ltcDetails[1].length,
                }));
                $('#dashboard_ltc_block').append($ltcDetails);
            });
        },

        _renderEmployeesOnTourPortlet: async function(item, grid){
            var self = this;
            var $ksItemContainer = self.renderEmployeesOnTourDetails();
            var $ks_gridstack_container = $(QWeb.render('GridstackEmployeesOnTourPortletContainer', {
                ksIsDashboardManager: self.ks_dashboard_data.ks_dashboard_manager,
                ks_dashboard_list: self.ks_dashboard_data.ks_dashboard_list,
                item_id: item.id,
                item_name: item.name,
            })).addClass('ks_dashboarditem_id');
            $ks_gridstack_container.find('#dashboard_employees_on_tour_block').html($ksItemContainer);
            item.$el = $ks_gridstack_container;
            if (item.id in self.gridstackConfig) {
                grid.addWidget($ks_gridstack_container, self.gridstackConfig[item.id].x, self.gridstackConfig[item.id].y, self.gridstackConfig[item.id].width, self.gridstackConfig[item.id].height,  true, 12, null, 6, 6, item.id);
            } else {
        ...
        renderEmployeesOnTourDetails: async function(){
            var self = this;
            return this._rpc({
                model: 'ks_dashboard_ninja.board',
                method: 'getEmployeeOnTourData',
                context: self.getContext(),
            }).done(function(result) {
                console.log(result)
                self.tour_data = result;
                $('#dashboard_employees_on_tour_block').empty();
                var $employeesOnTourDetails = $(QWeb.render('EmployeesOnTourDetails', {
                    tours : self.tour_data.employeesOnTourData,
                    tour_length : self.tour_data.employeesOnTourData.length
                }));
                var render = '';
                var cities = '';
                var renderTourMap = function(){
                    for(var i = 0; i < self.tour_data.employeesOnTourData.length; i++) {
                    render = self.tour_data.employeesOnTourData[i].id;
                    cities = self.tour_data.employeesOnTourData[i].cities;
                        Highcharts.chart({
                        chart: {
                            renderTo: render,
                            type: 'line',
                ...
                $('#dashboard_employees_on_tour_block').append($employeesOnTourDetails);
                renderTourMap();
            });
        },

        _renderMyTourPortlet: async function(item, grid){
            var self = this;
            var $ksItemContainer = self.renderMyTour();
            var $ks_gridstack_container = $(QWeb.render('GridstackMyTourPortletContainer', {
                ksIsDashboardManager: self.ks_dashboard_data.ks_dashboard_manager,
                ks_dashboard_list: self.ks_dashboard_data.ks_dashboard_list,
                item_id: item.id,
                item_name: item.name,
            })).addClass('ks_dashboarditem_id');
            $ks_gridstack_container.find('#dashboard_my_tour_block').html($ksItemContainer);
            item.$el = $ks_gridstack_container;
            if (item.id in self.gridstackConfig) {
                grid.addWidget($ks_gridstack_container, self.gridstackConfig[item.id].x, self.gridstackConfig[item.id].y, self.gridstackConfig[item.id].width, self.gridstackConfig[item.id].height,  true, 12, null, 6, 6, item.id);
            } else {
        ...
        renderMyTour: async function(){
            ...
            return this._rpc({
                ...
                console.log(result)
                self.my_tour_data = result;
                $('#dashboard_my_tour_block').empty();
                var $myTour = $(QWeb.render('MyTour', {
                    tour_data : self.my_tour_data.myTourData,
                    tour_data_length : self.my_tour_data.myTourData.length
                }));
                $('#dashboard_my_tour_block').append($myTour);
            });
        },

         _renderMyDepartmentPortlet: async function(item, grid){
            var self = this;
            var $ksItemContainer = self.renderMyDepartment();
            var $ks_gridstack_container = $(QWeb.render('GridstackMyDepartmentPortletContainer', {
                ksIsDashboardManager: self.ks_dashboard_data.ks_dashboard_manager,
                ks_dashboard_list: self.ks_dashboard_data.ks_dashboard_list,
                item_id: item.id,
                item_name: item.name,
            })).addClass('ks_dashboarditem_id');
            $ks_gridstack_container.find('#dashboard_my_department_block').html($ksItemContainer);
            item.$el = $ks_gridstack_container;
            if (item.id in self.gridstackConfig) {
                grid.addWidget($ks_gridstack_container, self.gridstackConfig[item.id].x, self.gridstackConfig[item.id].y, self.gridstackConfig[item.id].width, self.gridstackConfig[item.id].height,  true, 12, null, 6, 6, item.id);
            } else {
        ...
        renderMyDepartment: async function(){
            var self = this;
            return this._rpc({
                model: 'ks_dashboard_ninja.board',
                method: 'getMyDepartment',
                context: self.getContext(),
            }).done(function(result) {
                console.log(result)
                self.my_department_data = result;
                $('#dashboard_my_department_block').empty();
                var $myDepartment = $(QWeb.render('MyDepartment', {
                    department_data : self.my_department_data.myDepartmentData[0]
                }));
                
                $('#dashboard_my_department_block').append($myDepartment);
                console.log(self.my_department_data.myDepartmentData[1])
                var renderHighchart_graph = function() {
                    setTimeout(function() {
                        Highcharts.chart({
                            chart: {
                                renderTo: 'department_chart',
                            ...
                            series: [{
                                ...
                                innerSize: '50%',
                                showInLegend: false,
                                // data: self.my_department_data.myDepartmentData[1],
                                data: [
                                    ['Regular',self.my_department_data.myDepartmentData[4]],
                                    ['Probation',self.my_department_data.myDepartmentData[1]],
                                    ['Contract',self.my_department_data.myDepartmentData[2]],
                                    ['Deputation',self.my_department_data.myDepartmentData[3]]
                                ]
                            }],
                            credits: {
                                enabled: false
        ...
        _renderMyReimbursementPortlet: async function(item, grid){
            var self = this;
            var $ksItemContainer = self.renderMyReimbursement();
            var $ks_gridstack_container = $(QWeb.render('GridstackMyReimbursementPortletContainer', {
                ksIsDashboardManager: self.ks_dashboard_data.ks_dashboard_manager,
                ks_dashboard_list: self.ks_dashboard_data.ks_dashboard_list,
                item_id: item.id,
                item_name: item.name,
            })).addClass('ks_dashboarditem_id');
            $ks_gridstack_container.find('#dashboard_my_reimbursement_block').html($ksItemContainer);
            item.$el = $ks_gridstack_container;
            if (item.id in self.gridstackConfig) {
                grid.addWidget($ks_gridstack_container, self.gridstackConfig[item.id].x, self.gridstackConfig[item.id].y, self.gridstackConfig[item.id].width, self.gridstackConfig[item.id].height,  true, 12, null, 6, 6, item.id);
            } else {
        ...
        renderMyReimbursement: async function(){
            var self = this;
            return this._rpc({
                model: 'ks_dashboard_ninja.board',
                method: 'getMyReimbursements',
                context: self.getContext(),
            }).done(function(result) {
                console.log(result)
                self.my_reimbursement_data = result;
                $('#dashboard_my_reimbursement_block').empty();
                var $myReimbursement = $(QWeb.render('MyReimbursement', {
                    check_status : self.my_reimbursement_data.myReimbursementData[0],
                    check_status_length : self.my_reimbursement_data.myReimbursementData[0].length,
                    yet_to_apply : self.my_reimbursement_data.myReimbursementData[1],
                    yet_to_apply_length : self.my_reimbursement_data.myReimbursementData[1].length
                }));
                $('#dashboard_my_reimbursement_block').append($myReimbursement);
            });
        },

        reimbursement_apply: async function(){
            var self = this;
            console.log(self.reimbursements_type,self.reimbursements_period )
            self.do_action({
                type: 'ir.actions.act_window',
                res_model: 'reimbursement',
                view_id: 'reimbursement_stpi.reimbursement_menu_action',
                views: [
                    [false, 'form']
                ],
                target: 'current',
                context: {
                    'name': self.reimbursements_type,
                    'date_range': parseInt(self.reimbursements_period),
                    'form_view_ref': 'reimbursement_stpi.reimbursement_form_view',
                    'form_view_initial_mode': 'edit',
                },
            }, {
                on_reverse_breadcrumb: this.on_reverse_breadcrumb,
        ...
        _renderEmployeeInboxPortlet: async function(item, grid){
            var self = this;
            var $ksItemContainer = self.renderEmployeeInbox();
            var $ks_gridstack_container = $(QWeb.render('GridstackEmployeeInboxPortletContainer', {
                ksIsDashboardManager: self.ks_dashboard_data.ks_dashboard_manager,
                ks_dashboard_list: self.ks_dashboard_data.ks_dashboard_list,
                item_id: item.id,
                item_name: item.name,
            })).addClass('ks_dashboarditem_id');
            $ks_gridstack_container.find('#dashboard_employee_inbox_block').html($ksItemContainer);
            item.$el = $ks_gridstack_container;
            if (item.id in self.gridstackConfig) {
                grid.addWidget($ks_gridstack_container, self.gridstackConfig[item.id].x, self.gridstackConfig[item.id].y, self.gridstackConfig[item.id].width, self.gridstackConfig[item.id].height,  true, 12, null, 6, 6, item.id);
            } else {
        ...
        renderEmployeeInbox: async function(){
            var self = this;
            return this._rpc({
                model: 'ks_dashboard_ninja.board',
                method: 'getEmployeeInbox',
                context: self.getContext(),
            }).done(function(result) {
                console.log(result)
                self.employee_inbox = result;
                $('#dashboard_employee_inbox_block').empty();
                var $employeeInbox = $(QWeb.render('EmployeeInbox', {
                    approvals : self.employee_inbox.employeeInbox,
                    approvals_length: self.employee_inbox.employeeInbox.length
                }));
                $('#dashboard_employee_inbox_block').append($employeeInbox);
            });
        },

        _renderMyAppraisalPortlet: async function(item, grid){
            var self = this;
            var $ksItemContainer = self.renderMyAppraisal();
            var $ks_gridstack_container = $(QWeb.render('GridstackMyAppraisalPortletContainer', {
                ksIsDashboardManager: self.ks_dashboard_data.ks_dashboard_manager,
                ks_dashboard_list: self.ks_dashboard_data.ks_dashboard_list,
                item_id: item.id,
                item_name: item.name,
            })).addClass('ks_dashboarditem_id');
            $ks_gridstack_container.find('#dashboard_my_appraisal_block').html($ksItemContainer);
            item.$el = $ks_gridstack_container;
            if (item.id in self.gridstackConfig) {
                grid.addWidget($ks_gridstack_container, self.gridstackConfig[item.id].x, self.gridstackConfig[item.id].y, self.gridstackConfig[item.id].width, self.gridstackConfig[item.id].height,  true, 12, null, 6, 6, item.id);
            } else {
        ...
        renderMyAppraisal: async function(status){
            var self = this;
            console.log(status)
            var parameters = {}
            ...
            return this._rpc({
                model: 'ks_dashboard_ninja.board',
                method: 'getMyAppraisal',
                kwargs: parameters,
                context: self.getContext(),
            }).done(function(result) {
                console.log(result)
                self.my_appraisal = result;
                $('#dashboard_my_appraisal_block').empty();
                var $myAppraisal = $(QWeb.render('MyAppraisal', {
                    appraisal_year_ids : self.my_appraisal.my_appraisal[0],
                    appraisal_record : self.my_appraisal.my_appraisal[1],
                    appraisal_period_id : self.my_appraisal.my_appraisal[2],
                    average1_score : self.my_appraisal.my_appraisal[3],
                    average2_score : self.my_appraisal.my_appraisal[4],
                    average3_score : self.my_appraisal.my_appraisal[5],
                    overall_rate : self.my_appraisal.my_appraisal[6],
                }));
                $('#dashboard_my_appraisal_block').append($myAppraisal);
                
                setTimeout(function(){$('#filter-appraisal').val($('#filter-appraisal').attr('data-selected'));},200);
            });
        },

        _rendereFilePortlet: async function(item, grid){
            var self = this;
            var $ksItemContainer = self.rendereFile();
            var $ks_gridstack_container = $(QWeb.render('GridstackeFilePortletContainer', {
                ksIsDashboardManager: self.ks_dashboard_data.ks_dashboard_manager,
                ks_dashboard_list: self.ks_dashboard_data.ks_dashboard_list,
                item_id: item.id,
                item_name: item.name,
            })).addClass('ks_dashboarditem_id');
            $ks_gridstack_container.find('#dashboard_efile_block').html($ksItemContainer);
            item.$el = $ks_gridstack_container;
            if (item.id in self.gridstackConfig) {
                grid.addWidget($ks_gridstack_container, self.gridstackConfig[item.id].x, self.gridstackConfig[item.id].y, self.gridstackConfig[item.id].width, self.gridstackConfig[item.id].height,  true, 12, null, 6, 6, item.id);
            } else {
        ...
        rendereFile: async function(item, grid){
            var self = this;
            return this._rpc({
                model: 'ks_dashboard_ninja.board',
                method: 'geteFile',
                ...
                console.log(result)
                self.efile = result;
                $('#dashboard_efile_block').empty();
                var $eFile = $(QWeb.render('eFile', {
                    correspondences_ids : self.efile.efile[0],
                    correspondences_ids_length: self.efile.efile[0].length,
                    file_ids : self.efile.efile[1],
                    file_ids_length: self.efile.efile[1].length,
                }));
                $('#dashboard_efile_block').append($eFile);
            });
        },
