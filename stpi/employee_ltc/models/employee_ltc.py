...
class EmployeeLtcAdvance(models.Model):
    ...
    def validate_ltc_limitations(self):
        ...
        for ltc in self:
            for relative in ltc.relative_ids:
                if ltc.employee_id.date_of_join + relativedelta(years=8) >= datetime.now().date():
    ...
    def validate_place_of_travel(self):
        for record in self:
            ...
            if record.employee_id.date_of_join + relativedelta(years=8) <= datetime.now().date():
    ...
    def validate_date_travel(self):
        for line in self:
            if line.employee_id.date_of_join + relativedelta(years=8) <= datetime.now().date():
    ...
    def validation_are_you_not_availing(self):
        for record in self:
            ...
            if record.employee_id.date_of_join and (record.employee_id.date_of_join + relativedelta(years=1)) >= datetime.now().date():
    ...
    def create(self, vals):
        ...
        if res.are_you_coming == True:
            ...
            print(existing_first_sub_year)
            if res.employee_id.date_of_join.year > 2008:
