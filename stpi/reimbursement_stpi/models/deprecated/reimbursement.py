...
class Reimbursement(models.Model):
    ...
    _rec_name = "reimbursement_type_id"
    ...
    reimbursement_type_id = fields.Many2one('reimbursement.type','Reimbursement Type')
    name = fields.Char("Reimbursement Name",related="reimbursement_type_id.code")
    ...
    phone = fields.Binary(track_visibility='always')
        ...
        for rec in self:
            if rec.employee_id and rec.name == 'lunch':
                ...
                if rec.name == 'telephone' or rec.name == 'mobile':
                    gr_id = self.env['reimbursement.configuration'].search([('reimbursement_type_id', '=', rec.reimbursement_type_id.id),('group_ids.users','=',self.env.user.id)],order='reimbursement_type_id desc', limit=1)
                    if gr_id:
                    ...
                    gr_id = self.env['reimbursement.configuration'].search([('reimbursement_type_id', '=', rec.reimbursement_type_id.id),('group_ids.users','=',self.env.user.id)],order='name desc', limit=1)
                    if gr_id:
    ...
    def button_submit(self):
        for rec in self:
            search_id = self.env['reimbursement'].search([('employee_id', '=', rec.employee_id.id), ('reimbursement_type_id', '=', rec.reimbursement_type_id.id), ('state', 'not in', ['draft','rejected'])])
            index = False
                        ...
                        if rec.name == 'lunch' and int(rec.net_amount) <= 0:
                            ...
                            if rec.claim_date_from > rec.claim_date or rec.claim_date > rec.claim_date_to:
                                gr_id = self.env['reimbursement.configuration'].search(
                                    [('reimbursement_type_id', '=', rec.reimbursement_type_id.id), ('group_ids.users', '=', self.env.user.id), ('open', '=', True)], order='reimbursement_type_id desc',
                                    limit=1)
                                if gr_id:
                                    rec.write({'state': 'waiting_for_approval'})
                                else:
                                    raise ValidationError("You can claim for %s" % rec.reimbursement_type_id.name + " between  %s" % rec.claim_date_from + " and %s" % rec.claim_date_to)
                            else:
                                rec.write({'state': 'waiting_for_approval'})
                    else:
