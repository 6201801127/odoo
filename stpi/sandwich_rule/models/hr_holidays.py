    ...
    def compute_number_of_leave(self):
        ...
        if self.holiday_status_id and self.holiday_status_id.sandwich_rule:
            if self.holiday_status_id:
                if self.holiday_status_id.sandwich_rule:
                    print("????????????????????????????????////////////////")
        ...
        if self.holiday_status_id.leave_type.code == 'Child Care Leave':
            today = date.today()
            relatives_ids = self.env['employee.relative'].search([('employee_id', '=', self.employee_id.id), ('relate_type', 'in', (3,4)),('status2', '!=', 'non_surviving')])
            print('relatives_ids-2154545454--------------', relatives_ids)
            for rec_child in relatives_ids:
                delta = today.year - rec_child.birthday.year - ((today.month, today.day) < (rec_child.birthday.month, rec_child.birthday.day))
                print("???????????????????????????", delta)
            ...
            if self.request_date_from < today or self.request_date_to < today:
                raise ValidationError(_('you are not allowed to take this leave as the date selected are past date which are not allowed for CCL.'))
        ...
        if self.holiday_status_id.leave_type.code in ['Maternity Leave', 'Paternity Leave']:
            ...
            if len(relatives_ids) > 2:
                raise ValidationError(_('You are not allowed to take this leave as you have already taken leaves for 2 childrens and for the 3rd child its not allowed according to rule.'))
        if self.holiday_status_id.leave_type.code == 'Paternity Leave':
            leaves_ids = self.env['hr.leave'].search([('employee_id', '=', self.employee_id.id), ('holiday_status_id', '=', self.holiday_status_id.id)])
