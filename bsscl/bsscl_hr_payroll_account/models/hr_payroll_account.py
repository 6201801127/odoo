...
class HrPayslip(models.Model):
    ...
    date = fields.Date('Date Account / दिनांक खाता', states={'draft': [('readonly', False)]}, readonly=True,
        help="Keep empty to use the period of the validation(Payslip) date.")
    journal_id = fields.Many2one('account.journal', 'Salary Journal / वेतन पत्रिका', readonly=True, required=True,
        states={'draft': [('readonly', False)]}, default=lambda self: self.env['account.journal'].search([('type', '=', 'general')], limit=1))
    move_id = fields.Many2one('account.move', 'Accounting Entry / लेखा पृविष्टि', readonly=True, copy=False)
...
class HrSalaryRule(models.Model):
    ...
    analytic_account_id = fields.Many2one('account.analytic.account', 'Analytic Account / विश्लेषणात्मक खाता')
    account_tax_id = fields.Many2one('account.tax', 'Tax / टैक्स')
    account_debit = fields.Many2one('account.account', 'Debit Account / डेबिट खाता', domain=[('deprecated', '=', False)])
    account_credit = fields.Many2one('account.account', 'Credit Account / क्रेडिट खाता', domain=[('deprecated', '=', False)])
...
class HrContract(models.Model):
    ...
    analytic_account_id = fields.Many2one('account.analytic.account', 'Analytic Account / विश्लेषणात्मक खाता')
    journal_id = fields.Many2one('account.journal', 'Salary Journal / वेतन पत्रिका')


class HrPayslipRun(models.Model):
    _inherit = 'hr.payslip.run'

    journal_id = fields.Many2one('account.journal', 'Salary Journal / वेतन पत्रिका', states={'draft': [('readonly', False)]}, readonly=True,
