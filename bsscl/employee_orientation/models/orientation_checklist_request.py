...
class OrientationChecklistRequest(models.Model):
    ...
    request_name = fields.Char(string='Name /नाम')
    request_orientation = fields.Many2one('employee.orientation', string='Employee Orientation /कर्मचारी उन्मुखीकरण')
    employee_company = fields.Many2one('res.company', string='Company /कंपनी', required=True,
                                       default=lambda self: self.env.user.company_id)
    partner_id = fields.Many2one('res.users', string='Responsible User /जिम्मेदार उपयोगकर्ता')
    request_date = fields.Date(string="Date /तारीख")
    employee_id = fields.Many2one('hr.employee', string='Employee /कर्मचारी')
    request_expected_date = fields.Date(string="Expected Date /अपेक्षित तारीक")
    attachment_id_1 = fields.Many2many('ir.attachment', 'orientation_rel_1', string="Attachment /अटैचमेंट")
