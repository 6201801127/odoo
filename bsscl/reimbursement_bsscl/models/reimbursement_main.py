...
class Reimbursement(models.Model):
    ...
    def _onchange_name(self):
        for rec in self:
            ...
            rec.dependent_ids = False
            if rec.reimbursement_type_id:
                depend_ids = self.env['employee.relative'].search([('employee_id', '=', rec.employee_id.id),
                                                                    ('relate_type.name', 'in', ('Son', 'Daughter'))])
                if rec.reimbursement_type_id.code == 'tuition_fee':
                    dependent_ids = [[0, 0, {
                                            'name': r.name,
                                            'dob': r.birthday}] for r in depend_ids.filtered(lambda x: x.tuition)]
                    rec.dependent_ids = dependent_ids

                if rec.reimbursement_type_id.code == 'hostel':
                    rec.dependent_ids = [[0, 0, {
                                            'name': r.name,
                                            'dob': r.birthday}] for r in depend_ids.filtered(lambda x: x.hostel)]
    ...
    check_manager = fields.Boolean('Manager')
    check_admin = fields.Boolean('Admin')
    ...
    taxable_per_day = fields.Float(string='Taxable Per Day')

    @api.constrains('brief_date')
    @api.onchange('brief_date')
    def onchange_brief_date(self):
        print("mmmmmmmmmmm")
        current_date = str(datetime.now().date())
        for record in self:
            if record.brief_date:
                if str(record.brief_date) > current_date:
                    raise ValidationError("Brief date should  not be greater than current date")

    @api.constrains('no_of_months')
    @api.onchange('no_of_months')
    def onchange_no_of_months(self):
        for record in self:
            if record.no_of_months:
                if record.no_of_months > 12:
                    raise ValidationError("Number of months should  not be greater than 12")

    @api.constrains('approve_amount_approver')
    @api.onchange('approve_amount_approver')
    def onchange_approve_amount_approver(self):
        for record in self:
            if record.approve_amount_approver and record.claimed_amount:
                if record.approve_amount_approver > record.claimed_amount:
                    raise ValidationError("Approved Amount can not be greater then Claimed Amount")

    @api.constrains('invoice_number')
    @api.onchange('invoice_number')
    def _onchange_invoice_number(self):
        for rec in self:
            check_alnum = rec.invoice_number.isalnum() if rec.invoice_number else True
            if check_alnum == False:
                raise ValidationError("Invoice number should be alphanumeric")
        ...
        for rec in self:
            if rec.remarks:
                if len(rec.remarks) > 200:
                    raise ValidationError('Remarks number of characters must not exceed 200')

    @api.constrains('tution_document')
    @api.onchange('tution_document')
    def _onchange_tution_document(self):
        allowed_file_list = ['image/jpeg', 'image/jpg', 'image/png', 'application/pdf',
                             'application/vnd.openxmlformats-officedocument.wordprocessingml.document']
        for record in self:
            if record.tution_document:
                acp_size = ((len(record.tution_document) * 3 / 4) / 1024) / 1024
                mimetype = guess_mimetype(base64.b64decode(record.tution_document))
    ...
    def check_phone(self):
        ...
        for record in self:
            if record.phone:
                acp_size = ((len(record.phone) * 3 / 4) / 1024) / 1024
                mimetype = guess_mimetype(base64.b64decode(record.phone))
                if str(mimetype) not in allowed_file_list:
                    raise ValidationError("Only .jpeg, .jpg, .png, .pdf, .doc, .docx format are allowed.")
                if acp_size > 2:
                    raise ValidationError("Maximum file size is 2 MB / अधिकतम फ़ाइल आकार 2 एमबी है")
    ...
    def _onchange_bill_no(self):
        for rec in self:
            check_alnum = rec.bill_no.isalnum() if rec.bill_no else True
            if check_alnum == False:
                raise ValidationError("Bill number should be alphanumeric")

    @api.constrains('billing_from', 'billing_to')
    @api.onchange('billing_from', 'billing_to')
    def _check_billing_from(self):
        for rec in self:
            print("today today", rec.billing_from, rec.billing_to)
            if rec.billing_from and rec.billing_to:
                if rec.billing_from > rec.billing_to:
                    raise ValidationError('Your Billing From date should not be greater than Billing To date.')

    @api.constrains('bill_due_date')
    @api.onchange('bill_due_date')
    def _check_due_date(self):  
        for rec in self:
             if rec.billing_to:
                bill_fix_date = rec.billing_to + timedelta(days=7)
                if rec.bill_due_date and rec.billing_to:
                    if rec.bill_due_date < rec.billing_to:
                        raise ValidationError('Your Billing due date should not be less than Billing To date.')
                if rec.bill_due_date and bill_fix_date:
                    if rec.bill_due_date > bill_fix_date:
                        raise ValidationError('Your Billing due date be max 7 days of Billing To date')
    ...
    def _check_working_days(self):
        for rec in self:
            if rec.working_days:
                check_digit = (rec.working_days).isdigit()
                if check_digit is False:
                    raise UserError("Enter valid  number / Only Use Numeric Digits in number of days.")
    ...
    def _check_mobile_no(self):
        for rec in self:
            if rec.mobile_no:
                check_digit = (rec.mobile_no).isdigit()
                if check_digit is False:
                    raise UserError("Enter valid  number / Only Use Numeric Digits in Telephone or Landline No.")
                if len(rec.mobile_no) > 10:
                    raise UserError("Telephone or Landline No. should not be greater then 10")

    @api.constrains('attach_news')
    @api.onchange('attach_news')
    def _onchange_attach_news(self):
        allowed_file_list = ['image/jpeg', 'image/jpg', 'image/png', 'application/pdf',
                             'application/vnd.openxmlformats-officedocument.wordprocessingml.document']
        for record in self:
            if record.attach_news:
                acp_size = ((len(record.attach_news) * 3 / 4) / 1024) / 1024
                mimetype = guess_mimetype(base64.b64decode(record.attach_news))
                if str(mimetype) not in allowed_file_list:
                    raise ValidationError("Only .jpeg, .jpg, .png, .pdf, .doc, .docx format are allowed.")
                if acp_size > 2:
                    raise ValidationError("Maximum file size is 2 MB / अधिकतम फ़ाइल आकार 2 एमबी है")
    ...
    def _check_invoice_date(self):
        for rec in self:
            if rec.brief_date and rec.invoice_date:
                if rec.invoice_date > rec.brief_date:
                    raise ValidationError('Your invoice date should not be greater than Date.')

    def create_payment_advices(self):
        if self.env.user.has_group('reimbursement_bsscl.group_finance_authority'):
            line_ids, emp_lis = [], []
    ...
    def onchange_emp_get_base(self):
        ...
        if self.employee_id:
            ...
            self.branch_id = self.employee_id.current_office_id
    ...
    def compute_net_amount(self):
        for rec in self:
            if self.env.user.has_group('reimbursement_bsscl.group_approving_authority'):
            ...
            if rec.employee_id and rec.reimbursement_type_id.code == 'medical':
                joining_date = rec.employee_id.transfer_date
    ...
    def create(self, vals):
        ...
        if res.reimbursement_type_id.code != 'briefcase':
            if res.employee_id.transfer_date > res.date_range.date_end:
            ...
            print("kjjjjjjjjjjjjjjjjjjjjjjjjjj", res.employee_id,res.employee_id.transfer_date)
            if res.employee_id.transfer_date > res.brief_date:
                raise ValidationError(f"You can not apply for {res.reimbursement_type_id.name}\
                                            prior to your joining date.")
    ...
    def open_approve_wizard(self):
        ...
        group_code = self.job_id.pay_level_id.group_id.code
        form_view_id = self.env.ref("reimbursement_bsscl.view_reimbursement_attendance_compare_form").id 
        action= {
        ...
        if self.reimbursement_type_id.code not in ['lunch','el_encashment']:
            form_view_id = self.env.ref("reimbursement_bsscl.view_claim_amount_eligible_amount_form").id
    ...
    def _onchange_name(self):
        for rec in self:
            if rec.name and not re.match(r'^[A-Za-z]{1}[A-Za-z\s]*$', str(rec.name)):
                raise ValidationError("Service Provider should be an alphabet")
