...
class EmployeeChart(http.Controller):
	...
	@http.route('/get/colspan', type='json', auth='public', method=['POST'], csrf=False)
	def get_col_span(self, emp_id):
		if emp_id:
			employee = request.env['hr.job'].sudo().browse(int(emp_id))
			if employee:
				child_count = len(employee) * 4
				return child_count

	@http.route('/get/recruitment/job', type='json', auth='public', method=['POST'], csrf=False)
		...
		if child_ids:
			...
			for child in child_ids:
				...
				view = """ <div id='""" + str(child.id) + """' class='o_level_1'><a>
                    ...
                    <img style="background: black;"/></div>
                    <div class='employee_name'>
                    <p>""" + str(child.name) + """</p></div></a></div>"""
				child_nodes += child_table + view + """</div></td></tr></table></td>"""
			nodes = child_nodes + """</tr>"""
			return nodes

	def get_nodes_sub_child (self,child_ids):
		if child_ids:
			child_nodes="""<tr>"""
			for child in child_ids:
				child_table="""<td colspan='""" + str(2) + """'>
				...
				<p > Current No of Employee <span>&#8594;</span> """ + str(child.no_of_employee) + """</p>   
				<p > Expected no of Employee <span>&#8594;</span> """ + str(child.no_of_recruitment) + """</p>
				<p > Total Forcaste of Employee <span>&#8594;</span> """ + str(child.expected_employees) + """</p>
				<p > Hired Employee  <span>&#8594;</span>    """ + str(child.no_of_hired_employee) + """</p> 
				</div></a></div>"""
				child_nodes+=child_table + view + """</div></td></tr></table></td>"""
			nodes=child_nodes + """</tr>"""
			return nodes
	...
	@http.route('/get/recruitjob/opening', type='http', auth='user', method=['POST'], csrf=False)
	def get_parent_child(self, **post):
		if post:
			...
			for line in post:
				if line:
					val = int(line)
			child_ids = request.env['hr.job'].sudo().search([])
			...
			view = """ <div id="parent" class='o_chart_head'><a>
					...
					<div class='employee_name o_width'>
					<p>BSSCL Recruitment Structure</p></div></a></div>"""
	...
	@http.route('/get/data/rec', type='json', auth='user', method=['POST'], csrf=False)
	def get_child_data(self, click_id):
		if click_id:
			employee = request.env['hr.job'].sudo().browse(int(click_id))
			if employee:
				child_count = len(employee) * 2
				value = [child_count]
				lines = self.get_lines(child_count)
				nodes = self.get_nodes_sub_child(employee)
