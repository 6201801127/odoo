...
class ExceptionRule(models.Model):
    ...
    model = fields.Selection(
        selection=[],
        string='Apply on')
    active = fields.Boolean('Active / सक्रिय')
...
class Approvalslist(models.Model):
    ...
    model_id = fields.Many2one('ir.model', string='Approval Record / अनुमति रिकॉर्ड')
    model_name =fields.Char(related="model_id.name",string='Model / मॉडल')
    branch_id = fields.Many2one('res.branch',string="Center / शाखा", store=True)
    date = fields.Date(string='Requesting Date / अनुरोध की तारीख')
    user_id = fields.Many2one('res.users', string='Requesting User / अनुरोध करने वाला उपयोगकर्ता')
    rule_id = fields.Many2one('exception.rule', string='Approval Matrix / अनुमति मैट्रिक्स')
    group_id = fields.Many2one('res.groups', string='Approving Groups / अनुमति समूह')
    test_int = fields.Integer()
    resource_ref = fields.Reference(
        string='Record / रिकॉर्ड', selection='_selection_target_model',
    )
    state = fields.Selection(
        [('pending_approval', 'Pending Approval'),('approved', 'Approved'), ('rejected', 'Rejected')],
        string='State / स्थिति', default="pending_approval")
    group_approval_id = fields.Many2one('group.and.approval')
    approvals_done = fields.Integer(string="Approval Done / अनुमति किया हुआ")
    rejections_done = fields.Integer(string="Rejections Done / अस्वीकरण किया")
    approvals_required = fields.Integer(related='group_approval_id.minimum_approval' ,string="Approvals Required / अनुमति अपेक्षित")
    rejections_required = fields.Integer(related='group_approval_id.minimum_rejection',string="Rejections Required / अस्वीकरण अपेक्षित")
    approval_user_matrix_id = fields.One2many('approval.user.matrix','approval_id')

    day_approval=fields.Integer(string="Days To Approve / स्वीकृत करने के लिए दिन")
    approval_deadline=fields.Date(string="Approval DeadLine Date / अनुमति समय सीमा तारीख")
    color = fields.Integer('Color Index / रंग सूचकांक', default=0)
    condition_check = fields.Boolean('Condition Check / शर्त की जाँच', default=0)
...
# Code not working------------- shifted to wizard refuse_reason_wiz.py (def refuse_reason)
    def approve(self):
        print("--------------------working------------------")
        # import pdb
        # pdb.set_trace()
        if self.env.user.id in self.group_id.users.ids:
            ...
            if len(self.rule_id.group_approval_ids)>1:
                print("-----------------------------here it comes-------------------length=-",len(self.rule_id.group_approval_ids))
                for exp_line in self.rule_id.group_approval_ids:
                    print("-00000000000000", exp_line,exp_line.group)
                    if exp_line.group == self.group_id:
                        print("iuiuiuiiiiiiiiiiiiigroup", exp_line.sequence ,(exp_line.sequence + 1) )
                        matrix_check = self.env['group.and.approval'].search([('sequence','=',(exp_line.sequence + 1)), ('rule_id', '=', self.rule_id.id)])
                        print('matrix_checkoooooooooooooooooooooo', matrix_check, matrix_check.group.name)
                        exp1 = self.env['approvals.list'].search([('group_id','=', matrix_check.group.id),('resource_ref','=',self.resource_ref._name +','+ str(self.resource_ref.id))])
                        print('-==========', exp1)
                        for rec in exp1:
