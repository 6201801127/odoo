...
class Arrear(models.Model):
    ...
    da_amount = fields.Float(string="DA Amount")
    ta_amount = fields.Float(string="TA Amount")
    hra_amount = fields.Float(string="HRA Amount")
    gross_amount = fields.Float(string="Gross Amount")
    mpf_amount = fields.Float(string="MPF Amount")
    eps_amount = fields.Float(string="EPS Amount")
    epf_amount = fields.Float(string="EPF Amount")
    ...
    def check_mid_month_eps_factor(self, empObj,  payslip=None, **kwargs):
        ...
        if payslip:
            mid_month_join = payslip.date_from >= empObj.transfer_date <= payslip.date_to
            ...
            if mid_month_join:
                no_of_days = (payslip.date_to - empObj.transfer_date).days
            if mid_month_leave:
                no_of_days = (empObj.last_working_date - payslip.date_from).days
            if mid_month_join and mid_month_leave:
                no_of_days = (empObj.last_working_date - empObj.transfer_date).days
        else:
            date_from, date_to = kwargs.get('date_from'), kwargs.get('date_to')
            mid_month_join = date_from >= empObj.transfer_date <= date_to
            ...
            if mid_month_join:
                no_of_days = (date_to - empObj.transfer_date).days
            if mid_month_leave:
        ...
        if self.employee_id:
            ...
            if self.arrear_type in ('dr', 'promotion'):
                ...
                for contract in emp_contracts:
                    ...
                    date = self.date_to
                    month_days = self.calculate_month_days( date)
...
class ArrearBatch(models.Model):
    ...
    def validate_char_and_num(self):
        if self.name:
            ...
            if result == False:
                raise ValidationError('Only use characters')
