...
class kw_adv_claim_settlement(models.Model):
    ...
    claim_bill_line_ids = fields.One2many('kw_advance_claim_bill_line', 'claim_line_id', string="Claim Bill Lines", store=True)
    ...
    # section = fields.Many2one('hr.department', string="Section", related='empl_id.section', store=True)
    # practise = fields.Many2one('hr.department', string="Practice", related='empl_id.practise', store=True)
    ...
    @api.constrains('claim_bill_line_ids')
    def check_validation(self):
        for data in self:
            if len(data.claim_bill_line_ids) == 0:
                raise ValidationError("Please fill the claim bill data.")
    ...
    def claim_take_action_button(self):
        ...
        action = self.env.ref('kw_advance_claim.kw_adv_claim_settlement_takeaction_form_refresh').read()[0]
        target_id = self.id
