...
class FiscalYearFilter(models.TransientModel):
    ...
    def get_yearly_declarations(self):
        if self.date_range:
            ...
            active_true_emp = total_emp.filtered(lambda
                                                     x: x.active == True and x.transfer_date <= self.date_range.date_stop and x.employement_type.code != 'O').ids
            active_false_emp = total_emp.filtered(lambda x: x.active == False and x.employee_type != 'regular')
            active_false_emp = active_false_emp.filtered(
                lambda x: x.transfer_date <= self.date_range.date_stop if x.transfer_date else None)
            # active_false_emp = active_false_emp.filtered(
            #     lambda x: x.last_working_day <= self.date_range.date_stop if x.last_working_day else None).ids
            ...
            if total_employees:
                ...
                for emp in total_employees:
                    payslip = self.env['hr.payslip'].sudo().search([('employee_id', '=', emp), ('state', '=', 'done'),
                                                                    ('date_to', '>=', self.date_range.date_start),
                                                                    ('date_to', '<=', self.date_range.date_stop)])
                    payslip_basic = 0
                    for line in payslip:
                        payslip_basic = sum(line.line_ids.filtered(lambda x: x.code == 'BASIC').mapped('amount'))
                    contract = self.env['hr.contract'].sudo().search([('employee_id', '=', emp)], order='id desc',
                                                                     limit=1)
    ...
    def get_no_declarations(self):
        if self.date_range:
            payslip_emp = self.env['hr.payslip'].sudo().search(
                [('date_to', '>=', self.date_range.date_start), ('date_to', '<=', self.date_range.date_stop),
                 ('state', '=', 'done')]).mapped('employee_id.id')
            unique_emp = set(payslip_emp)
            total_employees = list(unique_emp)
            if total_employees:
                declarations = self.env['hr.declaration'].sudo().search(
                    [('employee_id', 'in', total_employees), ('date_range', '=', self.date_range.id)]).mapped(
                    'employee_id').ids
                ...
                for var in non_declaration_emp:
                    payslip = self.env['hr.payslip'].sudo().search(
                        [('employee_id', '=', var), ('date_to', '>=', self.date_range.date_start),
                         ('date_to', '<=', self.date_range.date_stop), ('state', '=', 'done')])
                    if payslip:
                ...
                for employee in declarations:
                    ...
                    for ids in tds_data:
                        tds_previous_employer = ids['tds_previous_employer'] if ids[
                                                                                    'tds_previous_employer'] != None else 0
                        housing_loan_institute = ids['housing_loan_institute'] if ids[
                                                                                      'housing_loan_institute'] != None else '--'
                        housing_loan_institute_pan = ids['housing_loan_institute_pan'] if ids[
                                                                                              'housing_loan_institute_pan'] != None else '--'
...
class NonHrDeclaration(models.Model):
    ...
    date_of_joining = fields.Date(string='Date to which Employed with Current Employer', default=lambda self: self.employee_id.transfer_date)
    # last_working_day = fields.Date(related='employee_id.last_working_day',
    #                                string='Date of Leaving Service from Current Employer')
    ...
    state = fields.Selection([('draft', 'Submitted'), ('approved', 'Approved'), ('not_submitted', 'Not-Submitted')],
    ...
    @api.depends('taxable_income', 'red', 'tax_payable', 'gross_total_income', 'final_amount', 'rebate',
                 'net_tax_payable', 'additional_sub_chrg', 'additional_edu_cess', 'total_tax_payable',
                 'balance_tax_payable', 'date_range')
            ...
            if rec.red == 2:
                ...
                if rec.cal_taxable_income > 250000 and rec.cal_taxable_income <= 500000:
                    ...
                    tax_slab = self.env['tax_slab'].sudo().search(
                        [('date_range', '=', rec.date_range.id), ('salary_from', '<=', rec.cal_taxable_income),
                         ('salary_to', '>=', rec.cal_taxable_income), ('tax_regime', '=', 'old_regime')], limit=1)
                ...
                tax_slab_rec = self.env['tax_slab'].sudo().search(
                    [('date_range', '=', rec.date_range.id), ('salary_from', '<=', rec.cal_taxable_income),
                     ('salary_to', '>=', rec.cal_taxable_income), ('tax_regime', '=', 'old_regime')], limit=1)
                rec.cal_additional_sub_chrg = rec.cal_net_tax_payable * tax_slab_rec.surcharge / 100
                rec.cal_additional_edu_cess = rec.cal_net_tax_payable * tax_slab_rec.cess / 100
                total_tax_payable = 0
                var_tax_payable = 0
    ...
    def view_yearly_hr_declaration(self):
        ...
        for rec in self:
            ...
            #         tds_above5percentage = tds_above5 * tax_slab.tax_rate / 100
            #         tax_slab_1 = self.env['tax_slab'].sudo().search(
            #             [('salary_from', '>=', 250001), ('salary_to', '<=', 5000000),
            #              ('tax_regime', '=', 'old_regime')], limit=1)
            #         tds_below5percentage = 250000 * tax_slab_1.tax_rate / 100
            #         temp_tax_payable = tds_above5percentage + tds_below5percentage
