odoo.define('kw_accounting.account_report', function (require) {
'use strict';
...
var M2MFilters = AbstractAction.extend({
    /**
     * @constructor
     * @param {Object} fields
     */
 ...
 var accountReportsWidget = AbstractAction.extend({
    ...
    events: {
        'click .o_account_reports_filter_input': 'filter_accounts',
        ...
        'click .js_account_report_bool_filter': 'js_account_report_bool_filter',
        'click .js_account_report_choice_filter': 'js_account_report_choice_filter',
        'click .js_account_report_date_filter': 'js_account_report_date_filter',
        'click .js_account_report_date_cmp_filter': 'js_account_report_date_cmp_filter',
        'click .js_foldable_trigger': 'js_foldable_trigger',
    },
    ...
    init: function(parent, action) {
        ...
        return this._super.apply(this, arguments);
         console.log("dadasdassssssssss", action)
    },
    start: function() {
        var self = this;
    ...
    render_searchview_buttons: function() {
        var self = this;
        console.log("jjjjjjjjjjjj",this.$searchview_buttons)
...
//            }
        ...
        _.each(this.$searchview_buttons.find('.js_format_date'), function(dt) {
            ...
            $(dt).html((new moment(date_value)).formajs_account_reports_datetimepickert('ll'));
        });
        ...
        this.$searchview_buttons.find('.js_account_report_bool_filter').each(function () {
            var $filter = $(this);
            $filter.toggleClass('selected', self.report_options[$filter.data('filter')]);
        });
...
//        });
        ...
        this.$searchview_buttons.find('.js_account_report_bool_filter').on('click', function (event) {
            var option_value = $(this).data('filter');
            console.log("dasdasssssssssssssssyadav");
            console.log(option_value, 'KJKJJJVJJG');
        ...
        this.$searchview_buttons.find('.js_account_report_choice_filter').click(function (event) {
            var option_value = $(this).data('filter');
            colsole.log('option', option_value)
    ...
    unfold: function(line) {
     ...
     js_account_report_bool_filter: function (ev) {
        var option_value = $(ev.currentTarget).data('filter');
        var ab = this.report_options['hierarchy'];
        console.log("hhhhhhhhhhhhhh", ab, option_value);
        console.log('DEEPAK', option_value);
        this.report_options['hierarchy'] = !this.report_options['hierarchy'];
        if (option_value === 'unfold_all') {
            var dd = this.unfold_all(this.report_options[option_value]);
            console.log(dd, 'FFFFF');
        }
        this.reload();
     },

//     js_account_reports_one_choice_filter

//    js_account_report_choice_filter: function() {
...
//console.log(sup_ready, 'supppppp')
//    });
...
//});
    ...
    js_account_report_choice_filter: function (ev) {
    var option = $(ev.target).attr('data-filter')
    var data_id = $(ev.target).attr('data-id')
    _.filter(this.report_options[option], function(el) {
                    if (''+el.id == ''+data_id){
                        if (el.selected === undefined || el.selected === null){el.selected = false;}
                        el.selected = !el.selected;
                    } else if (option === 'ir_filters') {
                        el.selected = false;
                    }
                    return el;
                });
...
js_account_report_date_filter: function (ev) {
var filter_option = $(ev.target).attr('data-filter')
console.log(filter_option, 'filterrrrr')
    this.report_options.date.filter = filter_option;
            var error = false;
            if (filter_option === 'custom_filter_date') {
                var date_from = self.$searchview_buttons.find('.o_datepicker_input[name="date_from"]');
                var date_to = self.$searchview_buttons.find('.o_datepicker_input[name="date_to"]');
                if (date_from.length > 0){
                    error = date_from.val() === "" || date_to.val() === "";
...
js_foldable_trigger: function(ev){
var filter_option = $(ev.target).attr('data-filter')
console.log( 'FFFFF')
console.log(filter_option, 'FFFFF')
$(document).on('click','a[data-filter^="'+filter_option+'"]',function(event){
console.log(event, 'HHHHHHHHHH')
            var a = $(this).toggleClass('o_closed_menu o_open_menu');
            console.log(a, 'aaaaaaa')
//            $('div.o_foldable_menu').next().next();
            $(this).next('.o_foldable_menu[data-filter="'+filter_option+'"]').toggleClass('o_closed_menu');
//                        $('body').find('.o_foldable_menu[data-filter="'+filter_option+'"]').toggleClass('o_closed_menu');
...
js_account_report_date_cmp_filter: function (ev) {
    var filter_option = $(ev.target).attr('data-filter');
    console.log(filter_option, 'mainnnnn')
    this.report_options.comparison.filter = filter_option;
    var error = false;
    var number_period = $(ev.currentTarget).parent().find('input[name="periods_number"]');
    this.report_options.comparison.number_period = (number_period.length > 0) ? parseInt(number_period.val()) : 1;
    if (filter_option === 'custom') {
        var date_from = this.$searchview_buttons.find('.o_datepicker_input[name="date_from_cmp"]');
        var date_to = this.$searchview_buttons.find('.o_datepicker_input[name="date_to_cmp"]');
        if (date_from.length > 0) {
            error = date_from.val() === "" || date_to.val() === "";
            this.report_options.comparison.date_from = field_utils.parse.date(date_from.val());
            this.report_options.comparison.date_to = field_utils.parse.date(date_to.val());
        } else {
            error = date_to.val() === "";
            this.report_options.comparison.date = field_utils.parse.date(date_to.val());
        }
    }
    if (error) {
        crash_manager.show_warning({ data: { message: _t('Date cannot be empty') } });
