...
class report_account_aged_partner(models.AbstractModel):
    ...
    def _get_lines(self, options, line_id=None):
        ...
        results, total, amls = self.env['report.base_accounting_kit.report_agedpartnerbalance'].with_context(include_nullified_amount=True)._get_partner_move_lines(account_types, self._context['date_to'], 'posted', 30)
        for values in results:
            ...
            vals = {
                ...
                'name': values['name'],
                'level': 2,
                'columns': [{'name': ''}] * 3 + [{'name': (self.with_context(no_format=True).format_value((sign * v)) if (sign * v) < 0 else self.with_context(no_format=True).format_value((sign * v)))} for v in [values['direction'], values['4'],
                                                                                                 values['3'], values['2'],
                ...
                for line in amls[values['partner_id']]:
                    ...
                    vals = {
                        ...
                        'columns': [{'name': v} for v in [aml.journal_id.code, aml.account_id.code, self._format_aml_name(aml)]] +\
                                   [{'name': v} for v in [line['period'] == 6-i and (self.with_context(no_format=True).format_value((sign * line['amount'])) if (sign * line['amount']) < 0 else self.with_context(no_format=True).format_value((sign * line['amount']))) or '' for i in range(7)]],
        ...
        if total and not line_id:
            total_line = {
                ...
                'class': 'total',
                'level': 2,
                'columns': [{'name': ''}] * 3 + [{'name': (self.with_context(no_format=True).format_value((sign * v)) if (sign * v) < 0 else self.with_context(no_format=True).format_value((sign * v)))} for v in [total[6], total[4], total[3], total[2], total[1], total[0], total[5]]],
            }
