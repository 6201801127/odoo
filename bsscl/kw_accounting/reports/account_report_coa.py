...
class report_account_coa(models.AbstractModel):
    ...
    def _post_process(self, grouped_accounts, initial_balances, options, comparison_table):
        ...
        for account in sorted_accounts:
            ...
            cols = [
                {'name': initial_balance > 0 and self.with_context(no_format=True).format_value(initial_balance) or zero_value, 'no_format_name': initial_balance > 0 and initial_balance or 0,'style': 'text-align:right'},
                {'name': initial_balance < 0 and self.with_context(no_format=True).format_value(-initial_balance) or zero_value, 'no_format_name': initial_balance < 0 and abs(initial_balance) or 0, 'style': ''},
            ]
            total_periods = 0
            for period in range(len(comparison_table)):
                ...
                cols += [{'name': debit > 0 and self.with_context(no_format=True).format_value(debit) or zero_value, 'no_format_name': debit > 0 and debit or 0,'style': 'text-align:right'},
                         {'name': credit > 0 and self.with_context(no_format=True).format_value(credit) or zero_value, 'no_format_name': credit > 0 and abs(credit) or 0, 'style': ''}]
            ...
            cols += [
                {'name': total_amount > 0 and self.with_context(no_format=True).format_value(total_amount) or zero_value, 'no_format_name': total_amount > 0 and total_amount or 0},
                {'name': total_amount < 0 and self.with_context(no_format=True).format_value(-total_amount) or zero_value, 'no_format_name': total_amount < 0 and abs(total_amount) or 0},
                ]
            name = account.name
            lines.append({
                'id': account.id,
        ...
        lines.append({
             ...
             'name': _('Total'),
             'class': 'total',
             'columns': [{'name': self.with_context(no_format=True).format_value(v['amount'])} for v in formatted_amount_list],
             'level': 1,
        })
        return lines
