    ...
    def _get_lines(self, options, line_id=None):
        ...
        for account in sorted_accounts:
            ...
            if offset == 0:
                lines.append({
                    ...
                    'columns': [{'name': v} for v in ['',amount_currency, self.with_context(no_format=True).format_value(debit), self.with_context(no_format=True).format_value(credit), (self.with_context(no_format=True).format_value((balance)) if balance < 0 else self.with_context(no_format=True).format_value((balance)))]],
                    'level': 2,
                    'unfoldable': True,
            ...
            if 'account_%s' % (account.id,) in options.get('unfolded_lines') or unfold_all:
                ...
                if offset == 0:
                    domain_lines.append({
                        ...
                        'columns': [{'name': v} for v in ['', '','', '', initial_currency, self.with_context(no_format=True).format_value(initial_debit), self.with_context(no_format=True).format_value(initial_credit), (self.with_context(no_format=True).format_value((initial_balance)) if initial_balance < 0 else self.with_context(no_format=True).format_value((initial_balance)))]],
                    })
                ...
                for line in amls:
                    ...
                    narration = (line.move_id.narration[:32] + "..." if len(line.move_id.narration) > 35 else line.move_id.narration) if line.move_id.narration else ''
                    name_title = name
                    ...
                    columns = [{'name': v} for v in [format_date(self.env, line.date), name, narration, partner_name, currency,
                                    line_debit != 0 and self.with_context(no_format=True).format_value(line_debit) or '',
                                    line_credit != 0 and self.with_context(no_format=True).format_value(line_credit) or '',
                                    (self.with_context(no_format=True).format_value((progress)) if progress < 0 else self.with_context(no_format=True).format_value((progress)))]]
        ...
        if not line_id:
            ...
            lines.append({
                ...
                'class': 'total',
                'level': 1,
                'columns': [{'name': v} for v in ['', '','', '', '', self.with_context(no_format=True).format_value(sum_debit), self.with_context(no_format=True).format_value(sum_credit), (self.with_context(no_format=True).format_value((abs(sum_balance))) if sum_balance < 0 else self.with_context(no_format=True).format_value((abs(sum_balance))))]],
            })
