...
class ReportAccountFinancialReport(models.Model):
	...
	date_range=fields.Boolean('Based on date ranges',default=True,
		help='specify if the report use date_range or single date')
	...
	hierarchy_option=fields.Boolean('Enable the hierarchy option',
		help='Display the hierarchy choice in the report options')
	...
	tax_report=fields.Boolean('Tax Report',
		help="Set to True to automatically filter out journal items that have the boolean field 'tax_exigible' set to False")
	filter_unfold_all=fields.Boolean('Unfold',
		help="Set to True to automatically filter out journal items that have the boolean field 'tax_exigible' set to False")
				...
				if last_group!=current_group or group_index in row_splits:
					current_hierarchy_line.append(
						{# field_index - 1 because ['period'] is not part of options['groups']['fields']
							'name':                                                     last_group.get(
								'string') if field=='period' else self._get_column_name(last_group,
								options['groups']['fields'][field_index - 1]),'colspan':current_colspan,
							'class':                                                    'number',})
					last_group=current_group
					current_colspan=0
	...
	def _get_filter_journals (self):
		if self==self.env.ref('kw_accounting.account_financial_report_cashsummary0'):
			return self.env['account.journal'].search(
				[('company_id','in',self.env.user.company_ids.ids or [self.env.user.company_id.id]),
					('type','in',['bank','cash'])],order="company_id, name")
	...
	def _get_options (self,previous_options=None):
		if self.date_range:
			...
			if self.comparison:
				filter_comparison={'date':'','filter':'no_comparison','number_period':1}
		if self.unfold_all_filter:
			self.filter_unfold_all=False
		if self.show_journal_filter:
			filter_journals=True
		filter_all_entries=False
		filter_analytic=self.analytic or None
		if self.analytic:
			filter_analytic_accounts=[] if self.env.user.id in self.env.ref(
				'analytic.group_analytic_accounting').users.ids else None
			filter_analytic_tags=[] if self.env.user.id in self.env.ref(
				'analytic.group_analytic_tags').users.ids else None
			#don't display the analytic filtering options if no option would be shown
			if filter_analytic_accounts is None and filter_analytic_tags is None:
				filter_analytic=None
		filter_hierarchy=True if self.hierarchy_option else None
		filter_ir_filters=self.applicable_filters_ids or None
	...
	def _create_action_and_menu (self,parent_id):
		...
		for report in self:
			if not report.generated_menu_id:
				action_vals={'name':report._get_report_name(),'tag':'account_report',
					'context':      {'model':'account.financial.html.report','id':report.id,},}
				...
				menu_vals={'name':report._get_report_name(),
	...
	def _get_lines (self,options,line_id=None):
		...
		res=line_obj.with_context(cash_basis=options.get('cash_basis'),filter_domain=domain,)._get_lines(self,
			currency_table,options,linesDicts)
		return res
...
class AccountFinancialReportLine(models.Model):
	...
	figure_type=fields.Selection([('float','Float'),('percents','Percents'),('no_unit','No Unit')],'Type',
	...
	special_date_changer=fields.Selection(
		[('from_beginning','From the beginning'),('to_beginning_of_period','At the beginning of the period'),
			('normal','Use given dates'),('strict_range','Force given dates for all accounts and account types'),
			('from_fiscalyear','From the beginning of the fiscal year'),],default='normal')
	...
	_sql_constraints=[('code_uniq','unique (code)',"A report line with the same code already exists."),]
	...
	def _copy_hierarchy (self,report_id=None,copied_report_id=None,parent_id=None,code_mapping=None):
		...
		copy_line_id=self.copy({'financial_report_id':financial_report_id,'parent_id':parent_id and parent_id.id,
	...
	def _get_with_statement (self,financial_report):
		...
		if financial_report==self.env.ref('kw_accounting.account_financial_report_cashsummary0'):
			...
			bank_accounts=bank_journals.mapped('default_debit_account_id') + bank_journals.mapped(
				'default_credit_account_id')

			if not bank_accounts:
				raise UserError(
					_('No bank account found. Please set Default Debit and Default Credit accounts in journal(s): %s.')%", ".join(
						str(b.name) for b in bank_journals))
			self._cr.execute('SELECT DISTINCT(move_id) FROM account_move_line WHERE account_id IN %s',
				[tuple(bank_accounts.ids)])
			...
			if self.code in ('CASHEND','CASHSTART'):
                ...
                WITH account_move_line AS (
                    ...
                    FROM account_move_line
                    WHERE account_id in %s
                    ''' + journal_ids_filter + ''')''',[tuple(bank_accounts.ids)] + journal_ids_params
			...
			sql='''
                WITH account_move_line AS (
                    ...
                    SELECT ''' + select_clause_1.replace('"account_move_line"."balance_cash_basis"',
				'CASE WHEN acc.reconcile THEN  "account_move_line".amount_residual ELSE "account_move_line".balance END AS balance_cash_basis') + '''
                    FROM account_move_line "account_move_line"
            ...
            '''
			params=bank_move_params + bank_move_params + bank_move_params + bank_move_params + [
				tuple(bank_accounts.ids)]
	...
	def report_move_lines_action (self):
		...
		return {'type': 'ir.actions.act_window','name':'Journal Items (%s)'%self.name,'res_model':'account.move.line',
	...
	def _get_sum (self,currency_table,financial_report,field_names=None):
		...
		if self.domain:
			...
			res=self.with_context(strict_range=strict_range,date_from=date_from,date_to=date_to,
				active_test=False)._compute_line(currency_table,financial_report,group_by=self.groupby,
	...
	def _get_rows_count (self):
		...
		tables,where_clause,where_params=self.env['account.move.line'].with_context(strict_range=strict_range,
			date_from=date_from,date_to=date_to)._query_get(domain=self._get_aml_domain())
	...
	def _get_aml_domain (self):
		return (safe_eval(self.domain) or []) + (self._context.get('filter_domain') or []) + (
			...
			for group_index,group in enumerate(groups['ids']):
				...
				if results_for_group:
					...
					c=FormulaContext(self.env['account.financial.html.report.line'].with_context(
						group_domain=self._get_group_domain(group,groups)),linesDict,currency_table,financial_report,
						only_sum=True)
					if formulas:
	...
	def _put_columns_together (self,data,domain_ids):
		...
		for period in data:
			...
			for domain_id in domain_ids:
				if debit_credit:
					res[domain_id].append(round(period.get(domain_id,{'debit':0})['debit'],2))
					res[domain_id].append(round(period.get(domain_id,{'credit':0})['credit'],2))
				res[domain_id].append(round(period.get(domain_id,{'balance':0})['balance'],2))

		# print(res)
		return res

	def _divide_line (self,line):
		line1={'id':     line['id'],'name':line['name'],'class':line['class'],'level':line['level'],
			'columns':   [{'name':''}]*len(line['columns']),'unfoldable':line['unfoldable'],'unfolded':line['unfolded'],
			'page_break':line['page_break'],}
		line2={'id':  line['id'],'name':_('Total') + ' ' + line['name'],'class':'total','level':line['level'] + 1,
		...
		for line in self:
			...
			for period in comparison_table:
				...
				r=line.with_context(date_from=date_from,date_to=date_to,strict_range=strict_range)._eval_formula(
					financial_report,debit_credit,currency_table,linesDicts[k],groups=options.get('groups'))
				debit_credit=False
				res.extend(r)
				for column in r:
			...
			vals={'id':      line.id,'name':line.name,'level':line.level,
			...
			if line.id in options.get('unfolded_lines',[]) or line.show_domain=='always':
				...
				for domain_id in domain_ids:
					...
					vals={'id':                   domain_id,'name':name,'level':line.level,'parent_id':line.id,
				...
				if domain_ids and self.env.user.company_id.totals_below_sections:
					lines.append({'id':'total_' + str(line.id),'name':_('Total') + ' ' + line.name,'level':line.level,
						'class':       'o_account_reports_domain_total','parent_id':line.id,
			...
			for vals in lines:
				if len(comparison_table)==2 and not options.get('groups'):
					...
					for i in [0,1]:
						vals['columns'][i]=line.with_context(no_format=True)._format(vals['columns'][i])
				else:
					print([v for v in vals['columns']])
					vals['columns']=[line.with_context(no_format=True)._format(v) for v in vals['columns']]
...
class IrModuleModule(models.Model):
	...
	# def _update_translations(self, filter_lang=None):  #     """ Create missing translations after loading the one of account.financial.html.report  #  #     Use the translations of the account.financial.html.report to translate the linked  #     ir.actions.client and ir.ui.menu generated at the creation of the report  #     """  #     res = super(IrModuleModule, self)._update_translations(filter_lang=filter_lang)  #  #     # generated missing action translations for translated reports  #     self.env.cr.execute("""  #        INSERT INTO ir_translation (lang, type, name, res_id, src, value, module, state)  #        SELECT DISTINCT ON (l.code, a.id) l.code, 'model', 'ir.actions.client,name', a.id, t.src, t.value, t.module, t.state  #          FROM account_financial_html_report r  #          JOIN ir_act_client a ON (r.name = a.name)  #          JOIN ir_translation t ON (t.res_id = r.id AND t.name = 'account.financial.html.report,name')  #          JOIN res_lang l on  (l.code = t.lang)  #         WHERE NOT EXISTS (  #               SELECT 1 FROM ir_translation tt  #               WHERE (tt.name = 'ir.actions.client,name'  #                 AND tt.lang = l.code  #                 AND type='model'  #                 AND tt.res_id = a.id)  #               )  #     """)  #  #     # generated missing menu translations for translated reports  #     self.env.cr.execute("""  #        INSERT INTO ir_translation (lang, type, name, res_id, src, value, module, state)  #        SELECT DISTINCT ON (l.code, m.id) l.code, 'model', 'ir.ui.menu,name', m.id, t.src, t.value, t.module, t.state  #          FROM account_financial_html_report r  #          JOIN ir_ui_menu m ON (r.name = m.name)  #          JOIN ir_translation t ON (t.res_id = r.id AND t.name = 'account.financial.html.report,name')  #          JOIN res_lang l on  (l.code = t.lang)  #         WHERE NOT EXISTS (  #               SELECT 1 FROM ir_translation tt  #               WHERE (tt.name = 'ir.ui.menu,name'  #                 AND tt.lang = l.code  #                 AND type='model'  #                 AND tt.res_id = m.id)  #               )  #     """)  #  #     return res
