...
class ReportPartnerLedger(models.AbstractModel):
    ...
    def _get_templates(self):
        ...
        templates['line_template'] = 'kw_accounting.line_template_partner_ledger_report'
        return templates
    ...
    def _get_lines(self, options, line_id=None):
        ...
        for partner in sorted_partners:
            ...
            columns = [(self.with_context(no_format=True).format_value((initial_balance)) if initial_balance < 0 else self.with_context(no_format=True).format_value((initial_balance))), self.with_context(no_format=True).format_value(debit), self.with_context(no_format=True).format_value(credit)]
            if self.user_has_groups('base.group_multi_currency'):
                columns.append('')
            columns.append((self.with_context(no_format=True).format_value((balance)) if balance < 0 else self.with_context(no_format=True).format_value((balance))))
            ...
            if 'partner_' + str(partner.id) in options.get('unfolded_lines') or unfold_all:
                ...
                for line in amls:
                    ...
                    domain_columns = [line.journal_id.code, line.account_id.code, self._format_aml_name(line),
                                      ...
                                      line.full_reconcile_id.name or '', (self.with_context(no_format=True).format_value((progress_before)) if progress_before < 0 else self.with_context(no_format=True).format_value((progress_before))),
                    ...
                    domain_columns.append((self.with_context(no_format=True).format_value((progress)) if progress < 0 else self.with_context(no_format=True).format_value((progress))))
        ...
        if not line_id:
            total_columns = ['', '', '', '', '', (self.with_context(no_format=True).format_value((total_initial_balance)) if total_initial_balance < 0 else self.with_context(no_format=True).format_value((total_initial_balance))), self.with_context(no_format=True).format_value(total_debit), self.with_context(no_format=True).format_value(total_credit)]
            if self.user_has_groups('base.group_multi_currency'):
                total_columns.append('')
            total_columns.append((self.with_context(no_format=True).format_value((total_balance)) if total_balance < 0 else self.with_context(no_format=True).format_value((total_balance))))
            lines.append({
