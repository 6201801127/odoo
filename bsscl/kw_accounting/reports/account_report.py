...
class AccountReport(models.AbstractModel):
	...
	def _build_options (self,previous_options=None):
		...
		filter_list=[attr for attr in dir(self) if
			attr.startswith('filter_') and len(attr)>7 and not callable(getattr(self,attr))]
		...
		if options.get('multi_company'):
			...
			if len(company_ids)>1:
				...
				options['multi_company']=[
					{'id':c.id,'name':c.name,'selected':True if c.id==self.env.user.company_id.id else False} for c in
					companies]
			else:
				del options['multi_company']

		if options.get('journals'):
			print('NNNNNNNNNNN', options.get('journals'))
			print('XXXXX',self._get_journals())
			options['journals']=self._get_journals()
			print('RAAAAAAM', options['journals'])
			...
			if key in previous_options and value is not None and previous_options[key] is not None:
				...
				if key=='date' or key=='comparison':
					...
					if previous_options[key].get('filter','custom')!='custom':
						...
						options[key]['date_from']=company_fiscalyear_dates['date_from'].strftime(
	...
	def _get_templates (self):
		return {'main_template':         'kw_accounting.main_template',
			'main_table_header_template':'kw_accounting.main_table_header',
			'line_template':             'kw_accounting.line_template',
			'footnotes_template':        'kw_accounting.footnotes_template',
			'search_template':           'kw_accounting.search_template',}
	...
	def _resolve_caret_option_view (self,target):
		...
		if target._name=='account.move':
			if target.move_type in ('in_refund','in_invoice'):
				return 'account.move_supplier_form'
			if target.move_type in ('out_refund','out_invoice'):
				return 'account.move_form'
	...
	def open_document (self,options,params=None):
		...
		return {'type': 'ir.actions.act_window','view_type':'tree','view_mode':'form','views':[(view_id,'form')],
			'res_model':document,'view_id':view_id,'res_id':target.id,'context':ctx,}
	...
	def open_tax (self,options,params=None):
		...
		domain=['|',('tax_ids','in',[active_id]),('tax_line_id','in',[active_id])]
	...
	def view_too_many (self,options,params=None):
		...
		if model=='account':
			...
			ctx.update({'search_default_account_id':[int(active_id)],'active_id':int(active_id),})
		if model=='partner':
			action=self.env.ref('account.action_move_line_select_by_partner').read()[0]
			ctx.update({'search_default_partner_id':[int(active_id)],'active_id':int(active_id),})
		action=clean_action(action)
		action['context']=ctx
		return action
	...
	def open_general_ledger (self,options,params=None):
		...
		ctx=self.env.context.copy()
		ctx.pop('id','')
		action=self.env.ref('kw_accounting.action_account_report_general_ledger').read()[0]
	...
	def open_journal_items (self,options,params):
		...
		if params and 'id' in params:
			active_id=params['id']
			ctx.update({'search_default_account_id':[active_id],})

		if options:
			if options.get('journals'):
				print("optionssssssssssssssssssssssss", options)
			...
			print('expression', expression.normalize_domain(ast.literal_eval(action.get('domain','[]'))))
	...
	def _set_context (self,options):
		...
		ctx=self.env.context.copy()
		if self.display_name and self.display_name in ('Profit and Loss', 'Balance Sheet'):
			if self.date_range:
				options['date']={'date_from':datetime.date.today(),'date_to':datetime.date.today(),'filter':'this_year'}
				if self.comparison:
		...
		if options.get('partner_ids'):
			ctx['partner_ids']=self.env['res.partner'].browse([int(partner) for partner in options['partner_ids']])
				...
				ctx['partner_ids']=self.env['res.partner'].browse([int(partner) for partner in self.env['res.partner'].search([])])
	...
	def get_report_informations (self,options):
		...
		if options.get('analytic_accounts') is not None:
			searchview_dict['analytic_accounts']=self.env.user.id in self.env.ref(
				'analytic.group_analytic_accounting').users.ids and [(t.id,t.name) for t in
													 self.env['account.analytic.account'].search([])] or False
		...
		info={'options':      options,'context':self.env.context,'report_manager_id':report_manager.id,
			'footnotes':      [{'id':f.id,'line':f.line,'text':f.text} for f in report_manager.footnotes_ids],
			'buttons':        self._get_reports_buttons(),'main_html':self.get_html(options),
			'searchview_html':self.env['ir.ui.view']._render_template(
				self._get_templates().get('search_template','account_report.search_template'),values=searchview_dict),}
		print('infoooo', info)
		return info
	...
	def _create_hierarchy (self,lines):
		...
		def get_account_codes (account):
			...
			codes=[]
			if account.account_sub_head_id:
				group = account.account_sub_head_id
				if group:
					code = group
					codes.append((MOST_SORT_PRIO, code))
					group = account.account_head_id
					if group:
						code = group
						codes.append((MOST_SORT_PRIO, code))
						...
						if group:
							code = group
							codes.append((MOST_SORT_PRIO, code))
							group = account.user_type_id
							if group:
								code = group
								codes.append((MOST_SORT_PRIO, code))
								group = account.group_type
			else:
				# Limit to 3 levels.
				code=account.code[:3]
				while code:
		...
		def get_hierarchy_lines (values,depth=1):
			...
			sorted_groups=((values.get('children',{}).keys()))
			print( '%%%%%%%%', values.get('children'))
			for key in sorted_groups:
				print(key, '***********&&&&&&&&&')
				sum_columns,sub_lines=get_hierarchy_lines(values['children'][key],depth=values['depth'])
				header_line={'id':'hierarchy','name':key[1],  # second member of the tuple
					'unfoldable': False,'unfolded':True,'level':values['depth'],'parent_id':values['parent_id'],
					'columns':    [
						{'name':self.with_context(no_format=True).format_value(c) if not isinstance(c,str) else c} for c
						in sum_columns],}
	...
	def get_html (self,options,line_id=None,additional_context=None):
		...
		* if line_id is set, the template used will be the line_template
		...
		report_manager=self._get_report_manager(options)
		report={'name':        self._get_report_name(),'summary':report_manager.summary,
			'company_currency':self.env.user.company_id.currency_id.name,'company_name':self.env.user.company_id.name,}
		...
		rcontext={'report':report,'lines':{'columns_header':self.get_header(options),'lines':lines},'options':options,
			'context':     self.env.context,'model':self,}
		if additional_context and type(additional_context)==dict:
			rcontext.update(additional_context)
		if self.env.context.get('analytic_account_ids'):
			rcontext['options']['analytic_account_ids']=[{'id':acc.id,'name':acc.name} for acc in
				self.env.context['analytic_account_ids']]

		render_template=templates.get('main_template','kw_accounting.main_template')
		if line_id is not None:
			render_template=templates.get('line_template','kw_accounting.line_template')
		html=self.env['ir.ui.view']._render_template(render_template,values=dict(rcontext),)
		if self.env.context.get('print_mode',False):
	...
	def get_html_footnotes (self,footnotes):
		template=self._get_templates().get('footnotes_template','kw_accounting.footnotes_template')
	...
	def _get_filter_journals (self):
		print('deepak*****', self.env['account.journal'].search(
	...
	def _get_dates_period (self,options,date_from,date_to,period_type=None):
		...
		* The period type (e.g. quarter) if not specified explicitly.
		...
		return {'string':string,'period_type':period_type,'date_from':date_from,'date_to':date_to,}
	...
	def format_value (self,value,currency=False):
		...
		if self.env.context.get('column_type')=='debit':
			balance_symbol='Dr'
			return '%s %s'%(currency_id.round(value),balance_symbol)
		elif self.env.context.get('column_type')=='credit':
			balance_symbol='Cr'
			return '%s %s'%(currency_id.round(value),balance_symbol)
		if self.env.context.get('no_format'):
			...
			formatted_amount=lang.format(fmt,currency_id.round(value),grouping=True,monetary=True).replace(r' ',
				u'\N{NO-BREAK SPACE}').replace(r'-',u'-\N{ZERO WIDTH NO-BREAK SPACE}')
	...
	def _format_aml_name (self,aml):
		name='-'.join((aml.move_id.name not in ['','/'] and [aml.move_id.name] or []) + (
	...
	def print_pdf (self,options):
		return {'type':'ir_actions_account_report_download',
			'data':    {'model':self.env.context.get('model'),'options':json.dumps(options),'output_format':'pdf',
	...
	def get_pdf (self,options,minimal_layout=True):
		...
		rcontext={'mode':'print','base_url':base_url,'company':self.env.user.company_id,}

		body=self.env['ir.ui.view']._render_template("kw_accounting.print_template",values=dict(rcontext),)
		...
		if minimal_layout:
			...
			rcontext.update({'css':'','o':self.env.user,'res_company':self.env.user.company_id,})
		...
		return self.env['ir.actions.report']._run_wkhtmltopdf([body],header=header,footer=footer,landscape=landscape,
			specific_paperformat_args=spec_paperformat_args)

	def print_xlsx (self,options):
		return {'type':'ir_actions_account_report_download',
			'data':    {'model':self.env.context.get('model'),'options':json.dumps(options),'output_format':'xlsx',
				'financial_id': self.env.context.get('id'),}}
	...
	def get_xlsx (self,options,response):
		...
		level_0_style=workbook.add_format(
			{'font_name':'Arial','bold':True,'font_size':13,'bottom':6,'font_color':'#666666','num_format':'#,###'})
		level_1_style=workbook.add_format(
			{'font_name':'Arial','bold':True,'font_size':13,'bottom':1,'font_color':'#666666','num_format':'#,###'})
		level_2_col1_style=workbook.add_format(
			{'font_name':'Arial','bold':True,'font_size':12,'font_color':'#666666','indent':1,'num_format':'#,###'})
		level_2_col1_total_style=workbook.add_format(
			{'font_name':'Arial','bold':True,'font_size':12,'font_color':'#666666','num_format':'#,###'})
		level_2_style=workbook.add_format(
			{'font_name':'Arial','bold':True,'font_size':12,'font_color':'#666666','num_format':'#,###'})
		level_3_col1_style=workbook.add_format(
			{'font_name':'Arial','font_size':12,'font_color':'#666666','indent':2,'num_format':'#,###'})
		level_3_col1_total_style=workbook.add_format(
			{'font_name':'Arial','bold':True,'font_size':12,'font_color':'#666666','indent':1,'num_format':'#,###'})
		level_3_style=workbook.add_format(
			{'font_name':'Arial','font_size':12,'font_color':'#666666','num_format':'#,###'})

		#Set the first column width to 50
		sheet.set_column(0,0,50)

		if self._get_report_name() in ['Trial Balance','General Ledger']:
		...
		for y in range(0,len(lines)):
			...
			if self._get_report_name() in ['Trial Balance','General Ledger']:
				cell_type,cell_value=self._get_cell_type_value_account_code(lines[y])
				...
				for x in range(1,len(lines[y]['columns']) + 1):
					...
					if cell_type=='date':
						sheet.write_datetime(y + y_offset,x + lines[y].get('colspan',1),cell_value,date_default_style)
					else:
	...
	def _get_cell_type_value_account_code (self,cell):
		# print(cell)
		return ('text',cell.get('account_code',''))

	def print_xml (self,options):
		return {'type':'ir_actions_account_report_download',
			'data':    {'model':self.env.context.get('model'),'options':json.dumps(options),'output_format':'xml',
				'financial_id': self.env.context.get('id'),}}
