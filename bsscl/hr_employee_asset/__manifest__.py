# -*- coding: utf-8 -*-
##############################################################################
#
#    OpenERP, Open Source Management Solution
#    Copyright (C) 2013-2014 CodUP (<http://codup.com>).
#
#    This program is free software: you can redistribute it and/or modify
#    it under the terms of the GNU Affero General Public License as
#    published by the Free Software Foundation, either version 3 of the
#    License, or (at your option) any later version.
#
#    This program is distributed in the hope that it will be useful,
#    but WITHOUT ANY WARRANTY; without even the implied warranty of
#    MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
#    GNU Affero General Public License for more details.
#
#    You should have received a copy of the GNU Affero General Public License
#    along with this program.  If not, see <http://www.gnu.org/licenses/>.
#
##############################################################################

{
    'name': 'Hr Employee Assets',
    'version': '1.6',
    'summary': 'Asset Management',
    'description': """
Managing Assets in Odoo11.
===========================
Support following feature:
    * Location for Asset
    * Assign Asset to employee
    * Track warranty information
    * Custom states of Asset
    * States of Asset for different team: Finance, Warehouse, Manufacture, Maintenance and Accounting
    * Drag&Drop manage states of Asset
    """,
    'author': 'CSM Technologies',
    'website': 'http://www.apagen.com',
    'category': 'Enterprise Asset Management',
    'sequence': 0,
    'images': ['images/assets.png'],
    'depends': ['account', 'product', 'purchase', 'hr', 'bsscl_employee'],
    'data': [
        'security/asset_security.xml',
        'security/ir_rule.xml',
        'asset_view.xml',
        'views/asset_sequence.xml',
        'views/asset.xml',
        # 'views/asset_product_view.xml',
        'views/asset_tracking_data.xml',
        'views/asset_tracking_sequence.xml',
        'views/asset_tracking_view.xml',
        'views/employee_asset_view.xml',
        'security/ir.model.access.csv',
    ],
    'installable': True,
    'application': True,
}
# vim:expandtab:smartindent:tabstop=4:softtabstop=4:shiftwidth=4:
